...
import {
  RequestDetails
} from '../core/models/RequestDetails.js';
  ...
  export class User extends controller.Controllers.Core.Controller {
    ...
    public redirLogin(req, res) {
      if (req.path.indexOf(sails.config.auth.loginPath) == -1) {
        ...
        let url = req.url;
        sails.log.error("determining redirect url")
        sails.log.error(url)
        if (!_.isEmpty(sails.config.http.rootContext)) {
          sails.log.error(sails.config.appUrl)
          sails.log.error(sails.config.http.rootContext)
    ...
    public update(req, res) {
      ...
      if (name) {
        UsersService.updateUserDetails(userid, name, details.email, details.password).subscribe(user => {
          let requestDetails = new RequestDetails(req);
          UsersService.addUserAuditEvent(user, "userDetailsUpdated", requestDetails).subscribe(response => {
            sails.log.debug(`User detauls updated audit event created: ${user.id}`)
          });
    ...
    public generateUserKey(req, res) {
      ...
      if (userid) {
        ...
        UsersService.setUserKey(userid, uuid).subscribe(user => {
          let requestDetails = new RequestDetails(req);
          UsersService.addUserAuditEvent(user, "apiKeySet", requestDetails).subscribe(response => {
            sails.log.debug(`User api key set audit event created: ${user.id}`)
          });
    ...
    public revokeUserKey(req, res) {
      ...
      if (userid) {
        ...
        UsersService.setUserKey(userid, uuid).subscribe(user => {
          let requestDetails = new RequestDetails(req);
          UsersService.addUserAuditEvent(user, "apiKeyRevoked", requestDetails).subscribe(response => {
            sails.log.debug(`User api key revoke audit event created: ${user.id}`)
          });
