...
import { HttpContext, withXhr } from '@angular/common/http';
...
describe('TypeaheadDataService', () => {
  ...
  it('requests historical vocabulary entries when asked', async () => {
    ...
    req.flush({
      data: [{ label: 'Legacy', value: 'legacy', historical: true }],
    });
  ...
  it('normalizes named query response.docs shape with mapping paths', async () => {
    ...
    const req = httpTesting.expectOne(
      request => request.method === 'GET' && request.url.includes('/query/vocab/contributors')
    );
    req.flush({
      data: {
        response: {
  ...
  it('normalizes external provider response using resultArrayProperty', async () => {
    ...
    const req = httpTesting.expectOne(
      request => request.method === 'POST' && request.url.includes('/external/vocab/geonamesCountries')
    );
    ...
    req.flush({
      data: {
        response: {
          docs: [{ utf8_name: 'Australia' }, { utf8_name: 'Austria' }],
        },
      },
    });
  ...
  describe('in-flight request dedup', () => {
    ...
    it('re-fetches once the in-flight request has resolved (no result caching)', async () => {
      ...
      const req1 = httpTesting.expectOne(
        request => request.url.includes('/vocab/access-rights/entries') && request.params.get('search') === 'op'
      );
  ...
  it('posts service lookup request and normalizes wrapped response', async () => {
    ...
    const req = httpTesting.expectOne(
      request => request.method === 'POST' && request.url.includes('/service/vocab/contributors')
    );
    expect(req.request.body).toEqual({
      search: 'jan',
      start: 5,
