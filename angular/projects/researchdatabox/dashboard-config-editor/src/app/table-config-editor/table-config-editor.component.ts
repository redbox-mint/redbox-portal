    ...
    <div class="dc-table-config-editor">
      <ul class="nav nav-tabs dc-tabs" role="tablist">
        <li role="presentation" [class.active]="activeTab === 'columns'">
          <a role="tab" (click)="activeTab = 'columns'">
            ...
            Columns
            @if (config.rowConfig?.length || 0; as count) {
              <span class="dc-tab-count">{{ count }}</span>
            }
          </a>
        </li>
        ...
        <li role="presentation" [class.active]="activeTab === 'rowRules'">
          <a role="tab" (click)="activeTab = 'rowRules'">
            ...
            Row Rules
            @if (config.rowRulesConfig?.length || 0; as count) {
              <span class="dc-tab-count">{{ count }}</span>
            }
          </a>
        </li>
      ...
      <div class="tab-content dc-tab-content">
        @if (activeTab === 'columns') {
          <div class="dc-tab-pane">
            <column-editor [columns]="config.rowConfig || []" (columnsChange)="updateRowConfig($event)"></column-editor>
          </div>
        }
        @if (activeTab === 'formatRules') {
          <div class="dc-tab-pane">
            <format-rules-editor
              [formatRules]="config.formatRules || {}"
              (formatRulesChange)="updateFormatRules($event)"
            ></format-rules-editor>
          </div>
        }
        @if (activeTab === 'rowRules') {
          <div class="dc-tab-pane">
            <rule-set-editor
              [ruleSets]="config.rowRulesConfig || []"
              (ruleSetsChange)="updateRowRules($event)"
            ></rule-set-editor>
          </div>
        }
        @if (activeTab === 'groupConfig') {
          <div class="dc-tab-pane">
            <section class="dc-group-section">
              ...
              <column-editor
                [columns]="config.groupRowConfig || []"
                (columnsChange)="updateGroupRowConfig($event)"
              ></column-editor>
            </section>
            <section class="dc-group-section">
              <h5 class="dc-group-section-title">
              ...
              <rule-set-editor
                [ruleSets]="config.groupRowRulesConfig || []"
                (ruleSetsChange)="updateGroupRowRules($event)"
              ></rule-set-editor>
            </section>
          </div>
        }
