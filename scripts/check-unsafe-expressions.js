...
const diagnosticSelfSourcePath = 'scripts/check-unsafe-expressions.js';
...
const maximumSelfDependencyCompositionWork = 131072;
...
const maximumSelfAnalysisWork = 67108864;
...
function collectBindings(sourceFile) {
  ...
  const selfAnalysis = sourceFile.fileName === 'scripts/check-unsafe-expressions.js';
  let remainingAnalysisWork = selfAnalysis ? maximumSelfAnalysisWork : maximumAnalysisWork;
  let remainingDependencyCompositionWork = selfAnalysis
    ? maximumSelfDependencyCompositionWork
    : maximumDependencyCompositionWork;
  ...
  function createCarrier(positional = false) {
    const carrier = {
      accessors: new Map(),
      accessorDefinitionRanks: new Map(),
      definiteAccessorProperties: new Set(),
      uncertainAccessorProperties: new Set(),
      ...
      definitelyDeletedProperties: new Set(),
      ...
      unknownAccessorWriteRanks: { get: undefined, set: undefined },
      unknownAccessorWriteUnranked: { get: false, set: false },
  ...
  function seedImportDeclaration(node) {
    ...
    if (clause.namedBindings && ts.isNamespaceImport(clause.namedBindings)) {
      seedImportBinding(clause.namedBindings.name, moduleValue(moduleName, true));
    }
    ...
    for (const element of clause.namedBindings.elements) {
      ...
      } else if (importedName === 'values') {
        seedImportBinding(element.name, originValue(origins.lodashValues));
      }
    }
  }
  ...
  function specialProperty(origin, propertyName) {
    ...
    if (origin === origins.arrayObject) {
      ...
      if (matches('values')) result.add(origins.lodashValues);
    ...
    } else if (origin === origins.lodashValues) {
      if (matches('default')) result.add(origins.lodashValues);
    }
    return result;
  }
    ...
    function visit(node) {
      ...
      if (ts.isVariableDeclaration(node) && node.initializer) {
        ...
        if (ts.isVariableDeclarationList(node.initializer)) {
          ...
          assignToTarget(node.initializer, elementValue, node);
        }
        ...
        if (
          ...
          assignToTarget(node.left, evaluateExpression(node.right), node);
        } else {
  ...
  function strongPropertyWriteSupersedesPositionalUncertainty(carrier, propertyName) {
    if (
      carrier.accessors.has(propertyName) &&
    ...
    for (const atom of value) {
      if (typeof atom === 'string') {
        ...
        if (state) {
          ...
          if (propertyNames === undefined) {
            for (const [propertyName, propertyValue] of state.properties) {
              if (!state.definitelyDeletedProperties.has(propertyName)) mergeValue(result, propertyValue);
            }
            if ([...state.accessors.values()].some(accessor => accessor.get.size > 0)) {
              for (const [propertyName, accessor] of state.accessors) {
                if (state.definitelyDeletedProperties.has(propertyName)) continue;
            ...
            for (const propertyName of propertyNames) {
              ...
              const definitelyDeleted = state.definitelyDeletedProperties.has(propertyName);
              const hasOwnProperty =
                !definitelyDeleted && (propertyValue !== undefined || accessor !== undefined || specialValue.size > 0);
              if (propertyValue && !definitelyDeleted) mergeValue(result, propertyValue);
              if (accessor && !definitelyDeleted) mergeValue(result, accessorReturnedValues(accessor.get, value));
              if (!definitelyDeleted) mergeValue(result, specialValue);
              mergeValue(result, state.unknownProperty);
              mergeValue(result, accessorReturnedValues(state.unknownAccessors.get, value));
              if (
                !hasOwnProperty ||
      ...
      if (propertyNames === undefined) {
        for (const [propertyName, propertyValue] of atom.properties) {
          if (atom.definitelyDeletedProperties.has(propertyName)) continue;
          const accessorShadowsData =
            atom.accessors.has(propertyName) &&
            atom.definiteAccessorProperties.has(propertyName) &&
            !atom.uncertainAccessorProperties.has(propertyName) &&
            !atom.deletedProperties.has(propertyName);
          if (!accessorShadowsData) mergeValue(result, propertyValue);
        }
        for (const [propertyName, accessor] of atom.accessors) {
          if (atom.definitelyDeletedProperties.has(propertyName)) continue;
          mergeValue(result, accessorReturnedValues(accessor.get, value));
        }
      } else {
        for (const propertyName of propertyNames) {
          const definitelyDeleted = atom.definitelyDeletedProperties.has(propertyName);
          const propertyValue = atom.properties.get(propertyName);
          const hasDefiniteAccessor =
            atom.accessors.has(propertyName) &&
            atom.definiteAccessorProperties.has(propertyName) &&
            !atom.uncertainAccessorProperties.has(propertyName) &&
            !atom.deletedProperties.has(propertyName);
          if (propertyValue && !hasDefiniteAccessor && !definitelyDeleted) mergeValue(result, propertyValue);
          const boundedOverflowValue = boundedOverflowPropertyValue(atom, propertyName);
          if (boundedOverflowValue && !hasDefiniteAccessor && !definitelyDeleted) {
  ...
  function putCarrierProperty(
    ...
    for (const propertyName of propertyNames) {
      ...
      if (strong) prepareStrongCarrierPropertyWrite(carrier, propertyName);
  ...
  function prepareStrongCarrierPropertyWrite(carrier, propertyName) {
    if (!carrier.deletedProperties.has(propertyName)) return;
    ...
    if (definitelyDeleted) {
      ...
      if (propertyValue) {
        ...
        carrier.properties.delete(propertyName);
        cleared = true;
      }
      ...
      if (accessor) {
        ...
        carrier.accessors.delete(propertyName);
        cleared = true;
      }
      for (const state of [
        ...
        if (state?.delete(propertyName)) cleared = true;
      }
      const index = arrayIndexPropertyValue(propertyName);
      if (index !== undefined) {
        if (carrier.overflowPositionalProperties.delete(index)) cleared = true;
        if (carrier.overflowPositionalOwnProperties.delete(index)) cleared = true;
      }
      if (cleared) notifyPropagationSubscribers(carrier);
      if (carrier.ownPropertyNames?.delete(propertyName)) notifyPropagationSubscribers(carrier);
      if (carrier.possiblyReorderedOwnPropertyNames?.delete(propertyName)) {
        notifyPropagationSubscribers(carrier);
      }
    } else if (carrier.ownPropertyNames?.has(propertyName) && arrayIndexPropertyValue(propertyName) === undefined) {
      carrier.possiblyReorderedOwnPropertyNames ??= new Set();
      if (!carrier.possiblyReorderedOwnPropertyNames.has(propertyName)) {
        carrier.possiblyReorderedOwnPropertyNames.add(propertyName);
        notifyPropagationSubscribers(carrier);
      }
    }
    let changed = carrier.deletedProperties.delete(propertyName);
    if (carrier.definitelyDeletedProperties.delete(propertyName)) changed = true;
    if (changed) notifyPropagationSubscribers(carrier);
  }
  ...
  function ownPropertyNamesInRuntimeOrder(target, includeAlternativeOrders = false) {
    ...
    for (const propertyName of target.ownPropertyNames ?? []) {
      if (knownPropertyNames.delete(propertyName)) insertionOrderedNames.push(propertyName);
    }
    for (const propertyName of knownPropertyNames) insertionOrderedNames.push(propertyName);
    const indexedNames = [];
    const stringNames = [];
    const symbolNames = [];
    for (const propertyName of insertionOrderedNames) {
      const propertyIndex = arrayIndexPropertyValue(propertyName);
      if (propertyIndex === undefined) {
        (propertyName === iteratorPropertyName ? symbolNames : stringNames).push(propertyName);
        continue;
      }
      let position = 0;
      while (position < indexedNames.length && arrayIndexPropertyValue(indexedNames[position]) < propertyIndex) {
        position += 1;
      }
      indexedNames.splice(position, 0, propertyName);
    }
    const baseOrder = [...indexedNames, ...stringNames, ...symbolNames];
    if (!includeAlternativeOrders) return baseOrder;
    let insertionOrders = [baseOrder];
    for (const propertyName of target.possiblyReorderedOwnPropertyNames ?? []) {
      if (arrayIndexPropertyValue(propertyName) !== undefined || !baseOrder.includes(propertyName)) continue;
      const additions = [];
      for (const order of insertionOrders) {
        const moved = [];
        for (const name of order) {
          if (name !== propertyName) moved.push(name);
        }
        moved.push(propertyName);
        let alreadyPresent = false;
        for (const candidate of insertionOrders) {
          if (candidate.length !== moved.length) continue;
          let matches = true;
          for (let index = 0; index < moved.length; index += 1) {
            if (candidate[index] !== moved[index]) {
              matches = false;
              break;
            }
          }
        ...
        if (!alreadyPresent) additions.push(moved);
        if (insertionOrders.length + additions.length >= maximumTrackedPositionalAlternatives) break;
      }
      insertionOrders.push(...additions);
      if (insertionOrders.length >= maximumTrackedPositionalAlternatives) break;
    }
    const runtimeOrders = [];
    const seen = new Set();
    for (const insertionOrder of insertionOrders) {
      ...
      const stringNames = [];
      const symbolNames = [];
      for (const propertyName of insertionOrder) {
        const propertyIndex = arrayIndexPropertyValue(propertyName);
        if (propertyIndex === undefined) {
          (propertyName === iteratorPropertyName ? symbolNames : stringNames).push(propertyName);
          continue;
      ...
      const runtimeOrder = [...indexedNames, ...stringNames, ...symbolNames];
      const key = runtimeOrder.join('\0');
      if (seen.has(key)) continue;
      seen.add(key);
      runtimeOrders.push(runtimeOrder);
    }
    return runtimeOrders;
  }

  function unrepresentedOverflowOwnPropertyValue(target, representedPropertyNames) {
    if (
      !target.positional ||
      target.positionalOverflowStart === undefined ||
      target.overflowPositionalValues.size === 0
    ) {
      return new Set();
    }
    let representedIndexLimit = 0;
    for (const propertyName of representedPropertyNames) {
      const index = arrayIndexPropertyValue(propertyName);
      if (index !== undefined) representedIndexLimit = Math.max(representedIndexLimit, index + 1);
    }
    const exactLengths = target.exactPositionalLengths;
    const mayHaveUnrepresentedOwnProperty =
      exactLengths.size === 0 ||
      [...exactLengths].some(length => length > representedIndexLimit) ||
      target.positionalUncertain ||
      target.overflowPositionalPropertiesUncertain;
    return mayHaveUnrepresentedOwnProperty
      ? retainReflectiveCallableProvenance(target.overflowPositionalValues)
      : new Set();
  }
  ...
  function putAbstractAccessor(
    ...
    if (propertyNames === undefined) {
      ...
      if (writeRank) {
        const previousRank = target.unknownAccessorWriteRanks[kind];
        if (!previousRank || rankPrecedes(writeRank, previousRank)) {
          target.unknownAccessorWriteRanks[kind] = writeRank;
          notifyPropagationSubscribers(target);
        }
      } else if (!target.unknownAccessorWriteUnranked[kind]) {
        target.unknownAccessorWriteUnranked[kind] = true;
        notifyPropagationSubscribers(target);
      }
      return;
    }
    for (const propertyName of propertyNames) {
      ...
      if (!accessor) {
        if (!present && possibleValues.size === 0) continue;
        rememberOwnPropertyName(target, propertyName);
        accessor = { get: new Set(), set: new Set() };
        target.accessors.set(propertyName, accessor);
      }
      if (replace) {
        let equivalent = accessor[kind].size === possibleValues.size;
        const unmatchedValues = [...possibleValues];
        for (const currentValue of accessor[kind]) {
          const matchIndex = unmatchedValues.findIndex(
            possibleValue =>
              currentValue === possibleValue ||
              (typeof currentValue !== 'string' &&
                currentValue.kind === 'function-value' &&
                typeof possibleValue !== 'string' &&
                possibleValue.kind === 'function-value' &&
                currentValue.iteratorNode === possibleValue.iteratorNode)
          );
          if (matchIndex < 0) {
            equivalent = false;
            break;
        ...
        if (!equivalent) {
          replaceTracked(accessor[kind], possibleValues, target);
        }
      } else mergeCarrierProperty(target, accessor[kind], possibleValues);
    }
  }
  ...
  function recordTargetProperty(
    ...
    const strongCarrierWrite =
      strong &&
      (activeAlternativeMutationDepth === 0 || forceStrong) &&
      writeRank !== undefined &&
  ...
  function recordTargetDataDescriptor(targetValue, propertyNames, descriptorValue, writeNode, strong) {
    ...
    const writeRank = deterministicWriteRank(writeNode);
    const onlyTarget = targetValue.size === 1 ? [...targetValue][0] : undefined;
    const strongCarrierWrite =
      strong &&
      activeAlternativeMutationDepth === 0 &&
      writeRank !== undefined &&
      propertyNames.length === 1 &&
      typeof onlyTarget !== 'string' &&
      onlyTarget?.kind === 'carrier';
    for (const atom of targetValue) {
      if (typeof atom === 'string' || atom.kind !== 'carrier') continue;
      for (const propertyName of propertyNames) {
        const application =
          atom.positional && propertyName === 'length'
            ? undefined
            : descriptorAppliedDataPropertyState(atom, propertyName, descriptorValue);
        if (strongCarrierWrite) {
          let accessorStateChanged = false;
          const accessor = atom.accessors.get(propertyName);
          if (accessor) {
            replaceTracked(accessor.get, new Set(), atom);
            replaceTracked(accessor.set, new Set(), atom);
            atom.accessors.delete(propertyName);
            accessorStateChanged = true;
          }
          if (atom.definiteAccessorProperties.delete(propertyName)) accessorStateChanged = true;
          if (atom.uncertainAccessorProperties.delete(propertyName)) accessorStateChanged = true;
          if (atom.accessorDefinitionRanks.delete(propertyName)) accessorStateChanged = true;
          if (accessorStateChanged) notifyPropagationSubscribers(atom);
        } else if (atom.accessors.has(propertyName) && !atom.uncertainAccessorProperties.has(propertyName)) {
  ...
  function recordTargetGenericDescriptor(targetValue, propertyNames, descriptorValue, writeNode, strong) {
    if (propertyNames === undefined) {
      recordTargetProperty(targetValue, undefined, unknownReflectiveCallableValue(), writeNode);
      return;
    }
    ...
    for (const atom of targetValue) {
      if (typeof atom === 'string' || atom.kind !== 'carrier') continue;
      for (const propertyName of propertyNames) {
        const ownState = indexedOwnDataPropertyState(atom, propertyName);
        const hasDefiniteAccessor =
          atom.accessors.has(propertyName) &&
          atom.definiteAccessorProperties.has(propertyName) &&
          !atom.uncertainAccessorProperties.has(propertyName) &&
        ...
        const definitelyMissing =
          atom.definitelyDeletedProperties.has(propertyName) ||
          (!hasDefiniteAccessor &&
        ...
        const propertyMayBeMissing = definitelyMissing || (!hasDefiniteAccessor && ownState.mayBeMissing);
        const attributes = {};
        for (const field of ['configurable', 'enumerable']) {
          const fieldState = descriptorBooleanValues(descriptorFieldState(descriptorValue, field));
          const values = new Set(fieldState.values);
          if (fieldState.mayBeAbsent) {
            mergeValue(values, dataPropertyAttributeValues(atom, propertyName, field));
            if (propertyMayBeMissing) values.add(false);
          }
          attributes[field] = values;
        }
        if (strongCarrierWrite && definitelyMissing) {
          attributes.writable = new Set([false]);
        ...
        if (!strongCarrierWrite && propertyMayBeMissing) {
          ...
          putCarrierProperty(atom, [propertyName], literalValue(undefined), writeRank, false, {
            ...attributes,
  ...
  function recordTargetAccessor(
    ...
    const onlyTarget = targetValue.size === 1 ? [...targetValue][0] : undefined;
    const writeRank = uncertainWriteRank(writeNode);
    ...
    for (const atom of targetValue) {
      if (typeof atom !== 'string' && atom.kind === 'carrier') {
        ...
        putAbstractAccessor(
          ...
          kind,
          value,
          definiteCarrierAccessor,
          present,
          replace && definiteCarrierAccessor,
          writeRank
        );
      } else if (typeof atom === 'string') {
        const state = builtinPrototypeState(atom);
        if (state) putAbstractAccessor(state, propertyNames, kind, value, false, present, false, writeRank);
      }
    }
  }
        ...
        for (const propertyName of propertyNames) {
          ...
          if (
            ...
            if (target.kind !== 'carrier') {
              const hasOwnProperty = target.properties.has(propertyName) || target.accessors.has(propertyName);
              outcome = hasOwnProperty ? undefined : true;
            } else {
              ...
              const hasModeledOwnProperty = ownState.hasModeledDataProperty || target.accessors.has(propertyName);
              if (!hasModeledOwnProperty) outcome = true;
              else {
                const mayBeMissing =
                  target.deletedProperties.has(propertyName) ||
                  (!hasDefiniteAccessor && (!ownState.hasModeledDataProperty || ownState.mayBeMissing)) ||
                  target.uncertainAccessorProperties.has(propertyName);
                const configurabilities = dataPropertyAttributeValues(target, propertyName, 'configurable');
                let presentSucceeds = configurabilities.size > 0;
                let presentFails = configurabilities.size > 0;
                for (const configurable of configurabilities) {
                  if (configurable) presentFails = false;
                  else presentSucceeds = false;
                }
                if (presentSucceeds) outcome = true;
                else if (!mayBeMissing && presentFails) outcome = false;
              }
            }
          }
          if (outcome === false) continue;
          const definitelyDeleted =
            outcome === true &&
            activeAlternativeMutationDepth === 0 &&
            deterministicWriteRank(deletionNode) !== undefined;
          if (!target.deletedProperties.has(propertyName)) {
            target.deletedProperties.add(propertyName);
            changed = true;
          }
          if (definitelyDeleted && !target.definitelyDeletedProperties.has(propertyName)) {
  ...
  function descriptorFieldValues(descriptorValue, field, observation) {
    const result = new Set();
    const invokesGetter = accessorMayRun(descriptorValue, [field], 'get');
    if (observation) observation.invokesGetter = invokesGetter;
    if (invokesGetter) invalidatePositionalTargets(descriptorValue, undefined, unknownValue());
    for (const atom of descriptorValue) {
      ...
      const definitelyDeleted = atom.definitelyDeletedProperties.has(field);
      const hasOwnField = !definitelyDeleted && (propertyValue !== undefined || accessor !== undefined);
      if (propertyValue && !definitelyDeleted) mergeValue(result, propertyValue);
      if (accessor && !definitelyDeleted) mergeValue(result, accessorReturnedValues(accessor.get, descriptorValue));
  ...
  function descriptorFieldState(descriptorValue, field) {
    const observation = { invokesGetter: false };
    const values = descriptorFieldValues(descriptorValue, field, observation);
    ...
    for (const atom of descriptorValue) {
      ...
      const hasOwnField =
        !atom.definitelyDeletedProperties.has(field) && (atom.properties.has(field) || atom.accessors.has(field));
      if (
        !hasOwnField ||
    ...
    return { invokesGetter: observation.invokesGetter, mayBeAbsent, values };
  }

  function descriptorKindState(descriptorValue) {
    const getState = descriptorFieldState(descriptorValue, 'get');
    const setState = descriptorFieldState(descriptorValue, 'set');
    const valueState = descriptorFieldState(descriptorValue, 'value');
    const writableState = descriptorFieldState(descriptorValue, 'writable');
    const accessorPossible = getState.values.size > 0 || setState.values.size > 0;
    const dataPossible = valueState.values.size > 0 || writableState.values.size > 0;
    const genericPossible =
      getState.mayBeAbsent && setState.mayBeAbsent && valueState.mayBeAbsent && writableState.mayBeAbsent;
    const possibleKindCount = Number(accessorPossible) + Number(dataPossible) + Number(genericPossible);
    return {
      accessorDefinite: accessorPossible && possibleKindCount === 1,
      accessorPossible,
      dataDefinite: dataPossible && possibleKindCount === 1,
      dataPossible,
      genericDefinite: genericPossible && possibleKindCount === 1,
      genericPossible,
    };
  }
  ...
  function indexedOwnDataPropertyState(carrier, propertyName) {
    ...
    const hasModeledDataProperty =
      !carrier.definitelyDeletedProperties.has(propertyName) &&
      !accessorShadowsData &&
  ...
  function inheritedIndexedSetAllowsCreation(
    ...
    propertyName,
    seen = new Set(),
    outcome = { requiresReceiverCreation: false, setterConsumesWrite: false }
  ) {
    ...
    for (const atom of prototypeValue) {
      ...
      if (accessor && !propertyDeleted) {
        ...
        outcome.setterConsumesWrite = true;
        continue;
      }
  ...
  function inheritedSetFailureIsProven(prototypeValue, propertyName, outcome) {
    if (prototypeValue.size === 0) {
      outcome.requiresReceiverCreation = true;
      return false;
    }
    ...
    for (let index = 0; index < pending.length; index += 1) {
      ...
      const propertyValue = target.properties.get(propertyName);
      const boundedOverflowValue = boundedOverflowPropertyValue(target, propertyName);
      if ((propertyValue && propertyValue.size > 0) || boundedOverflowValue) {
        if (typeof atom === 'string') return false;
        const writabilities = dataPropertyAttributeValues(target, propertyName, 'writable');
        if (writabilities.size === 0) return false;
        for (const writable of writabilities) {
          if (writable) {
            outcome.requiresReceiverCreation = true;
            return false;
          }
      ...
      const prototypes = effectivePrototypes(atom);
      if (prototypes.size === 0) {
        outcome.requiresReceiverCreation = true;
        return false;
      }
      for (const prototype of prototypes) pending.push(prototype);
    }
    return true;
  }
  ...
  function descriptorAppliedDataPropertyState(carrier, propertyName, descriptorValue) {
    const ownState = indexedOwnDataPropertyState(carrier, propertyName);
    const definitelyMissing = carrier.definitelyDeletedProperties.has(propertyName);
    const hasOwnDataProperty = ownState.hasModeledDataProperty && !definitelyMissing;
    const hasOwnAccessor =
      !definitelyMissing &&
      carrier.accessors.has(propertyName) &&
      carrier.definiteAccessorProperties.has(propertyName) &&
      !carrier.uncertainAccessorProperties.has(propertyName) &&
    ...
    if (valueField.mayBeAbsent) {
      if (hasOwnDataProperty) mergeValue(value, ownDataPropertyValues(carrier, propertyName));
      if (ownState.mayBeMissing || definitelyMissing || hasOwnAccessor) mergeValue(value, literalValue(undefined));
    }
    const attributes = {};
    for (const field of ['configurable', 'enumerable', 'writable']) {
      const fieldState = descriptorBooleanValues(descriptorFieldState(descriptorValue, field));
      const values = new Set(fieldState.values);
      if (fieldState.mayBeAbsent) {
        if (hasOwnDataProperty || hasOwnAccessor) {
          mergeValue(values, dataPropertyAttributeValues(carrier, propertyName, field));
        }
        if ((ownState.mayBeMissing || definitelyMissing) && !hasOwnAccessor) values.add(false);
      }
      attributes[field] = values;
    }
  ...
  function successfulIndexedSetAttributes(carrier, propertyName) {
    if (carrier.definitelyDeletedProperties.has(propertyName)) {
      return {
        configurable: new Set([true]),
        enumerable: new Set([true]),
  ...
  function reflectivePropertyWriteSuccessIsProven(
    ...
    if (
      ...
      if (target.uncertainAccessorProperties.has(propertyName)) return false;
      const setterConsumesWrite = target.accessors.get(propertyName).set.size > 0;
      if (setterConsumesWrite && writeEffect) writeEffect.setterConsumesWrite = true;
    ...
    if (defineProperty && target.accessors.has(propertyName) && !target.definitelyDeletedProperties.has(propertyName)) {
      ...
      const currentConfigurabilities = dataPropertyAttributeValues(target, propertyName, 'configurable');
      const currentEnumerabilities = dataPropertyAttributeValues(target, propertyName, 'enumerable');
      if (currentConfigurabilities.size === 0 || currentEnumerabilities.size === 0) return false;
      if ([...currentConfigurabilities].every(Boolean)) return true;
      if ([...currentConfigurabilities].some(Boolean)) return false;
      if (['value', 'writable'].some(field => descriptorFieldState(descriptorValue, field).values.size > 0)) {
        return false;
      }
      const requestedConfigurabilities = descriptorBooleanValues(
        descriptorFieldState(descriptorValue, 'configurable')
      ...
      for (const field of ['get', 'set']) {
        ...
        const matches =
          typeof requested !== 'string' && requested.kind === 'literal' && requested.value === undefined
            ? accessor[field].size === 0
            : abstractSameValueIsProven(accessor[field], requestedState.values);
    ...
    if (!ownState.mayBeMissing) {
      if (writeEffect) writeEffect.receiverOwnProperty = true;
      return true;
    }
    const inheritedOutcome = { requiresReceiverCreation: false, setterConsumesWrite: false };
    if (!inheritedIndexedSetAllowsCreation(effectivePrototypes(target), propertyName, new Set(), inheritedOutcome)) {
      return false;
    }
    const succeeds =
      !inheritedOutcome.requiresReceiverCreation || missingIndexedPropertyCreationIsProven(receiver, propertyName);
    if (succeeds && writeEffect) {
      if (ownState.hasModeledDataProperty || inheritedOutcome.requiresReceiverCreation) {
        writeEffect.receiverOwnProperty = true;
      }
      if (inheritedOutcome.setterConsumesWrite) writeEffect.setterConsumesWrite = true;
    }
    return succeeds;
  }
  ...
  function reflectivePropertyWriteFailureIsProven(
    ...
    if (
      ...
      const currentConfigurabilities = dataPropertyAttributeValues(target, propertyName, 'configurable');
      if (currentConfigurabilities.size === 0 || ![...currentConfigurabilities].every(value => !value)) {
        return false;
      }
      if (
        ['value', 'writable'].some(field => {
          const fieldState = descriptorFieldState(descriptorValue, field);
          return !fieldState.mayBeAbsent && fieldState.values.size > 0;
        })
      ) {
      ...
      const accessor = target.accessors.get(propertyName);
      if (!accessor) return false;
      return ['get', 'set'].some(field => {
        const requestedState = descriptorFieldState(descriptorValue, field);
        if (requestedState.mayBeAbsent) return false;
        if (requestedState.values.size !== 1) return true;
        const requested = [...requestedState.values][0];
        const matches =
          typeof requested !== 'string' && requested.kind === 'literal' && requested.value === undefined
            ? accessor[field].size === 0
    ...
    const inheritedOutcome = { requiresReceiverCreation: defineProperty };
    const inheritedFailure =
      !defineProperty && inheritedSetFailureIsProven(effectivePrototypes(target), propertyName, inheritedOutcome);
    const missingFailure =
      ((defineProperty || inheritedOutcome.requiresReceiverCreation) && target.extensible === false) ||
      inheritedFailure ||
  ...
  function recordDescriptorAccessors(targetValue, propertyNames, descriptorValue, writeNode, strong = false) {
    const accessorStates = new Map(['get', 'set'].map(kind => [kind, descriptorFieldState(descriptorValue, kind)]));
    if ([...accessorStates.values()].every(state => state.mayBeAbsent && state.values.size === 0)) return;
    let accessorMayBePresent = false;
    for (const state of accessorStates.values()) {
      if (state.values.size > 0) {
        accessorMayBePresent = true;
        break;
      }
    }
    const accessorDefinitelyPresent = [...accessorStates.values()].some(state => !state.mayBeAbsent);
    const writeRank = deterministicWriteRank(writeNode);
    if (writeRank && propertyNames?.length === 1 && targetValue.size === 1) {
      const target = [...targetValue][0];
      if (typeof target !== 'string' && target.kind === 'carrier') {
        const latestPropertyWriteRank = target.propertyWriteRanks.get(propertyNames[0]);
        if (latestPropertyWriteRank && rankPrecedes(writeRank, latestPropertyWriteRank)) return;
      }
    }
    const strongCarrierWrite =
      strong &&
      accessorDefinitelyPresent &&
      activeAlternativeMutationDepth === 0 &&
      writeRank !== undefined &&
      propertyNames?.length === 1 &&
      targetValue.size === 1 &&
      [...targetValue].every(atom => typeof atom !== 'string' && atom.kind === 'carrier');
    let replaceAccessorFields = false;
    if (strongCarrierWrite) {
      for (const atom of targetValue) {
        if (typeof atom === 'string' || atom.kind !== 'carrier') continue;
        for (const propertyName of propertyNames) {
          ...
          replaceAccessorFields =
            !sameDefinition && (atom.accessors.has(propertyName) || atom.properties.get(propertyName)?.size > 0);
          const ownDataState = indexedOwnDataPropertyState(atom, propertyName);
          const hasOwnAccessor =
            atom.accessors.has(propertyName) &&
            atom.definiteAccessorProperties.has(propertyName) &&
            !atom.uncertainAccessorProperties.has(propertyName) &&
            !atom.deletedProperties.has(propertyName);
          const hasOwnProperty = ownDataState.hasModeledDataProperty || hasOwnAccessor;
          const attributes = {};
          for (const field of ['configurable', 'enumerable']) {
            const fieldState = descriptorBooleanValues(descriptorFieldState(descriptorValue, field));
            const values = new Set(fieldState.values);
            if (fieldState.mayBeAbsent) {
              if (hasOwnProperty) mergeValue(values, dataPropertyAttributeValues(atom, propertyName, field));
    ...
    } else if (accessorMayBePresent && propertyNames !== undefined) {
      for (const atom of targetValue) {
        if (typeof atom === 'string' || atom.kind !== 'carrier') continue;
        for (const propertyName of propertyNames) {
          if (!atom.uncertainAccessorProperties.has(propertyName)) {
            atom.uncertainAccessorProperties.add(propertyName);
            notifyPropagationSubscribers(atom);
          }
          const previousWriteRank = atom.propertyWriteRanks.get(propertyName);
          if (writeRank && (!previousWriteRank || rankPrecedes(previousWriteRank, writeRank))) {
            atom.propertyWriteRanks.set(propertyName, writeRank);
          }
        }
      }
    ...
    recordTargetAccessor(
      ...
      propertyNames,
      'get',
      possibleAccessorValues(accessorStates.get('get').values),
      writeNode,
      strong,
      accessorDefinitelyPresent,
      replaceAccessorFields && !accessorStates.get('get').mayBeAbsent
    );
    recordTargetAccessor(
      targetValue,
      propertyNames,
      'set',
      possibleAccessorValues(accessorStates.get('set').values),
      writeNode,
      strong,
      accessorDefinitelyPresent,
      replaceAccessorFields && !accessorStates.get('set').mayBeAbsent
    );
  }

  function recordPropertyDescriptor(
    targetValue,
    propertyNames,
    descriptorValue,
    writeNode,
    ...
    const previousAlternativeDepth = activeAlternativeMutationDepth;
    if (forceStrong) activeAlternativeMutationDepth = 0;
    try {
      const descriptorKinds = kindState ?? descriptorKindState(descriptorValue);
      if (descriptorKinds.accessorPossible) {
        recordDescriptorAccessors(
          targetValue,
          propertyNames,
          descriptorValue,
          writeNode,
          success === true && descriptorKinds.accessorDefinite
        );
      }
      if (descriptorKinds.dataPossible) {
        recordTargetDataDescriptor(
          targetValue,
          propertyNames,
          descriptorValue,
          writeNode,
          success === true && descriptorKinds.dataDefinite
        );
      }
      if (descriptorKinds.genericPossible) {
        recordTargetGenericDescriptor(
          targetValue,
          propertyNames,
          descriptorValue,
          writeNode,
          success === true && descriptorKinds.genericDefinite
        );
      }
    } finally {
      activeAlternativeMutationDepth = previousAlternativeDepth;
    }
  }
    ...
    for (const propertyName of names) {
      ...
      for (let queueIndex = 0; queueIndex < pending.length; queueIndex += 1) {
        ...
        if (propertyName === undefined) {
          ...
          if (propertyValue && !definitelyDeleted) mergeValue(result, propertyValue);
          if (boundedOverflowValue && !definitelyDeleted) mergeValue(result, boundedOverflowValue);
          if (accessor && !definitelyDeleted) mergeValue(result, accessorReturnedValues(accessor.get, receiverValue));
    ...
    for (const propertyName of names) {
      ...
      for (let queueIndex = 0; queueIndex < pending.length; queueIndex += 1) {
        ...
        if (propertyName === undefined) {
          ...
          const accessor = target.accessors.get(propertyName);
          const definitelyDeleted = target.definitelyDeletedProperties.has(propertyName);
          if (accessor?.[kind].size > 0 && !definitelyDeleted) return true;
          ...
          if (
            accessor &&
            (accessor.get.size > 0 || accessor.set.size > 0) &&
            !propertyMayBeDeleted &&
            !definitelyDeleted
          )
            continue;
          if (target.properties.has(propertyName) && !propertyMayBeDeleted && !definitelyDeleted) continue;
  ...
  function possibleAccessorSetters(targetValue, propertyNames, invocationNode) {
    const callables = new Set();
    let unresolvedModeled = false;
    function mergeModeledSetter(value) {
      if ([...value].some(atom => typeof atom !== 'string' && atom.kind === 'unknown-value')) {
        unresolvedModeled = true;
      }
      mergeValue(callables, retainReflectiveCallableProvenance(value));
    }
    const names = propertyNames ?? [undefined];
    for (const propertyName of names) {
      const pending = [...targetValue];
      ...
      for (let queueIndex = 0; queueIndex < pending.length; queueIndex += 1) {
        ...
        if (!consumeAnalysisWork()) {
          callables.add(unknownReflectiveCallableAtom);
          break;
        }
        if (typeof atom !== 'string' && atom.kind === 'unknown-value') {
          callables.add(atom.callableReason ? atom : unknownReflectiveCallableAtom);
          continue;
        }
        const target = abstractTarget(atom);
        if (!target) continue;
        trackPropagationDependency(target);
        if (propertyName === undefined) {
          for (const accessor of target.accessors.values()) mergeModeledSetter(accessor.set);
          const invocationRank = invocationNode ? uncertainWriteRank(invocationNode) : undefined;
          const unknownAccessorWriteRank = target.unknownAccessorWriteRanks.set;
          const unknownAccessorCanPrecede =
            target.unknownAccessorWriteUnranked.set ||
            !invocationRank ||
            !unknownAccessorWriteRank ||
            !rankPrecedes(invocationRank, unknownAccessorWriteRank);
          if (target.unknownAccessors.set.size > 0 && unknownAccessorCanPrecede) {
            mergeModeledSetter(target.unknownAccessors.set);
          }
          for (const prototype of effectivePrototypes(atom)) pending.push(prototype);
          continue;
        }
        ...
        const accessor = target.accessors.get(propertyName);
        if (accessor && !definitelyDeleted) {
          mergeModeledSetter(accessor.set);
        }
        ...
        if (target.unknownProperty.size > 0) callables.add(unknownReflectiveCallableAtom);
        for (const prototype of effectivePrototypes(atom)) pending.push(prototype);
      }
    }
    return { callables, unresolvedModeled };
  }
  ...
  function applyAccessorSetterEffects(targetValue, propertyNames, receiverValue, assignedValue, invocationNode) {
    ...
    if (![...targetValue].some(atom => abstractTarget(atom))) return;
    const { callables: setters, unresolvedModeled } = possibleAccessorSetters(
      targetValue,
      propertyNames,
      invocationNode
    );
    if (setters.size === 0) return;
    if (activeDormantPropagationDepth > 0 && !activeFunctionInvocationState()) return;
    const invocation = { kinds: new Set(), result: new Set() };
    ...
    try {
      for (const setter of setters) {
        mergeInvocation(
          invocation,
          invokeTracked(new Set([setter]), [assignedValue], invocationNode, new Set(), receiverValue)
        );
      }
    } finally {
      if (alternativeSetter) activeAlternativeMutationDepth -= 1;
    }
    const hasUnsafeKind = invocation.kinds.has('direct-eval') || invocation.kinds.has('lodash-template');
    const unresolvedModeledSetter = !hasUnsafeKind && unresolvedModeled && !callableMayInstallTrackedEffects(setters);
    recordInvocationEvidence(invocationNode, invocation.kinds, unresolvedModeledSetter);
  }
  ...
  function commitDeterministicPositionalLength(carrier, length, writeRank) {
    ...
    for (const [propertyName, propertyValue] of carrier.properties) {
      ...
      if (!carrier.definitelyDeletedProperties.has(propertyName)) {
        carrier.definitelyDeletedProperties.add(propertyName);
        carrier.ownPropertyNames.delete(propertyName);
        carrier.possiblyReorderedOwnPropertyNames.delete(propertyName);
        changed = true;
      }
    }
  ...
  function invalidatePositionalTargets(
    ...
    for (const atom of targetValue) {
      if (typeof atom !== 'string' && atom.kind === 'carrier') {
        ...
        if (atom.positional && propertyNames?.length === 1 && propertyNames[0] === 'length') {
          ...
          if (lengths) {
            if (
              ...
              lengths.size === 1 &&
              targetValue.size === 1 &&
              (activeAlternativeMutationDepth === 0 || forceDeterministic)
            ) {
              const length = [...lengths][0];
              if (deterministicPositionalLengthAlreadyApplied(atom, length)) continue;
              const lengthWriteRank =
                ...
                (forceDeterministic
                  ...
                  : undefined);
              applyDeterministicPositionalLength(atom, length, lengthWriteRank);
              continue;
            }
  ...
  function applyObjectAssign(targetValue, sourceValues, writeNode) {
    ...
    for (const sourceValue of sourceValues) {
      ...
      for (const source of sourceValue) {
        ...
        const propertyOrders = ownPropertyNamesInRuntimeOrder(source, true);
        const alternativeOrder = propertyOrders.length !== 1;
        let sourceAborted = propertyOrders.length > 0;
        let sourceMayAbort = alternativeOrder;
        for (const propertyOrder of propertyOrders) {
          if (alternativeOrder) activeAlternativeMutationDepth += 1;
          let orderAborted = false;
          let orderMayAbort =
          ...
          for (const propertyName of propertyOrder) {
            ...
            if (propertyFailure) {
              ...
              if (conditionallyEnumerable) {
                orderMayAbort = true;
                activeAlternativeMutationDepth -= 1;
                continue;
              }
              orderAborted = true;
              break;
            ...
            const setterDefinitelyConsumesWrite =
              propertySuccess === true && writeEffect.setterConsumesWrite && !writeEffect.receiverOwnProperty;
            const propertyMayAbort = propertySuccess !== true || invokesSourceGetter || invokesTargetSetter;
            const conditionalProperty = orderMayAbort || propertyMayAbort;
            const forceStrong =
              !conditionalSource &&
              !alternativeOrder &&
              !orderMayAbort &&
              !conditionallyEnumerable &&
            ...
            if (
              (conditionalSource || orderMayAbort || propertyMayAbort || conditionallyEnumerable) &&
              propertyOwnState?.mayBeMissing
            ) {
              recordTargetDeletion(targetValue, propertyNames);
            }
            if (conditionalProperty) activeAlternativeMutationDepth -= 1;
            if (conditionallyEnumerable) activeAlternativeMutationDepth -= 1;
            if (propertyMayAbort) orderMayAbort = true;
          }
          if (alternativeOrder) activeAlternativeMutationDepth -= 1;
          if (!orderAborted) sourceAborted = false;
          if (orderAborted || orderMayAbort) sourceMayAbort = true;
        }
  ...
  function definePropertiesOnTarget(targetValue, descriptorsValue, writeNode) {
    ...
    for (const descriptors of descriptorAlternatives) {
      ...
      if (typeof descriptors === 'string' || descriptors.kind === 'unknown-value') {
        ...
        recordTargetProperty(targetValue, undefined, unknownReflectiveCallableValue());
        recordTargetAccessor(targetValue, undefined, 'get', unknownReflectiveCallableValue(), writeNode);
        recordTargetAccessor(targetValue, undefined, 'set', unknownReflectiveCallableValue(), writeNode);
        if (alternativeDescriptors) activeAlternativeMutationDepth -= 1;
        continue;
      }
      ...
      const propertyOrders = ownPropertyNamesInRuntimeOrder(descriptors, true);
      const alternativeOrder = propertyOrders.length > 1;
      for (const propertyOrder of propertyOrders) {
        if (alternativeOrder) activeAlternativeMutationDepth += 1;
        const descriptorsInOrder = [];
        let descriptorDefinitelyAborts = false;
        ...
        for (const propertyName of propertyOrder) {
          ...
          descriptorPreflight: {
            if (descriptorValue.size === 0) {
              conversion = 'abort';
              break descriptorPreflight;
            }
            if (descriptorValue.size !== 1) {
              conversion = 'maybe';
              break descriptorPreflight;
            }
            for (const descriptor of descriptorValue) {
              if (typeof descriptor !== 'string' && descriptor.kind === 'literal') {
                conversion = 'abort';
                break descriptorPreflight;
              }
              if (typeof descriptor === 'string' || descriptor.kind !== 'carrier') {
                conversion = 'maybe';
                break descriptorPreflight;
              }
              if (
                descriptor.unknownSpreadSource ||
                descriptor.unknownProperty.size > 0 ||
                descriptor.unknownPropertyDeletion ||
                descriptor.unknownAccessors.get.size > 0
              ) {
                conversion = 'maybe';
                break descriptorPreflight;
              }
            ...
            const descriptorStates = new Map(
              descriptorFields.map(field => [field, descriptorFieldState(descriptorValue, field)])
            );
            let conversionMayAbort = [...descriptorStates.values()].some(state => state.invokesGetter);
            for (const field of ['get', 'set']) {
              const fieldState = descriptorStates.get(field);
              let valid = fieldState.mayBeAbsent;
              let invalid = false;
              let uncertain = false;
              for (const fieldValue of fieldState.values) {
                let validity;
                if (typeof fieldValue === 'string') {
                  validity =
                    fieldValue !== origins.arrayPrototype &&
                    fieldValue !== origins.globalObject &&
                    fieldValue !== origins.jsonObject &&
                    fieldValue !== origins.lodashTemplateNamespace &&
                    fieldValue !== origins.mapPrototype &&
                    fieldValue !== origins.objectPrototype &&
                    fieldValue !== origins.reflectObject &&
                    fieldValue !== origins.setPrototype;
                } else if (fieldValue.kind === 'literal') validity = fieldValue.value === undefined;
                else if (
                  fieldValue.kind === 'bound-callable' ||
                  fieldValue.kind === 'function-value' ||
                  fieldValue.kind === 'invocation-method' ||
                  fieldValue.kind === 'iterator-next' ||
                  fieldValue.kind === 'positional-mutator'
                ) {
                  validity = true;
                } else if (fieldValue.kind === 'unknown-value' || fieldValue.kind === 'known-data')
                  validity = undefined;
                else validity = false;
                if (validity === true) valid = true;
                else if (validity === false) invalid = true;
                else uncertain = true;
              }
              if (!valid && invalid && !uncertain) {
                conversion = 'abort';
                break descriptorPreflight;
              }
              if (invalid || uncertain) conversionMayAbort = true;
            }
            const accessorStates = ['get', 'set'].map(field => descriptorStates.get(field));
            const dataStates = ['value', 'writable'].map(field => descriptorStates.get(field));
            const hasAccessorField = accessorStates.some(state => state.values.size > 0);
            const hasDataField = dataStates.some(state => state.values.size > 0);
            if (hasAccessorField && hasDataField) {
              const accessorIsDefinite = accessorStates.some(state => !state.mayBeAbsent && state.values.size > 0);
              const dataIsDefinite = dataStates.some(state => !state.mayBeAbsent && state.values.size > 0);
              if (accessorIsDefinite && dataIsDefinite) {
                conversion = 'abort';
                break descriptorPreflight;
              }
              conversionMayAbort = true;
            }
            conversion = conversionMayAbort ? 'maybe' : 'valid';
          }
          descriptorsInOrder.push({
            conditionallyEnumerable,
            conversion,
            descriptorValue,
            propertyName,
          });
          if (conversion === 'abort' && !conditionallyEnumerable) descriptorDefinitelyAborts = true;
          if (conditionallyEnumerable || invokesDescriptorMapGetter || conversion !== 'valid')
            descriptorMayAbort = true;
        }
        if (descriptorDefinitelyAborts) {
          if (alternativeOrder) activeAlternativeMutationDepth -= 1;
          continue;
        }
        let descriptorAborted = false;
        for (const descriptorEntry of descriptorsInOrder) {
          const { conditionallyEnumerable, descriptorValue, propertyName } = descriptorEntry;
          if (conditionallyEnumerable) activeAlternativeMutationDepth += 1;
          const propertyNames = [propertyName];
          ...
          const descriptorKinds = descriptorKindState(descriptorValue);
          const accessorDescriptor = descriptorKinds.accessorPossible;
          const accessorDescriptorIsDefinite = descriptorKinds.accessorDefinite;
          let propertyFailure = reflectivePropertyWriteFailureIsProven(
            targetValue,
            propertyNames,
            targetValue,
          ...
          const forceStrong =
            ...
            propertySuccess === true;
          if (conditionalProperty) activeAlternativeMutationDepth += 1;
          if (descriptorKinds.dataPossible && dataValue.size > 0) {
            invalidatePositionalTargets(targetValue, propertyNames, dataValue, false, true, forceStrong);
          }
          recordPropertyDescriptor(
            targetValue,
            propertyNames,
          ...
          if (accessorDescriptor) {
            invalidatePositionalTargets(targetValue, propertyNames, assignedValue, true, true, forceStrong);
          }
  ...
  function applyPositionalMutationIntrinsic(atom, argumentValues, thisValue, invocationNode, intrinsicResult) {
    ...
    if (atom === origins.reflectDeleteProperty) {
      ...
      recordTargetDeletion(argumentValues[0] ?? new Set(), propertyNames, invocationNode);
      return true;
    }
    if (
      atom === origins.objectDefineProperty ||
      ...
      const definesProperty = atom === origins.objectDefineProperty || atom === origins.reflectDefineProperty;
      const descriptorValue = argumentValues[2] ?? new Set();
      let descriptorConversion = 'valid';
      singularDescriptorPreflight: if (definesProperty) {
        const syntaxNode = ts.isCallExpression(invocationNode) ? invocationNode.arguments[2] : undefined;
        if (syntaxNode) {
          const current = unwrapExpression(syntaxNode);
          if (ts.isObjectLiteralExpression(current)) {
            let hasAccessorField = false;
            let hasDataField = false;
            let syntaxMayAbort = false;
            for (const property of current.properties) {
              if (ts.isSpreadAssignment(property) || ts.isGetAccessorDeclaration(property)) {
                syntaxMayAbort = true;
                continue;
              }
              const descriptorPropertyNames = declaredPropertyNames(property.name);
              if (descriptorPropertyNames?.length !== 1) {
                syntaxMayAbort = true;
                continue;
              }
              const descriptorPropertyName = descriptorPropertyNames[0];
              if (descriptorPropertyName === 'get' || descriptorPropertyName === 'set') {
                ...
                if (
                  ...
                  descriptorConversion = 'abort';
                  break singularDescriptorPreflight;
                }
                if (
              ...
              } else if (descriptorPropertyName === 'value' || descriptorPropertyName === 'writable') {
                hasDataField = true;
              }
            }
            descriptorConversion = hasAccessorField && hasDataField ? 'abort' : syntaxMayAbort ? 'maybe' : 'valid';
            break singularDescriptorPreflight;
          }
        }
        if (descriptorValue.size === 0) {
        ...
        for (const field of ['get', 'set']) {
          ...
          for (const fieldValue of fieldState.values) {
            ...
            if (typeof fieldValue === 'string') {
              ...
              validity = true;
            } else if (fieldValue.kind === 'unknown-value' || fieldValue.kind === 'known-data') validity = undefined;
            else validity = false;
            if (validity === true) valid = true;
            else if (validity === false) invalid = true;
            else uncertain = true;
        ...
        descriptorConversion = conversionMayAbort ? 'maybe' : 'valid';
      }
      if (descriptorConversion === 'abort') return true;
      if (descriptorConversion === 'maybe') activeAlternativeMutationDepth += 1;
      const assignedValue = definesProperty
        ? new Set(['get', 'set', 'value'].flatMap(field => [...descriptorFieldValues(descriptorValue, field)]))
        : (argumentValues[2] ?? new Set());
      const descriptorKinds = definesProperty ? descriptorKindState(descriptorValue) : undefined;
      const accessorDescriptor = descriptorKinds?.accessorPossible === true;
      const writeEffect = { receiverOwnProperty: false, setterConsumesWrite: false };
      let reflectiveIndexedSuccess =
        definesProperty || atom === origins.reflectSet
          ...
          : undefined;
      if (definesProperty && reflectiveIndexedSuccess === undefined) {
      ...
      const setterDefinitelyConsumesWrite =
        !definesProperty &&
        reflectiveIndexedSuccess === true &&
        writeEffect.setterConsumesWrite &&
        !writeEffect.receiverOwnProperty;
      const setterMayConsumeInsteadOfWritingReceiver =
        !definesProperty &&
        reflectiveIndexedSuccess === true &&
        writeEffect.setterConsumesWrite &&
        writeEffect.receiverOwnProperty;
      const strongDataWrite =
        (!definesProperty || descriptorKinds.dataDefinite) &&
        (atom === origins.objectDefineProperty ||
          (reflectiveIndexedSuccess === true &&
            !setterDefinitelyConsumesWrite &&
            !setterMayConsumeInsteadOfWritingReceiver));
      const forceDescriptorStrong =
        definesProperty &&
        descriptorConversion === 'valid' &&
        reflectiveIndexedSuccess === true &&
        deterministicWriteRank(invocationNode) !== undefined &&
        propertyNames?.length === 1 &&
        targetValue.size === 1;
      if (
      ...
      if (!descriptorOnlyArrayLengthUpdate && !accessorDescriptor) {
        invalidatePositionalTargets(
          targetValue,
          propertyNames,
          assignedValue,
      ...
      if (definesProperty) {
        ...
        if (accessorDescriptor) {
          invalidatePositionalTargets(targetValue, propertyNames, assignedValue, true, true, forceDescriptorStrong);
        }
      } else {
        ...
        if (receiverValue?.size === 1 && propertyNames?.length === 1) {
          const receiver = [...receiverValue][0];
          if (typeof receiver !== 'string' && receiver.kind === 'carrier') {
            receiverOwnState = indexedOwnDataPropertyState(receiver, propertyNames[0]);
          }
        }
        const preserveMissingReceiver =
          !setterDefinitelyConsumesWrite && invokesSetter && receiverOwnState?.mayBeMissing === true;
        if (setterMayConsumeInsteadOfWritingReceiver || preserveMissingReceiver) {
          activeAlternativeMutationDepth += 1;
        }
        invalidatePositionalTargets(
          receiverValue ?? new Set(),
        ...
        if ((propertyNames === undefined || propertyNames.includes('__proto__')) && invokesSetter) {
          ...
          if (setterMayConsumeInsteadOfWritingReceiver || preserveMissingReceiver) {
            recordTargetDeletion(receiverValue ?? new Set(), propertyNames);
          }
        }
        if (setterMayConsumeInsteadOfWritingReceiver || preserveMissingReceiver) {
          activeAlternativeMutationDepth -= 1;
    ...
    if (atom === origins.objectPrototypeDefineGetter || atom === origins.objectPrototypeDefineSetter) {
      ...
      recordTargetAccessor(thisValue ?? new Set(), propertyNames, kind, accessorValue, invocationNode);
  ...
  function recordDeletion(node) {
    ...
    const targetValue = evaluateExpression(current.expression);
    invalidatePositionalTargets(targetValue, propertyNames, unknownValue());
    recordTargetDeletion(targetValue, propertyNames, current);
  }
  ...
  function objectProjectionValue(argumentValues, invocationNode, entries) {
    ...
    for (const atom of targetValue) {
      ...
      if (atom.kind !== 'carrier') continue;
      trackPropagationDependency(atom);
      const propertyOrders = ownPropertyNamesInRuntimeOrder(atom, true);
      if (propertyOrders.length !== 1 || targetValue.size !== 1) unknown = true;
      for (const [orderIndex, propertyOrder] of propertyOrders.entries()) {
        let index = 0;
        const unrepresentedOverflowValues = unrepresentedOverflowOwnPropertyValue(atom, propertyOrder);
        let addedUnrepresentedOverflow = false;
        for (const propertyName of propertyOrder) {
          ...
          if (
            ...
            !addedUnrepresentedOverflow &&
            unrepresentedOverflowValues.size > 0
          ) {
            let projectedOverflowValues = unrepresentedOverflowValues;
            if (entries) {
              const entry = syntheticCarrierFor(
                invocationNode,
                `object-entry-${targetIndex}-${orderIndex}-overflow`,
                true
              );
              putCarrierProperty(entry, ['0'], unknownValue());
              putCarrierProperty(entry, ['1'], unrepresentedOverflowValues);
              mergeCarrierPositionalState(entry, new Set([2]), false, new Set());
              projectedOverflowValues = new Set([entry]);
            }
            hasOverflow = true;
            overflowStart = overflowStart === undefined ? index : Math.min(overflowStart, index);
            mergeValue(overflowValues, projectedOverflowValues);
            addedUnrepresentedOverflow = true;
          }
          const observation = enumerableOwnPropertyObservation(atom, propertyName);
          if (
            observation.enumerabilities.size > 0 &&
          ...
          if (observation.enumerabilities.size === 0 || observation.enumerabilities.has(false)) unknown = true;
          const propertyValue = observation.value;
          let projectedValue = propertyValue;
          if (entries) {
            const entry = syntheticCarrierFor(
              invocationNode,
              `object-entry-${targetIndex}-${orderIndex}-${propertyName}`,
              true
            );
            putCarrierProperty(entry, ['0'], literalValue(propertyName));
            putCarrierProperty(entry, ['1'], propertyValue);
            mergeCarrierPositionalState(entry, new Set([2]), false, new Set());
            projectedValue = new Set([entry]);
          }
          if (addedUnrepresentedOverflow) mergeValue(overflowValues, projectedValue);
          if (index < maximumTrackedInvocationArguments) {
            putCarrierProperty(array, [String(index)], projectedValue);
          } else {
            hasOverflow = true;
            overflowStart = overflowStart === undefined ? index : Math.min(overflowStart, index);
            mergeValue(overflowValues, projectedValue);
          }
          index += 1;
        }
        if (!addedUnrepresentedOverflow && unrepresentedOverflowValues.size > 0) {
          let projectedOverflowValues = unrepresentedOverflowValues;
          if (entries) {
            const entry = syntheticCarrierFor(
              invocationNode,
        ...
        if (addedUnrepresentedOverflow) unknown = true;
        lengths.add(index);
      }
    ...
    if (hasOverflow) {
      mergeCarrierPositionalOverflow(array, overflowStart ?? maximumTrackedInvocationArguments, overflowValues);
    }
    mergeCarrierPositionalState(array, lengths, unknown, uncertainValues);
    if (!unknown) mergeCarrierExactPositionalLengths(array, lengths);
    return new Set([array]);
  }
  ...
  function propertyDescriptorsValue(argumentValues, invocationNode) {
    ...
    for (const atom of targetValue) {
      ...
      trackPropagationDependency(atom);
      const propertyOrders = ownPropertyNamesInRuntimeOrder(atom, true);
      const processedPropertyNames = new Set();
      let overflowDescriptor;
      for (const propertyOrder of propertyOrders) {
        for (const propertyName of propertyOrder) {
          if (processedPropertyNames.has(propertyName)) continue;
          processedPropertyNames.add(propertyName);
          if (processedPropertyNames.size > maximumTrackedInvocationArguments) {
            overflowDescriptor ??= syntheticCarrierFor(invocationNode, 'property-descriptor:overflow');
            putPropertyDescriptorFields(overflowDescriptor, new Set([atom]), [propertyName], true);
            putCarrierProperty(descriptors, undefined, new Set([overflowDescriptor]));
            continue;
          }
          const descriptor = syntheticCarrierFor(invocationNode, `property-descriptor:${propertyName}`);
      ...
      if (propertyOrders.length > 1) {
        for (const propertyName of processedPropertyNames) {
          if (arrayIndexPropertyValue(propertyName) === undefined) {
            descriptors.possiblyReorderedOwnPropertyNames.add(propertyName);
          }
        }
      }
      const unrepresentedOverflowValues = unrepresentedOverflowOwnPropertyValue(atom, processedPropertyNames);
      if (unrepresentedOverflowValues.size > 0) {
        overflowDescriptor ??= syntheticCarrierFor(invocationNode, 'property-descriptor:overflow');
        putCarrierProperty(overflowDescriptor, ['value'], unrepresentedOverflowValues);
        putCarrierProperty(descriptors, undefined, new Set([overflowDescriptor]));
      }
  ...
  function valueMayInvokeTrackedSetter(value) {
    ...
    return (
      knownTargets.size > 0 &&
      callableMayInstallTrackedEffects(possibleAccessorSetters(knownTargets, undefined).callables)
    );
  }
  ...
  function invokeTracked(
    ...
    try {
      for (const atom of callable) {
        ...
        } else if (atom === origins.lodashValues || atom === origins.objectEntries || atom === origins.objectValues) {
          mergeValue(
            invocation.result,
  ...
  function recordInvocationEvidence(invocationNode, kinds, includeAnalysisLimits = false) {
    const reportedKinds = [...kinds].filter(
      kind =>
        kind === 'direct-eval' ||
        kind === 'lodash-template' ||
        (includeAnalysisLimits && kind.startsWith(`${analysisLimitKind}:`))
    );
    if (reportedKinds.length === 0) return;
    let evidence = invocationEvidence.get(invocationNode);
    if (!evidence) {
      evidence = new Set();
      invocationEvidence.set(invocationNode, evidence);
    }
    for (const kind of reportedKinds) evidence.add(kind);
  }
  ...
  function assignToTarget(node, value, effectNode = node) {
    const current = unwrapExpression(node);
    if (ts.isIdentifier(current)) {
      const binding = lookupBinding(current);
      if (binding) {
    ...
    if (ts.isPropertyAccessExpression(current) || ts.isElementAccessExpression(current)) {
      ...
      if (
        carrierTargets.length === 1 &&
        [...targetValue].every(
          target =>
            target === carrierTargets[0] ||
            (typeof target !== 'string' && target.kind === 'unknown-value' && target.opaqueThis)
        )
      ) {
      ...
      const recreatesDeletedProperty =
        indexedWrite &&
        [...writeTargetValue].some(
          target =>
            typeof target !== 'string' && target.kind === 'carrier' && target.deletedProperties.has(propertyNames[0])
        );
      ...
      let arrayLengthWriteOutcome;
      if (writeSuccess === undefined && propertyNames?.length === 1 && propertyNames[0] === 'length') {
        ...
        writeSuccess = arrayLengthWriteOutcome;
      }
      const writeFailure =
      ...
      const setterDefinitelyConsumesWrite =
        writeSuccess === true && writeEffect.setterConsumesWrite && !writeEffect.receiverOwnProperty;
      const setterMayConsumeInsteadOfWritingReceiver =
        writeSuccess === true && writeEffect.setterConsumesWrite && writeEffect.receiverOwnProperty;
      const uncertainWrite = writeSuccess !== true;
      ...
      if (conditionalReceiverWrite && propertyNames?.length === 1) {
        for (const target of writeTargetValue) {
          if (
            typeof target !== 'string' &&
            target.kind === 'carrier' &&
            indexedOwnDataPropertyState(target, propertyNames[0]).mayBeMissing
          ) {
            possiblyAbsentTargets.push(target);
          }
        }
      }
      ...
      if ((propertyNames === undefined || propertyNames.includes('__proto__')) && invokesSetter) {
        ...
        if (writeSuccess === true && writeTargetValue.size === 1 && propertyNames?.length === 1) {
          const target = [...writeTargetValue][0];
          if (typeof target !== 'string' && target.kind === 'carrier') {
        ...
        if (conditionalReceiverWrite && propertyNames?.length === 1) {
          for (const target of possiblyAbsentTargets) recordTargetDeletion(new Set([target]), propertyNames);
        }
      }
      if (conditionalReceiverWrite) activeAlternativeMutationDepth -= 1;
      return;
    ...
    if (ts.isArrayLiteralExpression(current)) {
      ...
      for (const [index, element] of current.elements.entries()) {
        ...
        if (ts.isSpreadElement(element)) {
          assignToTarget(element.expression, arrayRestValue(value, index, element, expansion), effectNode);
        } else {
          assignToTarget(element, positionalValueAt(expansion, index), effectNode);
        }
      }
      return;
    }
    if (ts.isObjectLiteralExpression(current)) {
      for (const property of current.properties) {
        if (ts.isShorthandPropertyAssignment(property)) {
          assignToTarget(property.name, observedPropertyValue(value, [property.name.text]), effectNode);
        } else if (ts.isPropertyAssignment(property)) {
          assignToTarget(
            property.initializer,
            observedPropertyValue(value, declaredPropertyNames(property.name)),
            effectNode
          );
        } else if (ts.isSpreadAssignment(property)) {
          observeOwnAccessorReads(value);
          assignToTarget(property.expression, value, effectNode);
        }
      }
    }
  }
  ...
  function collectPropagationOperations(node) {
    ...
    if (ts.isVariableDeclaration(node)) {
      ...
      run = () => assignToTarget(node.left, evaluateExpression(node.right), node);
    } else if (ts.isBinaryExpression(node) && ts.isAssignmentOperator(node.operatorToken.kind)) {
  ...
  return { analysisLimit, invocationEvidence, unsafeKindsForCall, unsafeKindsForNew, unsafeKindsForTag };
}
...
function scanSource(source, relativePath) {
  ...
  try {
    ...
    while (pending.length > 0 && findings.length < maximumFindingsPerFile) {
      ...
      } else if (ts.isBinaryExpression(node) || ts.isForInStatement(node) || ts.isForOfStatement(node)) {
        addFindings(node, new Set(bindings.invocationEvidence.get(node) ?? []));
      }
...
function runGuard(root = repositoryRoot) {
  ...
  const productionExcludedSourcePaths = new Set([...excludedSourcePaths, diagnosticSelfSourcePath]);
  const findings = scanRepository(root, productionExcludedSourcePaths);
