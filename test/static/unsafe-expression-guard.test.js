...
test('non-configurable indexed properties block every deterministic length truncation path', () => {
  const mutationModes = [
    ['assignment', (target, length) => `${target}.length=${length};`],
    ['Reflect.set', (target, length) => `Reflect.set(${target},'length',${length});`],
    ['Object.assign', (target, length) => `try{Object.assign(${target},{length:${length}})}catch{}`],
    [
      'Object.defineProperty',
      (target, length) => `try{Object.defineProperty(${target},'length',{value:${length}})}catch{}`,
    ],
    ['Reflect.defineProperty', (target, length) => `Reflect.defineProperty(${target},'length',{value:${length}});`],
    [
      'Object.defineProperties',
      (target, length) => `try{Object.defineProperties(${target},{length:{value:${length}}})}catch{}`,
    ],
  ];
  const paths = [
    ['direct', () => '', (mutation, index) => mutation('values', index)],
    ['helper', (mutation, index) => `function shrink(value){${mutation('value', index)}}`, () => 'shrink(values);'],
  ];

  for (const [caseIndex, truncationCase] of [1, 64, 65]
    .flatMap(index =>
      mutationModes.flatMap(([modeName, mutation]) =>
        paths.map(([pathName, prefix, invoke]) => ({
          index,
          name: `${pathName} ${modeName} at index ${index}`,
          source: `${prefix(mutation, index)}const values=Array(${index + 1}).fill(JSON.parse);
            values[${index}]=eval;Object.seal(values);${invoke(mutation, index)}
            values[${index}](configuredSource);`,
        }))
      )
    )
    .entries()) {
    const relativePath = `packages/example/src/non-configurable-length-truncation-${caseIndex}.ts`;
    const firstFindings = scanSource(truncationCase.source, relativePath);
    const secondFindings = scanSource(truncationCase.source, relativePath);
    assert.deepEqual(firstFindings, secondFindings, `${truncationCase.name} should be deterministic`);
    assert.ok(
      firstFindings.some(finding => finding.kind === 'direct-eval'),
      `${truncationCase.name} should preserve the non-configurable eval: ${JSON.stringify(firstFindings)}`
    );
    assert.ok(firstFindings.length <= 2, `${truncationCase.name} diagnostics should remain bounded`);
  }

  for (const [caseIndex, partialCase] of [1, 64, 65]
    .flatMap(index =>
      mutationModes.map(([modeName, mutation]) => {
        const inheritedIndex = index + 1;
        return {
          name: `${modeName} partial failure at index ${index}`,
          source: `const prototype=[];prototype[${inheritedIndex}]=eval;
            const values=Array(${inheritedIndex + 1}).fill(JSON.parse);Object.setPrototypeOf(values,prototype);
            Object.defineProperty(values,'${index}',{value:JSON.parse,configurable:false});
            ${mutation('values', index)}values[${inheritedIndex}](configuredSource);`,
        };
      })
    )
    .entries()) {
    const relativePath = `packages/example/src/partial-failed-length-truncation-${caseIndex}.ts`;
    const findings = scanSource(partialCase.source, relativePath);
    assert.ok(
      findings.some(finding => finding.kind === 'direct-eval'),
      `${partialCase.name} should expose the inherited eval above the restored length: ${JSON.stringify(findings)}`
    );
    assert.ok(findings.length <= 2, `${partialCase.name} diagnostics should remain bounded`);
  }

  for (const [indexPosition, index] of [1, 64, 65].entries()) {
    const source = `const values=Array(${index + 1}).fill(JSON.parse);
      try{Object.defineProperties(values,{
        length:{value:${index}},${index}:{value:eval,configurable:false}
      })}catch{}values[${index}](configuredSource);`;
    const findings = scanSource(
      source,
      `packages/example/src/non-configurable-descriptor-key-order-${indexPosition}.ts`
    );
    assert.ok(
      findings.some(finding => finding.kind === 'direct-eval'),
      `integer-key ordering should install and retain eval at index ${index}: ${JSON.stringify(findings)}`
    );
    assert.ok(findings.length <= 2, `integer-key ordering at index ${index} should remain bounded`);
  }
});

test('failed and partially failed length truncations execute isolated runtime markers', () => {
  const markerKey = '__a12_failed_length_truncation_marker__';
  const mutationModes = [
    ['assignment', (target, length) => `${target}.length=${length};`],
    ['reflect-set', (target, length) => `Reflect.set(${target},'length',${length});`],
    ['assign', (target, length) => `try{Object.assign(${target},{length:${length}})}catch{}`],
    ['define', (target, length) => `try{Object.defineProperty(${target},'length',{value:${length}})}catch{}`],
    ['reflect-define', (target, length) => `Reflect.defineProperty(${target},'length',{value:${length}});`],
    ['defines', (target, length) => `try{Object.defineProperties(${target},{length:{value:${length}}})}catch{}`],
  ];
  const blockedBranches = [1, 64, 65].flatMap(index =>
    mutationModes.flatMap(([modeName, mutation]) => [
      `{const values=Array(${index + 1}).fill(JSON.parse);values[${index}]=eval;Object.seal(values);
        ${mutation('values', index)}globalThis.${markerKey}.push('${modeName}-direct-${index}-length-'+values.length);
        values[${index}](mark('${modeName}-direct-${index}-eval'));}`,
      `{function shrink(value){${mutation('value', index)}}const values=Array(${index + 1}).fill(JSON.parse);
        values[${index}]=eval;Object.seal(values);shrink(values);
        globalThis.${markerKey}.push('${modeName}-helper-${index}-length-'+values.length);
        values[${index}](mark('${modeName}-helper-${index}-eval'));}`,
    ])
  );
  const partialBranches = [1, 64, 65].flatMap(index =>
    mutationModes.map(([modeName, mutation]) => {
      const inheritedIndex = index + 1;
      return `{const prototype=[];prototype[${inheritedIndex}]=eval;
        const values=Array(${inheritedIndex + 1}).fill(JSON.parse);Object.setPrototypeOf(values,prototype);
        Object.defineProperty(values,'${index}',{value:JSON.parse,configurable:false});${mutation('values', index)}
        globalThis.${markerKey}.push('${modeName}-partial-${index}-length-'+values.length);
        globalThis.${markerKey}.push('${modeName}-partial-${index}-own-'+Object.hasOwn(values,'${inheritedIndex}'));
        values[${inheritedIndex}](mark('${modeName}-partial-${index}-eval'));}`;
    })
  );
  const source = [
    `globalThis.${markerKey}=[];`,
  ...
  assert.equal(result.status, 0, result.stderr);
  assert.deepEqual(JSON.parse(result.stdout), [
    ...[1, 64, 65].flatMap(index =>
      mutationModes.flatMap(([modeName]) => [
        `${modeName}-direct-${index}-length-${index + 1}`,
        `${modeName}-direct-${index}-eval`,
        `${modeName}-helper-${index}-length-${index + 1}`,
        `${modeName}-helper-${index}-eval`,
      ])
    ),
    ...[1, 64, 65].flatMap(index =>
      mutationModes.flatMap(([modeName]) => [
        `${modeName}-partial-${index}-length-${index + 1}`,
        `${modeName}-partial-${index}-own-false`,
        `${modeName}-partial-${index}-eval`,
      ])
    ),
  ]);
});

test('frozen inherited indexed descriptors block reflective and sloppy replacements', () => {
  const mutationModes = [
    ['Reflect.set', (target, index) => `Reflect.set(${target},'${index}',JSON.parse);`],
    ['sloppy assignment', (target, index) => `${target}[${index}]=JSON.parse;`],
  ];
  const paths = [
    ['direct', () => '', (mutation, index) => mutation('values', index)],
    [
  ...
  for (const [caseIndex, failureCase] of [1, 64, 65]
    .flatMap(index =>
      mutationModes.flatMap(([modeName, mutation]) =>
        paths.map(([pathName, prefix, invoke]) => ({
          name: `${pathName} ${modeName} at index ${index}`,
          source: `${prefix(mutation, index)}const prototype=[];prototype[${index}]=eval;
            Object.freeze(prototype);const values=Array(${index});Object.setPrototypeOf(values,prototype);
            ${invoke(mutation, index)}values[${index}](configuredSource);`,
        }))
      )
    )
    .entries()) {
    const relativePath = `packages/example/src/frozen-inherited-indexed-write-${caseIndex}.ts`;
    const firstFindings = scanSource(failureCase.source, relativePath);
    const secondFindings = scanSource(failureCase.source, relativePath);
    assert.deepEqual(firstFindings, secondFindings, `${failureCase.name} should be deterministic`);
    assert.ok(
...
test('frozen inherited indexed write failures execute isolated runtime markers', () => {
  const markerKey = '__a12_frozen_inherited_write_marker__';
  const cases = [1, 64, 65].flatMap(index => [
    {
      mutation: `const succeeded=Reflect.set(values,'${index}',JSON.parse);
        globalThis.${markerKey}.push('reflect-${index}-write-'+succeeded);`,
      name: `reflect-${index}`,
    },
    {
      mutation: `values[${index}]=JSON.parse;`,
      name: `sloppy-${index}`,
    },
    {
      mutation: `function replace(value){return Reflect.set(value,'${index}',JSON.parse)}
        const succeeded=replace(values);globalThis.${markerKey}.push('helper-${index}-write-'+succeeded);`,
      name: `helper-${index}`,
    },
  ]);
  const source = [
    ...
    `const mark=name=>"globalThis.${markerKey}.push("+JSON.stringify(name)+")";`,
    ...cases.map(
      failureCase => `{const prototype=[];prototype[${failureCase.name.split('-').at(-1)}]=eval;
        Object.freeze(prototype);const values=Array(${failureCase.name.split('-').at(-1)});
        Object.setPrototypeOf(values,prototype);${failureCase.mutation}
        globalThis.${markerKey}.push('${failureCase.name}-own-'+Object.hasOwn(values,'${failureCase.name.split('-').at(-1)}'));
        values[${failureCase.name.split('-').at(-1)}](mark('${failureCase.name}-eval'));}`
    ),
    `process.stdout.write(JSON.stringify(globalThis.${markerKey}));delete globalThis.${markerKey};`,
  ].join('\n');
  const result = spawnSync(process.execPath, ['--eval', source], {
  ...
  assert.deepEqual(
    JSON.parse(result.stdout),
    [1, 64, 65].flatMap(index => [
      `reflect-${index}-write-false`,
      `reflect-${index}-own-false`,
      `reflect-${index}-eval`,
      `sloppy-${index}-own-false`,
      `sloppy-${index}-eval`,
      `helper-${index}-write-false`,
      `helper-${index}-own-false`,
      `helper-${index}-eval`,
    ])
  );
});

test('Reflect.defineProperty distinguishes negative and positive zero with SameValue semantics', () => {
  const paths = [
    ['direct', index => '', index => `Reflect.defineProperty(values,'${index}',{value:0})`],
    [
      'helper',
      index => `function replace(value){return Reflect.defineProperty(value,'${index}',{value:0})}`,
      () => 'replace(values)',
    ],
  ];
  for (const [caseIndex, descriptorCase] of [1, 64, 65]
    .flatMap(index =>
      paths.map(([pathName, prefix, mutation]) => ({
        name: `${pathName} signed-zero replacement at index ${index}`,
        source: `${prefix(index)}const values=[];
          Object.defineProperty(values,'${index}',{value:-0,writable:false,configurable:false});
          const succeeded=${mutation(index)};
          ({true:JSON.parse,false:eval})[succeeded](configuredSource);`,
      }))
    )
    .entries()) {
    const relativePath = `packages/example/src/signed-zero-same-value-${caseIndex}.ts`;
    const firstFindings = scanSource(descriptorCase.source, relativePath);
    const secondFindings = scanSource(descriptorCase.source, relativePath);
    assert.deepEqual(firstFindings, secondFindings, `${descriptorCase.name} should be deterministic`);
    assert.ok(
      firstFindings.some(finding => finding.kind === 'direct-eval'),
      `${descriptorCase.name} should select the false branch: ${JSON.stringify(firstFindings)}`
    );
    assert.ok(firstFindings.length <= 2, `${descriptorCase.name} diagnostics should remain bounded`);
  }

  const sameValueControls = [
    ['negative zero', '-0', '-0'],
    ['positive zero', '0', '0'],
    ['number', '1', '1'],
    ['string', "'same'", "'same'"],
  ...
  for (const [caseIndex, [name, currentValue, requestedValue]] of sameValueControls.entries()) {
    const source = `const values=[];
      Object.defineProperty(values,'1',{value:${currentValue},writable:false,configurable:false});
      const succeeded=Reflect.defineProperty(values,'1',{value:${requestedValue}});
      ({true:JSON.parse,false:eval})[succeeded]('{}');`;
    assert.deepEqual(
      scanSource(source, `packages/example/src/safe-same-value-primitive-${caseIndex}.ts`),
      [],
      `${name} should retain SameValue-compatible descriptor precision`
    );
  }
});

test('signed-zero descriptor failures execute isolated runtime markers', () => {
  const markerKey = '__a12_signed_zero_descriptor_marker__';
  const cases = [1, 64, 65].flatMap(index => [
    {
      mutation: `Reflect.defineProperty(values,'${index}',{value:0})`,
      name: `direct-${index}`,
      prefix: '',
    },
    {
      mutation: 'replace(values)',
      name: `helper-${index}`,
      prefix: `function replace(value){return Reflect.defineProperty(value,'${index}',{value:0})}`,
    },
  ]);
  const source = [
    `globalThis.${markerKey}=[];`,
    `const mark=name=>"globalThis.${markerKey}.push("+JSON.stringify(name)+")";`,
    ...cases.map(descriptorCase => {
      const index = descriptorCase.name.split('-').at(-1);
      return `{${descriptorCase.prefix}const values=[];
        Object.defineProperty(values,'${index}',{value:-0,writable:false,configurable:false});
        const succeeded=${descriptorCase.mutation};globalThis.${markerKey}.push('${descriptorCase.name}-write-'+succeeded);
        ({true:JSON.parse,false:eval})[succeeded](mark('${descriptorCase.name}-eval'));}`;
    }),
    `process.stdout.write(JSON.stringify(globalThis.${markerKey}));delete globalThis.${markerKey};`,
  ].join('\n');
  const result = spawnSync(process.execPath, ['--eval', source], {
  ...
  assert.deepEqual(
    JSON.parse(result.stdout),
    cases.flatMap(descriptorCase => [`${descriptorCase.name}-write-false`, `${descriptorCase.name}-eval`])
  );
});

test('definitely failed reflective writes expose only the false result branch', () => {
  const mutationModes = [
    ['Reflect.set', (target, index) => `Reflect.set(${target},'${index}',JSON.parse)`],
    ['Reflect.defineProperty', (target, index) => `Reflect.defineProperty(${target},'${index}',{value:eval})`],
  ];
  const paths = [
    ['direct', () => '', (mutation, index) => mutation('values', index)],
    [
  ...
  for (const [caseIndex, failureCase] of [1, 64, 65]
    .flatMap(index =>
      mutationModes.flatMap(([modeName, mutation]) =>
        paths.map(([pathName, prefix, invoke]) => ({
          index,
          name: `${pathName} ${modeName} failure at index ${index}`,
          prefix: prefix(mutation, index),
          mutation: invoke(mutation, index),
        }))
      )
    ...
    .entries()) {
    const setup = `${failureCase.prefix}const values=Array(${failureCase.index + 1}).fill(JSON.parse);
      Object.freeze(values);const succeeded=${failureCase.mutation};`;
    assert.deepEqual(
      scanSource(
        `${setup}({true:eval,false:JSON.parse})[succeeded]('{}');`,
        `packages/example/src/safe-failed-reflective-result-${caseIndex}.ts`
      ),
      [],
      `${failureCase.name} should not retain an impossible true branch`
    );
    const findings = scanSource(
      `${setup}({true:JSON.parse,false:eval})[succeeded](configuredSource);`,
      `packages/example/src/unsafe-failed-reflective-result-${caseIndex}.ts`
    );
    assert.ok(
      findings.some(finding => finding.kind === 'direct-eval'),
      `${failureCase.name} should retain the actual false branch: ${JSON.stringify(findings)}`
    );
    assert.ok(findings.length <= 2, `${failureCase.name} diagnostics should remain bounded`);
  }

  for (const [caseIndex, uncertainCase] of mutationModes
    .flatMap(([modeName, mutation]) =>
      paths.map(([pathName, prefix, invoke]) => ({
        name: `${pathName} ${modeName}`,
        prefix: prefix(mutation, 0),
        mutation: invoke(mutation, 0),
      }))
    )
    .entries()) {
    const source = `${uncertainCase.prefix}const values=[JSON.parse];if(flag)Object.freeze(values);
      const succeeded=${uncertainCase.mutation};
      ({true:eval,false:JSON.parse})[succeeded](configuredSource);`;
    const findings = scanSource(source, `packages/example/src/uncertain-reflective-result-${caseIndex}.ts`);
    assert.ok(
      findings.some(finding => finding.kind === 'direct-eval'),
      `${uncertainCase.name} should preserve a feasible true branch: ${JSON.stringify(findings)}`
    );
    assert.ok(findings.length <= 2, `${uncertainCase.name} uncertain results should remain bounded`);
  }

  for (const [caseIndex, [pathName, prefix, mutation]] of [
    ['direct', '', "Reflect.defineProperty(values,'length',{value:0})"],
    ['helper', "function resize(value){return Reflect.defineProperty(value,'length',{value:0})}", 'resize(values)'],
  ].entries()) {
    const successfulSetup = `${prefix}const values=[];Object.freeze(values);const succeeded=${mutation};`;
    assert.deepEqual(
      scanSource(
        `${successfulSetup}({true:JSON.parse,false:eval})[succeeded]('{}');`,
        `packages/example/src/safe-same-frozen-length-result-${caseIndex}.ts`
      ),
      [],
      `${pathName} SameValue length descriptor should retain only the true result branch`
    );
    const failedSetup = `${prefix}const values=[JSON.parse];Object.freeze(values);const succeeded=${mutation};`;
    const findings = scanSource(
      `${failedSetup}({true:JSON.parse,false:eval})[succeeded](configuredSource);`,
      `packages/example/src/unsafe-failed-frozen-length-result-${caseIndex}.ts`
    );
    assert.ok(
      findings.some(finding => finding.kind === 'direct-eval'),
      `${pathName} incompatible frozen length should retain the false branch: ${JSON.stringify(findings)}`
    );
  }
});

test('reflective result truth tables execute isolated runtime markers', () => {
  const markerKey = '__a12_reflective_result_truth_table_marker__';
  const mutationModes = [
    ['set', (target, index) => `Reflect.set(${target},'${index}',JSON.parse)`],
    ['define', (target, index) => `Reflect.defineProperty(${target},'${index}',{value:eval})`],
  ];
  const paths = [
    ['direct', () => '', (mutation, index) => mutation('values', index)],
    [
  ...
  const lengthCases = [
    {
      mutation: "Reflect.defineProperty(values,'length',{value:0})",
      name: 'length-direct',
      prefix: '',
    },
    {
      mutation: 'resize(values)',
      name: 'length-helper',
      prefix: "function resize(value){return Reflect.defineProperty(value,'length',{value:0})}",
    },
  ];
  const source = [
    `globalThis.${markerKey}=[];`,
    `const mark=name=>"globalThis.${markerKey}.push("+JSON.stringify(name)+")";`,
    ...cases.flatMap(reflectiveCase => [
      `{${reflectiveCase.prefix}const values=Array(${reflectiveCase.index + 1}).fill(JSON.parse);
        Object.freeze(values);const succeeded=${reflectiveCase.mutation};
        globalThis.${markerKey}.push('${reflectiveCase.name}-safe-write-'+succeeded);
        ({true:eval,false:JSON.parse})[succeeded]('{}');
        globalThis.${markerKey}.push('${reflectiveCase.name}-safe-branch');}`,
      `{${reflectiveCase.prefix}const values=Array(${reflectiveCase.index + 1}).fill(JSON.parse);
        Object.freeze(values);const succeeded=${reflectiveCase.mutation};
        globalThis.${markerKey}.push('${reflectiveCase.name}-unsafe-write-'+succeeded);
        ({true:JSON.parse,false:eval})[succeeded](mark('${reflectiveCase.name}-unsafe-eval'));}`,
    ]),
    ...lengthCases.flatMap(reflectiveCase => [
      `{${reflectiveCase.prefix}const values=[];Object.freeze(values);const succeeded=${reflectiveCase.mutation};
        globalThis.${markerKey}.push('${reflectiveCase.name}-same-write-'+succeeded);
        ({true:JSON.parse,false:eval})[succeeded]('{}');}`,
      `{${reflectiveCase.prefix}const values=[JSON.parse];Object.freeze(values);const succeeded=${reflectiveCase.mutation};
        globalThis.${markerKey}.push('${reflectiveCase.name}-different-write-'+succeeded);
        ({true:JSON.parse,false:eval})[succeeded](mark('${reflectiveCase.name}-different-eval'));}`,
    ]),
    `process.stdout.write(JSON.stringify(globalThis.${markerKey}));delete globalThis.${markerKey};`,
  ].join('\n');
  const result = spawnSync(process.execPath, ['--eval', source], {
  ...
  assert.deepEqual(
    JSON.parse(result.stdout),
    cases
      .flatMap(reflectiveCase => [
        `${reflectiveCase.name}-safe-write-false`,
        `${reflectiveCase.name}-safe-branch`,
        `${reflectiveCase.name}-unsafe-write-false`,
        `${reflectiveCase.name}-unsafe-eval`,
      ])
      .concat(
        lengthCases.flatMap(reflectiveCase => [
          `${reflectiveCase.name}-same-write-true`,
          `${reflectiveCase.name}-different-write-false`,
          `${reflectiveCase.name}-different-eval`,
        ])
      )
  );
});

test('non-writable array lengths allow reflective writes into existing holes', () => {
  const modes = [
    ['Reflect.set', (target, index) => `Reflect.set(${target},'${index}',JSON.parse)`],
    ['Reflect.defineProperty', (target, index) => `Reflect.defineProperty(${target},'${index}',{value:JSON.parse})`],
  ];
  const paths = [
    ['direct', () => '', (mutation, index) => mutation('values', index)],
    [
  ...
  for (const [caseIndex, writeCase] of [1, 64, 65]
    .flatMap(index =>
      modes.flatMap(([modeName, mutation]) =>
        paths.map(([pathName, prefix, invoke]) => ({
          index,
          name: `${pathName} ${modeName} at existing hole ${index}`,
          prefix: prefix(mutation, index),
          mutation: invoke(mutation, index),
        }))
      )
    ...
    .entries()) {
    const source = `${writeCase.prefix}const values=Array(${writeCase.index + 1});
      Object.defineProperty(values,'length',{writable:false});
      const succeeded=${writeCase.mutation};
      ({true:eval,false:JSON.parse})[succeeded](configuredSource);`;
    const relativePath = `packages/example/src/non-writable-existing-hole-${caseIndex}.ts`;
    const firstFindings = scanSource(source, relativePath);
    const secondFindings = scanSource(source, relativePath);
    assert.deepEqual(firstFindings, secondFindings, `${writeCase.name} should be deterministic`);
    assert.ok(
      firstFindings.some(finding => finding.kind === 'direct-eval'),
      `${writeCase.name} should select the successful branch: ${JSON.stringify(firstFindings)}`
    );
    assert.ok(firstFindings.length <= 2, `${writeCase.name} diagnostics should remain bounded`);

    const extendingSetup = `${writeCase.prefix}const values=Array(${writeCase.index});
      Object.defineProperty(values,'length',{writable:false});const succeeded=${writeCase.mutation};`;
    if (writeCase.name.startsWith('direct ')) {
      assert.deepEqual(
        scanSource(
          `${extendingSetup}({true:eval,false:JSON.parse})[succeeded]('{}');`,
          `packages/example/src/safe-non-writable-extending-hole-${caseIndex}.ts`
        ),
        [],
        `${writeCase.name} should retain no impossible success branch when the write extends length`
      );
    }
    const extendingFindings = scanSource(
      `${extendingSetup}({true:JSON.parse,false:eval})[succeeded](configuredSource);`,
      `packages/example/src/failed-non-writable-extending-hole-${caseIndex}.ts`
    );
    assert.ok(
      extendingFindings.some(finding => finding.kind === 'direct-eval'),
      `${writeCase.name} should preserve the extending-write failure branch`
    );
  }
});

test('existing-hole reflective writes execute the runtime success branch', () => {
  const markerKey = '__a12_existing_hole_write_marker__';
  const modes = [
    ['set', (target, index) => `Reflect.set(${target},'${index}',JSON.parse)`],
    ['define', (target, index) => `Reflect.defineProperty(${target},'${index}',{value:JSON.parse})`],
  ];
  ...
  const source = [
    ...
    `const mark=name=>"globalThis.${markerKey}.push("+JSON.stringify(name)+")";`,
    ...cases.map(
      writeCase => `{${writeCase.prefix}const values=Array(${writeCase.index + 1});
        Object.defineProperty(values,'length',{writable:false});const succeeded=${writeCase.mutation};
        globalThis.${markerKey}.push('${writeCase.name}-write-'+succeeded);
        globalThis.${markerKey}.push('${writeCase.name}-own-'+Object.hasOwn(values,'${writeCase.index}'));
        ({true:eval,false:JSON.parse})[succeeded](mark('${writeCase.name}-eval'));}`
    ),
    `process.stdout.write(JSON.stringify(globalThis.${markerKey}));delete globalThis.${markerKey};`,
  ].join('\n');
  const result = spawnSync(process.execPath, ['--eval', source], {
  ...
  assert.deepEqual(
    JSON.parse(result.stdout),
    cases.flatMap(writeCase => [`${writeCase.name}-write-true`, `${writeCase.name}-own-true`, `${writeCase.name}-eval`])
  );
});

test('partial truncation retains the requested non-writable length transition', () => {
  const truncationForms = [
    ['direct', '', "Reflect.defineProperty(values,'length',{value:0,writable:false});"],
    [
      'helper',
      "function shrink(value){return Reflect.defineProperty(value,'length',{value:0,writable:false})}",
      'shrink(values);',
    ],
    ['Object.defineProperty', '', "try{Object.defineProperty(values,'length',{value:0,writable:false})}catch{}"],
    [
      'Object.defineProperty helper',
      "function shrink(value){try{Object.defineProperty(value,'length',{value:0,writable:false})}catch{}}",
      'shrink(values);',
    ],
    ['Object.defineProperties', '', 'try{Object.defineProperties(values,{length:{value:0,writable:false}})}catch{}'],
    [
      'Object.defineProperties helper',
      'function shrink(value){try{Object.defineProperties(value,{length:{value:0,writable:false}})}catch{}}',
      'shrink(values);',
    ],
    [
      'conditional descriptor',
      '',
      "Reflect.defineProperty(values,'length',flag?{value:0,writable:false}:{value:0,writable:false});",
    ],
    ['spread descriptor', '', "Reflect.defineProperty(values,'length',{...{value:0,writable:false}});"],
  ];
  const laterWrites = [
    ['assignment', '', index => `values[${index}]=JSON.parse;`],
    ['helper assignment', index => `function replace(value){value[${index}]=JSON.parse}`, () => 'replace(values);'],
    ['Reflect.set', '', index => `Reflect.set(values,'${index}',JSON.parse);`],
    ['Object.assign', '', index => `try{Object.assign(values,{${index}:JSON.parse})}catch{}`],
    ['Object.defineProperty', '', index => `try{Object.defineProperty(values,'${index}',{value:JSON.parse})}catch{}`],
    ['Reflect.defineProperty', '', index => `Reflect.defineProperty(values,'${index}',{value:JSON.parse});`],
    [
      'Object.defineProperties',
      '',
      index => `try{Object.defineProperties(values,{${index}:{value:JSON.parse}})}catch{}`,
    ],
  ];

  for (const [caseIndex, truncationCase] of [1, 64, 65]
    .flatMap(index =>
      truncationForms.flatMap(([formName, formPrefix, truncate]) =>
        laterWrites.map(([writeName, writePrefix, write]) => ({
          index,
          name: `${formName}, ${writeName}, index ${index}`,
          source: `${formPrefix}${typeof writePrefix === 'function' ? writePrefix(index) : writePrefix}
            const prototype=[];prototype[${index}]=eval;
            const values=Array(${index + 1}).fill(JSON.parse);Object.setPrototypeOf(values,prototype);
            Object.defineProperty(values,'${index - 1}',{configurable:false});${truncate}
            ${write(index)}values[${index}](configuredSource);`,
        }))
      )
    )
    .entries()) {
    const relativePath = `packages/example/src/partial-non-writable-truncation-${caseIndex}.ts`;
    const findings = scanSource(truncationCase.source, relativePath);
    assert.ok(
      findings.some(finding => finding.kind === 'direct-eval'),
      `${truncationCase.name} should preserve inherited eval: ${JSON.stringify(findings)}`
    );
    assert.ok(findings.length <= 2, `${truncationCase.name} diagnostics should remain bounded`);
  }
});

test('partial non-writable truncations execute failed later writes at runtime', () => {
  const markerKey = '__a12_partial_non_writable_marker__';
  const truncationModes = [
    ['object', "Object.defineProperty(values,'length',{value:0,writable:false})", 'throw'],
    ['reflect', "Reflect.defineProperty(values,'length',{value:0,writable:false})", 'false'],
    ['plural', 'Object.defineProperties(values,{length:{value:0,writable:false}})', 'throw'],
  ];
  const laterWrites = [
    ['assignment', index => `values[${index}]=JSON.parse;`],
    ['helper-assignment', index => `function replace(value){value[${index}]=JSON.parse}replace(values);`],
    ['reflect-set', index => `Reflect.set(values,'${index}',JSON.parse);`],
    ['assign', index => `try{Object.assign(values,{${index}:JSON.parse})}catch{}`],
    ['define', index => `try{Object.defineProperty(values,'${index}',{value:JSON.parse})}catch{}`],
    ['reflect-define', index => `Reflect.defineProperty(values,'${index}',{value:JSON.parse});`],
    ['defines', index => `try{Object.defineProperties(values,{${index}:{value:JSON.parse}})}catch{}`],
  ];
  const cases = [1, 64, 65].flatMap(index =>
    truncationModes.flatMap(([truncationName, truncate, outcome]) =>
      laterWrites.map(([name, write]) => ({
        index,
        name: `${truncationName}-${name}-${index}`,
        outcome,
        truncate,
        write: write(index),
      }))
  ...
  const source = [
    ...
    `const mark=name=>"globalThis.${markerKey}.push("+JSON.stringify(name)+")";`,
    ...cases.map(
      truncationCase => `{const prototype=[];prototype[${truncationCase.index}]=eval;
        const values=Array(${truncationCase.index + 1}).fill(JSON.parse);Object.setPrototypeOf(values,prototype);
        Object.defineProperty(values,'${truncationCase.index - 1}',{configurable:false});
        let truncated='success';try{const result=${truncationCase.truncate};if(result===false)truncated='false'}
        catch{truncated='throw'}${truncationCase.write}
        const descriptor=Object.getOwnPropertyDescriptor(values,'length');
        globalThis.${markerKey}.push('${truncationCase.name}-truncate-'+truncated);
        globalThis.${markerKey}.push('${truncationCase.name}-length-'+values.length);
        globalThis.${markerKey}.push('${truncationCase.name}-writable-'+descriptor.writable);
        globalThis.${markerKey}.push('${truncationCase.name}-own-'+Object.hasOwn(values,'${truncationCase.index}'));
        values[${truncationCase.index}](mark('${truncationCase.name}-eval'));}`
    ),
    `process.stdout.write(JSON.stringify(globalThis.${markerKey}));delete globalThis.${markerKey};`,
  ].join('\n');
  const result = spawnSync(process.execPath, ['--eval', source], {
  ...
  assert.deepEqual(
    JSON.parse(result.stdout),
    cases.flatMap(truncationCase => [
      `${truncationCase.name}-truncate-${truncationCase.outcome}`,
      `${truncationCase.name}-length-${truncationCase.index}`,
      `${truncationCase.name}-writable-false`,
      `${truncationCase.name}-own-false`,
      `${truncationCase.name}-eval`,
    ])
  );
});

test('definitely failed accessor definitions do not shadow inherited indexed eval', () => {
  const descriptors = [
    ['getter', '{get(){return JSON.parse}}'],
    ['setter', '{set(_) {}}'],
  ];
  const modes = [
    [
      'Object.defineProperty',
      (target, index, descriptor) => `Object.defineProperty(${target},'${index}',${descriptor})`,
      true,
    ],
    [
      'Reflect.defineProperty',
      (target, index, descriptor) => `Reflect.defineProperty(${target},'${index}',${descriptor})`,
      false,
    ],
    [
      'Object.defineProperties',
      (target, index, descriptor) => `Object.defineProperties(${target},{${index}:${descriptor}})`,
      true,
    ],
  ];
  const paths = [
    ['direct', () => '', (mutation, catches) => (catches ? `try{${mutation}}catch{}` : `${mutation};`)],
    [
      'helper',
      (mutation, catches) =>
        catches
          ? `function install(value){${mutation.replace('values', 'value')}}`
          : `function install(value){return ${mutation.replace('values', 'value')}}`,
      (mutation, catches) => (catches ? 'try{install(values)}catch{}' : 'install(values);'),
    ],
  ];

  for (const [caseIndex, accessorCase] of [1, 64, 65]
    .flatMap(index =>
      descriptors.flatMap(([descriptorName, descriptor]) =>
        modes.flatMap(([modeName, mutation, catches]) => {
          const directMutation = mutation('values', index, descriptor);
          return paths.map(([pathName, prefix, invoke]) => ({
            index,
            name: `${pathName} ${modeName} ${descriptorName} at index ${index}`,
            source: `${prefix(directMutation, catches)}const prototype=[];prototype[${index}]=eval;
              const values=Array(${index});Object.setPrototypeOf(values,prototype);Object.preventExtensions(values);
              ${invoke(directMutation, catches)}values[${index}](configuredSource);`,
          }));
        })
      )
    )
    .entries()) {
    const relativePath = `packages/example/src/failed-indexed-accessor-${caseIndex}.ts`;
    const findings = scanSource(accessorCase.source, relativePath);
    assert.ok(
      findings.some(finding => finding.kind === 'direct-eval'),
      `${accessorCase.name} should retain inherited eval: ${JSON.stringify(findings)}`
    );
    assert.ok(findings.length <= 2, `${accessorCase.name} diagnostics should remain bounded`);
  }
});

test('failed indexed accessor definitions execute inherited eval at runtime', () => {
  const markerKey = '__a12_failed_accessor_marker__';
  const descriptors = [
    ['getter', '{get(){return JSON.parse}}'],
    ['setter', '{set(_) {}}'],
  ];
  const modes = [
    ['object', (target, index, descriptor) => `Object.defineProperty(${target},'${index}',${descriptor})`, 'throw'],
    ['reflect', (target, index, descriptor) => `Reflect.defineProperty(${target},'${index}',${descriptor})`, 'false'],
    ['plural', (target, index, descriptor) => `Object.defineProperties(${target},{${index}:${descriptor}})`, 'throw'],
  ];
  const cases = [1, 64, 65].flatMap(index =>
    descriptors.flatMap(([descriptorName, descriptor]) =>
      modes.map(([modeName, mutation, outcome]) => ({
        index,
        mutation: mutation('values', index, descriptor),
        name: `${modeName}-${descriptorName}-${index}`,
        outcome,
      }))
    )
  );
  const source = [
    ...
    `const mark=name=>"globalThis.${markerKey}.push("+JSON.stringify(name)+")";`,
    ...cases.map(
      accessorCase => `{const prototype=[];prototype[${accessorCase.index}]=eval;
        const values=Array(${accessorCase.index});Object.setPrototypeOf(values,prototype);
        Object.preventExtensions(values);let outcome='success';
        try{const result=${accessorCase.mutation};if(result===false)outcome='false'}catch{outcome='throw'}
        globalThis.${markerKey}.push('${accessorCase.name}-outcome-'+outcome);
        globalThis.${markerKey}.push('${accessorCase.name}-own-'+Object.hasOwn(values,'${accessorCase.index}'));
        values[${accessorCase.index}](mark('${accessorCase.name}-eval'));}`
    ),
    `process.stdout.write(JSON.stringify(globalThis.${markerKey}));delete globalThis.${markerKey};`,
  ].join('\n');
  const result = spawnSync(process.execPath, ['--eval', source], {
  ...
  assert.deepEqual(
    JSON.parse(result.stdout),
    cases.flatMap(accessorCase => [
      `${accessorCase.name}-outcome-${accessorCase.outcome}`,
      `${accessorCase.name}-own-false`,
      `${accessorCase.name}-eval`,
    ])
  );
});

test('bulk indexed compatibility failures abort before later length keys', () => {
  const forms = [
    ['direct', operation => '', operation => operation],
    [
      'helper',
      operation =>
        operation.startsWith('Object.assign')
          ? 'function bulk(target,source){return Object.assign(target,source)}'
          : 'function bulk(target,source){return Object.defineProperties(target,source)}',
      operation => operation.replace(/^Object\.(?:assign|defineProperties)/, 'bulk'),
    ],
    ['conditional', operation => '', operation => operation.replace(/(\{length:[^;]+\})\)$/, 'flag?$1:$1)')],
    [
      'spread',
      operation => `const source=${operation.slice(operation.indexOf(',') + 1, -1)};`,
      operation => `${operation.slice(0, operation.indexOf(',') + 1)}{...source})`,
    ],
  ];

  for (const [caseIndex, abortCase] of [1, 64, 65]
    .flatMap(index => {
      const operations = [
        ['Object.assign', `Object.assign(values,{length:${index},0:JSON.parse})`, false],
        ['Object.defineProperties', `Object.defineProperties(values,{length:{value:${index}},0:{value:eval}})`, true],
      ];
      return operations.flatMap(([operationName, operation, nonConfigurable]) =>
        forms.map(([formName, prefix, invoke]) => ({
          index,
          name: `${formName} ${operationName} abort at index ${index}`,
          source: `${prefix(operation)}const values=Array(${index + 1}).fill(JSON.parse);values[${index}]=eval;
            Object.defineProperty(values,'0',{writable:false${nonConfigurable ? ',configurable:false' : ''}});
            try{${invoke(operation)}}catch{}values[${index}](configuredSource);`,
        }))
      );
    })
    .entries()) {
    const relativePath = `packages/example/src/bulk-indexed-abort-${caseIndex}.ts`;
    const findings = scanSource(abortCase.source, relativePath);
    assert.ok(
      findings.some(finding => finding.kind === 'direct-eval'),
      `${abortCase.name} should retain the value above the unapplied length: ${JSON.stringify(findings)}`
    );
    assert.ok(findings.length <= 2, `${abortCase.name} diagnostics should remain bounded`);
  }

  for (const [caseIndex, index] of [1, 64, 65].entries()) {
    const assignSource = `const values=Array(${index + 1}).fill(JSON.parse);values[${index}]=eval;
      Object.assign(values,{length:${index},0:JSON.parse});values[${index}]('{}');`;
    assert.deepEqual(
      scanSource(assignSource, `packages/example/src/safe-bulk-assign-${caseIndex}.ts`),
      [],
      `a compatible Object.assign at index ${index} should apply its later length`
    );
    const defineSource = `const values=Array(${index + 1}).fill(JSON.parse);values[${index}]=eval;
      Object.defineProperty(values,'0',{writable:false,configurable:false});
      Object.defineProperties(values,{length:{value:${index}},0:{value:JSON.parse}});values[${index}]('{}');`;
    assert.deepEqual(
      scanSource(defineSource, `packages/example/src/safe-bulk-defines-${caseIndex}.ts`),
      [],
      `a SameValue-compatible Object.defineProperties at index ${index} should apply its later length`
    );
  }
});

test('bulk indexed compatibility failures preserve runtime key order and partial state', () => {
  const markerKey = '__a12_bulk_abort_marker__';
  const cases = [1, 64, 65].flatMap(index => [
    {
      index,
      mutation: `Object.assign(values,{length:${index},0:JSON.parse})`,
      name: `assign-${index}`,
      setup: "Object.defineProperty(values,'0',{writable:false});",
    },
    {
      index,
      mutation: `Object.defineProperties(values,{length:{value:${index}},0:{value:eval}})`,
      name: `defines-${index}`,
      setup: "Object.defineProperty(values,'0',{writable:false,configurable:false});",
    },
  ]);
  const source = [
    `globalThis.${markerKey}=[];`,
    `const mark=name=>"globalThis.${markerKey}.push("+JSON.stringify(name)+")";`,
    ...cases.map(
      abortCase => `{const values=Array(${abortCase.index + 1}).fill(JSON.parse);values[${abortCase.index}]=eval;
        ${abortCase.setup}let threw=false;try{${abortCase.mutation}}catch{threw=true}
        globalThis.${markerKey}.push('${abortCase.name}-throw-'+threw);
        globalThis.${markerKey}.push('${abortCase.name}-length-'+values.length);
        globalThis.${markerKey}.push('${abortCase.name}-own-'+Object.hasOwn(values,'${abortCase.index}'));
        values[${abortCase.index}](mark('${abortCase.name}-eval'));}`
    ),
    `process.stdout.write(JSON.stringify(globalThis.${markerKey}));delete globalThis.${markerKey};`,
  ].join('\n');
  const result = spawnSync(process.execPath, ['--eval', source], {
  ...
  assert.deepEqual(
    JSON.parse(result.stdout),
    cases.flatMap(abortCase => [
      `${abortCase.name}-throw-true`,
      `${abortCase.name}-length-${abortCase.index + 1}`,
      `${abortCase.name}-own-true`,
      `${abortCase.name}-eval`,
    ])
  );
});
...
test('accessor descriptor replacement clears setters and preserves empty accessor properties', () => {
  const paths = [
    ['direct', '', mutation => mutation],
    [
      'helper',
      'const define=(target,key,descriptor)=>Object.defineProperty(target,key,descriptor);',
      mutation => mutation.replace('Object.defineProperty', 'define'),
    ],
  ];
  for (const [caseIndex, accessorCase] of [1, 64, 65]
    .flatMap(index =>
      paths.flatMap(([pathName, prefix, invoke]) => [
        {
          name: `${pathName} removed setter at index ${index}`,
          source: `${prefix}const prototype={};
            Object.defineProperty(prototype,'${index}',{set(_) {},configurable:true});
            ${invoke(`Object.defineProperty(prototype,'${index}',{set:undefined})`)};
            const values=Object.create(prototype);Object.preventExtensions(values);
            const succeeded=Reflect.set(values,'${index}',JSON.parse);
            ({true:JSON.parse,false:eval})[succeeded](configuredSource);`,
        },
        {
          name: `${pathName} empty accessor at index ${index}`,
          source: `${prefix}const prototype={};
            ${invoke(`Object.defineProperty(prototype,'${index}',{get:undefined,set:undefined})`)};
            const values=Object.create(prototype);Object.preventExtensions(values);
            const succeeded=Reflect.set(values,'${index}',JSON.parse);
            ({true:JSON.parse,false:eval})[succeeded](configuredSource);`,
        },
    ...
    .entries()) {
    const relativePath = `packages/example/src/accessor-replacement-${caseIndex}.ts`;
    const firstFindings = scanSource(accessorCase.source, relativePath);
    const secondFindings = scanSource(accessorCase.source, relativePath);
    assert.deepEqual(firstFindings, secondFindings, `${accessorCase.name} should be deterministic`);
    assert.ok(
      firstFindings.some(finding => finding.kind === 'direct-eval'),
      `${accessorCase.name} should retain the false Reflect.set branch: ${JSON.stringify(firstFindings)}`
    );
    assert.ok(firstFindings.length <= 2, `${accessorCase.name} diagnostics should remain bounded`);
  }

  for (const [caseIndex, index] of [1, 64, 65].entries()) {
    const safeCases = [
      `const prototype={};Object.defineProperty(prototype,'${index}',{set(_) {},configurable:true});
        Object.defineProperty(prototype,'${index}',{get:undefined});
        const values=Object.create(prototype);Object.preventExtensions(values);
        const succeeded=Reflect.set(values,'${index}',JSON.parse);
        ({true:JSON.parse,false:eval})[succeeded]('{}');`,
      `const prototype={};Object.defineProperty(prototype,'${index}',{get:undefined,set:undefined});
        const values=Object.create(prototype);Object.preventExtensions(values);
        const succeeded=Reflect.set(values,'${index}',JSON.parse);
        ({true:eval,false:JSON.parse})[succeeded]('{}');`,
      `const define=target=>Reflect.defineProperty(target,'${index}',{get(){return JSON.parse}});
        const values=Array(${index + 1});const succeeded=define(values);
        ({true:JSON.parse,false:eval})[succeeded]('{}');`,
    ];
    for (const [controlIndex, source] of safeCases.entries()) {
      assert.deepEqual(
        scanSource(source, `packages/example/src/safe-accessor-replacement-${caseIndex}-${controlIndex}.ts`),
        [],
        `accessor replacement control ${controlIndex} should remain precise at index ${index}`
      );
    }
  }
});

test('accessor replacement findings agree with direct and helper runtime branches', () => {
  const markerKey = '__a12_accessor_replacement_marker__';
  const cases = [1, 64, 65].flatMap(index =>
    ['direct', 'helper'].flatMap(pathName => {
      const helper =
        pathName === 'helper'
          ? 'const define=(target,key,descriptor)=>Object.defineProperty(target,key,descriptor);'
          : '';
      const define = mutation =>
        pathName === 'helper' ? mutation.replace('Object.defineProperty', 'define') : mutation;
      return [
        {
          mutation: define(`Object.defineProperty(prototype,'${index}',{set:undefined})`),
          name: `removed-${pathName}-${index}`,
          prefix: helper,
          setup: `const prototype={};Object.defineProperty(prototype,'${index}',{set(_) {},configurable:true});`,
        },
        {
          mutation: define(`Object.defineProperty(prototype,'${index}',{get:undefined,set:undefined})`),
          name: `empty-${pathName}-${index}`,
          prefix: helper,
          setup: 'const prototype={};',
        },
      ];
    })
  );
  const source = [
    ...
    `const mark=name=>"globalThis.${markerKey}.push("+JSON.stringify(name)+")";`,
    ...cases.map(
      runtimeCase => `{${runtimeCase.prefix}${runtimeCase.setup}${runtimeCase.mutation};
        const values=Object.create(prototype);Object.preventExtensions(values);
        const succeeded=Reflect.set(values,'${runtimeCase.name.split('-').at(-1)}',JSON.parse);
        globalThis.${markerKey}.push('${runtimeCase.name}-result-'+succeeded);
        ({true:JSON.parse,false:eval})[succeeded](mark('${runtimeCase.name}-eval'));}`
    ),
    `process.stdout.write(JSON.stringify(globalThis.${markerKey}));delete globalThis.${markerKey};`,
  ].join('\n');
  const result = spawnSync(process.execPath, ['--eval', source], {
  ...
  assert.deepEqual(
    JSON.parse(result.stdout),
    cases.flatMap(runtimeCase => [`${runtimeCase.name}-result-false`, `${runtimeCase.name}-eval`])
  );
});

test('Object.assign copies only enumerable own length and indexed properties', () => {
  const paths = [
    ['direct', '', 'Object.assign(values,source)'],
    ['helper', 'const assign=(target,source)=>Object.assign(target,source);', 'assign(values,source)'],
  ];
  for (const [caseIndex, assignCase] of [1, 64, 65]
    .flatMap(index =>
      paths.flatMap(([pathName, prefix, invoke]) =>
        ['length', 'index'].map(propertyKind => ({
          name: `${pathName} non-enumerable ${propertyKind} at index ${index}`,
          source: `${prefix}const values=Array(${index + 1}).fill(JSON.parse);values[${index}]=eval;
            const source={};Object.defineProperty(source,'${propertyKind === 'length' ? 'length' : index}',{
              value:${propertyKind === 'length' ? index : 'JSON.parse'},enumerable:false});
            ${invoke};values[${index}](configuredSource);`,
        }))
      )
    )
    .entries()) {
    const findings = scanSource(assignCase.source, `packages/example/src/non-enumerable-assign-${caseIndex}.ts`);
    assert.ok(
      findings.some(finding => finding.kind === 'direct-eval'),
      `${assignCase.name} should skip the source key: ${JSON.stringify(findings)}`
    );
    assert.ok(findings.length <= 2, `${assignCase.name} diagnostics should remain bounded`);
  }

  for (const [caseIndex, index] of [1, 64, 65].entries()) {
    const safeCases = [
      `const values=Array(${index + 1}).fill(JSON.parse);values[${index}]=eval;
        const source={};Object.defineProperty(source,'${index}',{value:JSON.parse,enumerable:true});
        Object.assign(values,source);values[${index}]('{}');`,
      `const values=Array(${index + 1}).fill(JSON.parse);const source={};
        Object.defineProperty(source,'${index}',{value:eval,enumerable:false});
        Object.assign(values,source);values[${index}]('{}');`,
    ];
    for (const [controlIndex, source] of safeCases.entries()) {
      assert.deepEqual(
        scanSource(source, `packages/example/src/safe-enumerable-assign-${caseIndex}-${controlIndex}.ts`),
        [],
        `Object.assign enumerability control ${controlIndex} should stay clean at index ${index}`
      );
    }
  }
});

test('non-enumerable Object.assign keys are skipped by the Node runtime', () => {
  const markerKey = '__a12_non_enumerable_assign_marker__';
  const cases = [1, 64, 65].flatMap(index =>
    ['direct', 'helper'].flatMap(pathName =>
      ['length', 'index'].map(propertyKind => ({
        index,
        name: `${pathName}-${propertyKind}-${index}`,
        pathName,
        propertyKind,
      }))
    )
  ...
  const source = [
    ...
    `const mark=name=>"globalThis.${markerKey}.push("+JSON.stringify(name)+")";`,
    ...cases.map(
      runtimeCase => `{const values=Array(${runtimeCase.index + 1}).fill(JSON.parse);
      values[${runtimeCase.index}]=eval;const source={};
      Object.defineProperty(source,'${runtimeCase.propertyKind === 'length' ? 'length' : runtimeCase.index}',{
        value:${runtimeCase.propertyKind === 'length' ? runtimeCase.index : 'JSON.parse'},enumerable:false});
      ${runtimeCase.pathName === 'helper' ? 'const assign=(target,source)=>Object.assign(target,source);assign(values,source)' : 'Object.assign(values,source)'};
      globalThis.${markerKey}.push('${runtimeCase.name}-length-'+values.length);
      values[${runtimeCase.index}](mark('${runtimeCase.name}-eval'));}`
    ),
    `process.stdout.write(JSON.stringify(globalThis.${markerKey}));delete globalThis.${markerKey};`,
  ...
  assert.deepEqual(
    JSON.parse(result.stdout),
    cases.flatMap(runtimeCase => [`${runtimeCase.name}-length-${runtimeCase.index + 1}`, `${runtimeCase.name}-eval`])
  );
});

test('Object.defineProperties ignores non-enumerable descriptor-map entries', () => {
  const paths = [
    ['direct', '', 'Object.defineProperties(values,descriptors)'],
    [
      'helper',
      'const defineAll=(target,descriptors)=>Object.defineProperties(target,descriptors);',
      'defineAll(values,descriptors)',
    ],
  ];
  for (const [caseIndex, defineCase] of [1, 64, 65]
    .flatMap(index =>
      paths.flatMap(([pathName, prefix, invoke]) =>
        ['length', 'index'].map(propertyKind => ({
          name: `${pathName} non-enumerable descriptor ${propertyKind} at index ${index}`,
          source: `${prefix}const values=Array(${index + 1}).fill(JSON.parse);values[${index}]=eval;
            const descriptors={};Object.defineProperty(descriptors,'${propertyKind === 'length' ? 'length' : index}',{
              value:{value:${propertyKind === 'length' ? index : 'JSON.parse'}},enumerable:false});
            ${invoke};values[${index}](configuredSource);`,
        }))
      )
    )
    .entries()) {
    const findings = scanSource(defineCase.source, `packages/example/src/non-enumerable-defines-${caseIndex}.ts`);
    assert.ok(
      findings.some(finding => finding.kind === 'direct-eval'),
      `${defineCase.name} should skip the descriptor entry: ${JSON.stringify(findings)}`
    );
    assert.ok(findings.length <= 2, `${defineCase.name} diagnostics should remain bounded`);
  }

  for (const [caseIndex, index] of [1, 64, 65].entries()) {
    const safeCases = [
      `const values=Array(${index + 1}).fill(JSON.parse);values[${index}]=eval;
        const descriptors={};Object.defineProperty(descriptors,'${index}',{
          value:{value:JSON.parse},enumerable:true});Object.defineProperties(values,descriptors);
        values[${index}]('{}');`,
      `const values=Array(${index + 1}).fill(JSON.parse);const descriptors={};
        Object.defineProperty(descriptors,'${index}',{value:{value:eval},enumerable:false});
        Object.defineProperties(values,descriptors);values[${index}]('{}');`,
    ];
    for (const [controlIndex, source] of safeCases.entries()) {
      assert.deepEqual(
        scanSource(source, `packages/example/src/safe-enumerable-defines-${caseIndex}-${controlIndex}.ts`),
        [],
        `Object.defineProperties enumerability control ${controlIndex} should stay clean at index ${index}`
      );
    }
  }
});

test('non-enumerable descriptor-map entries are skipped by the Node runtime', () => {
  const markerKey = '__a12_non_enumerable_defines_marker__';
  const cases = [1, 64, 65].flatMap(index =>
    ['direct', 'helper'].flatMap(pathName =>
      ['length', 'index'].map(propertyKind => ({
        index,
  ...
  const source = [
    ...
    ...cases.map(
      runtimeCase => `{const values=Array(${runtimeCase.index + 1}).fill(JSON.parse);
      values[${runtimeCase.index}]=eval;const descriptors={};
      Object.defineProperty(descriptors,'${runtimeCase.propertyKind === 'length' ? 'length' : runtimeCase.index}',{
        value:{value:${runtimeCase.propertyKind === 'length' ? runtimeCase.index : 'JSON.parse'}},enumerable:false});
      ${runtimeCase.pathName === 'helper' ? 'const defineAll=(target,descriptors)=>Object.defineProperties(target,descriptors);defineAll(values,descriptors)' : 'Object.defineProperties(values,descriptors)'};
      globalThis.${markerKey}.push('${runtimeCase.name}-length-'+values.length);
      values[${runtimeCase.index}](mark('${runtimeCase.name}-eval'));}`
    ),
    `process.stdout.write(JSON.stringify(globalThis.${markerKey}));delete globalThis.${markerKey};`,
...
test('descriptor preflight rejects non-callable accessors before plural and singular writes', () => {
  const pluralPaths = [
    ['direct', '', descriptors => `Object.defineProperties(values,${descriptors})`],
    [
      'helper',
      'const defineAll=(target,descriptors)=>Object.defineProperties(target,descriptors);',
      descriptors => `defineAll(values,${descriptors})`,
    ],
  ];
  for (const [caseIndex, preflightCase] of [1, 64, 65]
    .flatMap(index =>
      pluralPaths.flatMap(([pathName, prefix, invoke]) =>
        ['get', 'set'].flatMap(field =>
          ['invalid-first', 'length-first'].map(order => {
            const invalid = `x:{${field}:${field === 'get' ? 1 : '{}'}}`;
            const length = `length:{value:${index}}`;
            const descriptors = `{${order === 'invalid-first' ? `${invalid},${length}` : `${length},${invalid}`}}`;
            return {
              name: `${pathName} ${field} ${order} at index ${index}`,
              source: `${prefix}const values=Array(${index + 1}).fill(JSON.parse);values[${index}]=eval;
                try{${invoke(descriptors)}}catch{}values[${index}](configuredSource);`,
            };
          })
        )
      )
    ...
    .entries()) {
    const findings = scanSource(preflightCase.source, `packages/example/src/invalid-descriptor-${caseIndex}.ts`);
    assert.ok(
      findings.some(finding => finding.kind === 'direct-eval'),
      `${preflightCase.name} should abort plural preflight: ${JSON.stringify(findings)}`
    );
    assert.ok(findings.length <= 2, `${preflightCase.name} diagnostics should remain bounded`);
  }

  for (const [caseIndex, index] of [1, 64, 65].entries()) {
    const singularCases = [
      `const values=Array(${index + 1}).fill(JSON.parse);values[${index}]=eval;
        try{Object.defineProperty(values,'${index}',{get:1})}catch{}values[${index}](configuredSource);`,
      `const define=(target,key,descriptor)=>Reflect.defineProperty(target,key,descriptor);
        const values=Array(${index + 1}).fill(JSON.parse);values[${index}]=eval;
        try{define(values,'${index}',{set:{}})}catch{}values[${index}](configuredSource);`,
    ];
    for (const [singularIndex, source] of singularCases.entries()) {
      const findings = scanSource(
        source,
        `packages/example/src/invalid-singular-descriptor-${caseIndex}-${singularIndex}.ts`
      );
      assert.ok(
        findings.some(finding => finding.kind === 'direct-eval'),
        `invalid singular descriptor ${singularIndex} should preserve eval at index ${index}: ${JSON.stringify(findings)}`
      );
      assert.ok(findings.length <= 2, `invalid singular descriptor diagnostics should remain bounded`);
    }

    const validCases = [
      `const values=Array(${index + 1}).fill(JSON.parse);values[${index}]=eval;
        Object.defineProperties(values,{x:{get(){return JSON.parse},set:undefined},
          ${index}:{value:JSON.parse}});values[${index}]('{}');`,
      `const define=target=>Reflect.defineProperty(target,'${index}',{get(){return JSON.parse},set:undefined});
        const values=Array(${index + 1}).fill(JSON.parse);values[${index}]=eval;
        const succeeded=define(values);({true:JSON.parse,false:eval})[succeeded]('{}');`,
    ];
    for (const [controlIndex, source] of validCases.entries()) {
      assert.deepEqual(
        scanSource(source, `packages/example/src/safe-callable-descriptor-${caseIndex}-${controlIndex}.ts`),
        [],
        `valid callable descriptor control ${controlIndex} should stay clean at index ${index}`
      );
    }
  }
});

test('invalid accessor descriptor preflight agrees with the Node runtime', () => {
  const markerKey = '__a12_invalid_descriptor_preflight_marker__';
  const pluralCases = [1, 64, 65].flatMap(index =>
    ['direct', 'helper'].flatMap(pathName =>
      ['get', 'set'].flatMap(field =>
        ['invalid-first', 'length-first'].map(order => ({
          field,
          index,
          name: `${pathName}-${field}-${order}-${index}`,
          order,
          pathName,
        }))
      )
  ...
  const singularCases = [1, 64, 65].flatMap(index => [
    { index, mutation: `Object.defineProperty(values,'${index}',{get:1})`, name: `singular-object-${index}` },
    {
      index,
      mutation: `define(values,'${index}',{set:{}})`,
      name: `singular-helper-${index}`,
  ...
  const source = [
    `globalThis.${markerKey}=[];`,
    `const mark=name=>"globalThis.${markerKey}.push("+JSON.stringify(name)+")";`,
    ...pluralCases.map(runtimeCase => {
      const invalid = `x:{${runtimeCase.field}:${runtimeCase.field === 'get' ? 1 : '{}'}}`;
      const length = `length:{value:${runtimeCase.index}}`;
      const descriptors = `{${runtimeCase.order === 'invalid-first' ? `${invalid},${length}` : `${length},${invalid}`}}`;
      const mutation =
        runtimeCase.pathName === 'helper'
          ? `const defineAll=(target,descriptors)=>Object.defineProperties(target,descriptors);defineAll(values,${descriptors})`
          : `Object.defineProperties(values,${descriptors})`;
      return `{const values=Array(${runtimeCase.index + 1}).fill(JSON.parse);values[${runtimeCase.index}]=eval;
        let threw=false;try{${mutation}}catch{threw=true}
        globalThis.${markerKey}.push('${runtimeCase.name}-throw-'+threw);
        globalThis.${markerKey}.push('${runtimeCase.name}-length-'+values.length);
        values[${runtimeCase.index}](mark('${runtimeCase.name}-eval'));}`;
    }),
    ...singularCases.map(
      runtimeCase => `{${runtimeCase.prefix ?? ''}
      const values=Array(${runtimeCase.index + 1}).fill(JSON.parse);values[${runtimeCase.index}]=eval;
      let threw=false;try{${runtimeCase.mutation}}catch{threw=true}
      globalThis.${markerKey}.push('${runtimeCase.name}-throw-'+threw);
      values[${runtimeCase.index}](mark('${runtimeCase.name}-eval'));}`
    ),
  ...
  assert.deepEqual(
    ...
    pluralCases
      ...
      .concat(singularCases.flatMap(runtimeCase => [`${runtimeCase.name}-throw-true`, `${runtimeCase.name}-eval`]))
  );
});

test('safe accessor and enumerable bulk controls agree with the scanner and Node runtime', () => {
  const markerKey = '__a12_safe_descriptor_control_marker__';
  const cases = [1, 64, 65].flatMap(index =>
    ['direct', 'helper'].map(pathName => {
      const prefix =
        pathName === 'helper'
          ? `const define=(target,key,descriptor)=>Object.defineProperty(target,key,descriptor);
            ...
            const defineAll=(target,descriptors)=>Object.defineProperties(target,descriptors);
            const reflectDefine=(target,key,descriptor)=>Reflect.defineProperty(target,key,descriptor);`
          : '';
      const define =
        pathName === 'helper'
          ? `define(prototype,'${index}',{get:undefined})`
          : `Object.defineProperty(prototype,'${index}',{get:undefined})`;
      const assign = pathName === 'helper' ? 'assign(values,source)' : 'Object.assign(values,source)';
      const defineAll =
        pathName === 'helper' ? 'defineAll(described,descriptors)' : 'Object.defineProperties(described,descriptors)';
      const reflectDefine =
        pathName === 'helper'
          ? `reflectDefine(accessed,'${index}',{get(){return JSON.parse},set:undefined})`
          : `Reflect.defineProperty(accessed,'${index}',{get(){return JSON.parse},set:undefined})`;
      return {
        name: `${pathName}-${index}`,
        source: `${prefix}
          const prototype={};Object.defineProperty(prototype,'${index}',{set(_) {},configurable:true});
          ${define};const blocked=Object.create(prototype);Object.preventExtensions(blocked);
          const retained=Reflect.set(blocked,'${index}',JSON.parse);
          ({true:JSON.parse,false:eval})[retained]('{}');
          const values=Array(${index + 1}).fill(JSON.parse);values[${index}]=eval;const source={};
          Object.defineProperty(source,'${index}',{value:JSON.parse,enumerable:true});
          Object.defineProperty(source,'length',{value:${index},enumerable:false});
          ${assign};values[${index}]('{}');
          const described=Array(${index + 1}).fill(JSON.parse);described[${index}]=eval;
          const descriptors={};Object.defineProperty(descriptors,'${index}',{
            value:{value:JSON.parse},enumerable:true});
          Object.defineProperty(descriptors,'length',{value:{value:${index}},enumerable:false});
          ${defineAll};described[${index}]('{}');
          const accessed=Array(${index + 1}).fill(JSON.parse);accessed[${index}]=eval;
          const succeeded=${reflectDefine};
          ({true:JSON.parse,false:eval})[succeeded]('{}');`,
      };
    })
  );
  ...
  for (const runtimeCase of cases) {
    assert.deepEqual(
      scanSource(runtimeCase.source, `packages/example/src/safe-descriptor-runtime-${runtimeCase.name}.ts`),
      [],
      `${runtimeCase.name} should remain scanner-safe`
    );
  }
  ...
  const source = [
    `globalThis.${markerKey}=[];`,
    ...cases.map(
      runtimeCase => `{${runtimeCase.source}
        globalThis.${markerKey}.push('${runtimeCase.name}-safe');}`
    ),
    `process.stdout.write(JSON.stringify(globalThis.${markerKey}));delete globalThis.${markerKey};`,
  ].join('\n');
  const result = spawnSync(process.execPath, ['--eval', source], {
...
test('conditional delete and recreation exposes live enumerable keys to bulk operations', () => {
  const markerKey = '__a12_conditional_recreated_bulk_marker__';
  const modes = [
    {
      bulk: (pathName, target, source) =>
        pathName === 'helper' ? `bulk(${target},${source})` : `Object.assign(${target},${source})`,
    ...
    },
    {
      bulk: (pathName, target, source) =>
        pathName === 'helper' ? `bulk(${target},${source})` : `Object.defineProperties(${target},${source})`,
      descriptor: '{value:JSON.parse}',
      name: 'define-properties',
      replacement: '{value:eval}',
      helper: 'const bulk=(target,source)=>Object.defineProperties(target,source);',
    },
  ];
  const paths = [
    {
      name: 'direct',
      prefix: () => '',
      recreate: (key, replacement) => `try{source[${key}]=${replacement}}catch{}finally{void 0}`,
    },
    {
      name: 'helper',
      prefix: (key, replacement, mode) =>
        `function recreate(value){'use strict';try{value[${key}]=${replacement}}catch{}finally{void 0}}
         ${mode.helper}`,
      recreate: () => 'recreate(source)',
    },
    {
      name: 'reflect-helper',
      prefix: (key, replacement) =>
        `function recreate(value){try{return Reflect.set(value,${key},${replacement})}finally{void 0}}`,
      recreate: () => 'recreate(source)',
    },
  ];
  const cases = ['run', 1, 64, 65].flatMap(propertyName =>
    modes.flatMap(mode =>
      paths.map(pathCase => ({
        mode,
        name: `${mode.name}-${pathCase.name}-${propertyName}`,
        pathCase,
        propertyName,
      }))
    )
  ...
  const buildSource = (runtimeCase, flag, payload) => {
    const { mode, pathCase, propertyName } = runtimeCase;
    const key = JSON.stringify(propertyName);
    const target =
      typeof propertyName === 'number' ? `Array(${propertyName + 1}).fill(JSON.parse)` : '{run:JSON.parse}';
    return `${pathCase.prefix(key, mode.replacement, mode)}
      const source={};Object.defineProperty(source,${key},{value:${mode.descriptor},enumerable:false,
        writable:false,configurable:true});if(${flag})delete source[${key}];
      ${pathCase.recreate(key, mode.replacement)};const target=${target};
      ${mode.bulk(pathCase.name, 'target', 'source')};try{target[${key}](${payload})}catch{}`;
  };

  for (const runtimeCase of cases) {
    const findings = scanSource(
      buildSource(runtimeCase, 'globalThis.flag', 'configuredSource'),
      `packages/example/src/conditional-recreated-bulk-${runtimeCase.name}.ts`
    );
    assert.ok(
      findings.some(finding => finding.kind === 'direct-eval'),
      `${runtimeCase.name} should retain the enumerable recreation: ${JSON.stringify(findings)}`
    );
    assert.ok(findings.length <= 2, `${runtimeCase.name} diagnostics should remain bounded`);
  }

  const safeSources = [
    `const source={};Object.defineProperty(source,'run',{value:JSON.parse,enumerable:false,writable:false,
      configurable:true});try{source.run=eval}catch{}const target={run:JSON.parse};
      Object.assign(target,source);target.run('{}');`,
    `const source={};Object.defineProperty(source,'run',{value:eval,enumerable:false,writable:false,
      configurable:true});delete source.run;source.run=JSON.parse;const target={run:JSON.parse};
      Object.assign(target,source);target.run('{}');`,
    `const source={};Object.defineProperty(source,'run',{value:{value:JSON.parse},enumerable:false,
      writable:false,configurable:true});Reflect.set(source,'run',{value:eval});const target={run:JSON.parse};
      Object.defineProperties(target,source);target.run('{}');`,
    `const source={};Object.defineProperty(source,'run',{value:{value:eval},enumerable:false,writable:false,
      configurable:true});delete source.run;Reflect.set(source,'run',{value:JSON.parse});
      const target={run:JSON.parse};Object.defineProperties(target,source);target.run('{}');`,
  ];
  for (const [index, safeSource] of safeSources.entries()) {
    assert.deepEqual(
      scanSource(safeSource, `packages/example/src/safe-conditional-recreated-bulk-${index}.ts`),
      [],
      `conditional recreation control ${index} should stay clean`
    );
  }

  const runtimeBranches = cases.flatMap(runtimeCase =>
    [false, true].map(flag => {
      const name = `${runtimeCase.name}-${flag}`;
      const payload = JSON.stringify(`globalThis.${markerKey}.push('${name}-eval')`);
      return `{${buildSource(runtimeCase, flag, payload)}
        globalThis.${markerKey}.push('${name}-enumerable-'+
          Object.prototype.propertyIsEnumerable.call(source,${JSON.stringify(runtimeCase.propertyName)}));}`;
    })
  );
  const source = [
    `globalThis.${markerKey}=[];`,
  ...
  assert.deepEqual(
    ...
    cases.flatMap(runtimeCase => [
      `${runtimeCase.name}-false-enumerable-false`,
      `${runtimeCase.name}-true-eval`,
      `${runtimeCase.name}-true-enumerable-true`,
    ])
  );
});
...
test('inherited setters preserve getter provenance without inventing receiver data properties', () => {
  const markerKey = '__a12_inherited_setter_consumption_marker__';
  const paths = [
    {
      name: 'direct',
      write: key => `try{target[${key}]=JSON.parse}catch{}finally{void 0}`,
    },
    {
      name: 'helper',
      prefix: key => `function replace(value){'use strict';try{value[${key}]=JSON.parse}catch{}finally{void 0}}`,
      write: () => 'replace(target)',
    },
    {
      name: 'reflect',
      write: key => `try{Reflect.set(target,${key},JSON.parse)}catch{}finally{void 0}`,
    },
    {
      name: 'reflect-helper',
      prefix: key => `function replace(value){try{return Reflect.set(value,${key},JSON.parse)}finally{void 0}}`,
      write: () => 'try{replace(target)}catch{}',
    },
  ];
  const setterModes = [
    { definition: 'set(_){}', name: 'consumed' },
    { definition: `set(_){throw new Error('blocked')}`, name: 'throwing' },
  ];
  const cases = ['run', 1, 64, 65].flatMap(propertyName =>
    setterModes.flatMap(setterMode =>
      paths.map(pathCase => ({
        name: `${setterMode.name}-${pathCase.name}-${propertyName}`,
        pathCase,
        propertyName,
        setterMode,
      }))
  ...
  const buildSource = (runtimeCase, payload) => {
    const { pathCase, propertyName, setterMode } = runtimeCase;
    const key = JSON.stringify(propertyName);
    const prototype = typeof propertyName === 'number' ? '[]' : '{}';
    const target =
      typeof propertyName === 'number'
        ? `const target=Array(${propertyName});Object.setPrototypeOf(target,prototype);`
        : 'const target=Object.create(prototype);';
    return `${pathCase.prefix?.(key) ?? ''}const prototype=${prototype};
      Object.defineProperty(prototype,${key},{get(){return eval},${setterMode.definition}});
      ${target}${pathCase.write(key)};target[${key}](${payload});`;
  };

  for (const runtimeCase of cases) {
    const findings = scanSource(
      buildSource(runtimeCase, 'configuredSource'),
      `packages/example/src/inherited-setter-${runtimeCase.name}.ts`
    );
    assert.ok(
      findings.some(finding => finding.kind === 'direct-eval'),
      `${runtimeCase.name} should preserve the inherited getter: ${JSON.stringify(findings)}`
    );
    assert.ok(findings.length <= 2, `${runtimeCase.name} diagnostics should remain bounded`);
  }

  const conditionalCases = ['run', 1, 64, 65].flatMap(propertyName =>
    ['direct', 'reflect'].map(pathName => ({
      name: `conditional-${pathName}-${propertyName}`,
      pathName,
      propertyName,
    }))
  );
  const buildConditionalSource = (runtimeCase, flag, payload) => {
    const { pathName, propertyName } = runtimeCase;
    const key = JSON.stringify(propertyName);
    const prototype = typeof propertyName === 'number' ? '[]' : '{}';
    const target =
      typeof propertyName === 'number'
        ? `const target=Array(${propertyName});Object.setPrototypeOf(target,prototype);`
        : 'const target=Object.create(prototype);';
    const write = pathName === 'reflect' ? `Reflect.set(target,${key},JSON.parse)` : `target[${key}]=JSON.parse`;
    return `const unsafePrototype=${prototype};Object.defineProperty(unsafePrototype,${key},{
        get(){return eval},set(_){}});const safePrototype=${prototype};
      const prototype=${flag}?unsafePrototype:safePrototype;${target}
      ${write};try{target[${key}](${payload})}catch{}`;
  };
  for (const runtimeCase of conditionalCases) {
    const findings = scanSource(
      buildConditionalSource(runtimeCase, 'globalThis.flag', 'configuredSource'),
      `packages/example/src/conditional-inherited-setter-${runtimeCase.name}.ts`
    );
    assert.ok(
      findings.some(finding => finding.kind === 'direct-eval'),
      `${runtimeCase.name} should merge the setter-consumed branch: ${JSON.stringify(findings)}`
    );
    assert.ok(findings.length <= 2, `${runtimeCase.name} diagnostics should remain bounded`);
  }

  const safeSources = [
    `const prototype={};Object.defineProperty(prototype,'run',{get(){return JSON.parse},set(_){}});
      const target=Object.create(prototype);target.run=JSON.parse;target.run('{}');`,
    `const prototype={};Object.defineProperty(prototype,'run',{value:eval,writable:true});
      const target=Object.create(prototype);target.run=JSON.parse;target.run('{}');`,
    `const prototype={};Object.defineProperty(prototype,'65',{get(){return JSON.parse},set(_){}});
      const target=Object.create(prototype);
      Reflect.set(target,'65',JSON.parse);target[65]('{}');`,
  ];
  for (const [index, safeSource] of safeSources.entries()) {
    assert.deepEqual(
      scanSource(safeSource, `packages/example/src/safe-inherited-setter-${index}.ts`),
      [],
      `inherited setter control ${index} should stay clean`
    );
  }
  ...
  const source = [
    ...
    ...cases.map(runtimeCase => {
      const payload = JSON.stringify(`globalThis.${markerKey}.push('${runtimeCase.name}-eval')`);
      return `{${buildSource(runtimeCase, payload)}
        globalThis.${markerKey}.push('${runtimeCase.name}-own-'+
          Object.hasOwn(target,${JSON.stringify(runtimeCase.propertyName)}));}`;
    }),
    ...conditionalCases.flatMap(runtimeCase =>
      [false, true].map(flag => {
        const name = `${runtimeCase.name}-${flag}`;
        const payload = JSON.stringify(`globalThis.${markerKey}.push('${name}-eval')`);
        return `{${buildConditionalSource(runtimeCase, flag, payload)}
          globalThis.${markerKey}.push('${name}-own-'+
            Object.hasOwn(target,${JSON.stringify(runtimeCase.propertyName)}));}`;
      })
    ),
    `process.stdout.write(JSON.stringify(globalThis.${markerKey}));delete globalThis.${markerKey};`,
  ...
  assert.deepEqual(
    JSON.parse(result.stdout),
    cases
      .flatMap(runtimeCase => [`${runtimeCase.name}-eval`, `${runtimeCase.name}-own-false`])
      .concat(
        conditionalCases.flatMap(runtimeCase => [
          `${runtimeCase.name}-false-own-true`,
          `${runtimeCase.name}-true-eval`,
