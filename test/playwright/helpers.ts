...
export async function collectLocalAssetUrls(page: Page): Promise<string[]> {
  const assetUrls = await page.evaluate(() => {
    const linkedAssets = Array.from(
      document.querySelectorAll<HTMLLinkElement | HTMLScriptElement>('link[rel="stylesheet"][href], script[src]')
    )
      .map(element => {
...
export async function verifyAssets(
  ...
  route: SmokeRoute,
  assetUrls: string[],
  baseAssetIncludes: string[]
): Promise<void> {
  const requiredIncludes = [
    ...(route.includeBaseAssets === false ? [] : baseAssetIncludes),
    ...route.requiredAssetIncludes,
  ];
...
export async function assertSmokeRoute(page: Page, route: SmokeRoute, baseAssetIncludes: string[]): Promise<void> {
  ...
  for (const selector of route.fallbackSelectors) {
    await expect(page.locator(selector), `${route.path} fallback selector still present: ${selector}`).toHaveCount(0, {
      timeout: 20_000,
    });
  }
