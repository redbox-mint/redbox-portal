...
describe('ConfigModels', function () {
  ...
  it('should expose navigation config models with schema metadata', () => {
    ...
    expect(menuInfo.tsGlob).to.equal('internal/sails-ts/api/configmodels/MenuConfig.ts');

    expect(homePanelsInfo).to.not.be.undefined;
    expect(homePanelsInfo.schema).to.exist;
    expect(homePanelsInfo.tsGlob).to.equal('internal/sails-ts/api/configmodels/HomePanelConfig.ts');

    expect(adminSidebarInfo).to.not.be.undefined;
    expect(adminSidebarInfo.schema).to.exist;
    expect(adminSidebarInfo.tsGlob).to.equal('internal/sails-ts/api/configmodels/AdminSidebarConfig.ts');
  });
