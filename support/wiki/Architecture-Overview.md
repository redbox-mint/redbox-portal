...
|---|---|
| `api/` | Sails.js API definitions (Controllers, Models, Policies, Services) - **generated by the redbox-core loader**. |
| `config/` | Sails.js configuration files - **shims generated by the redbox-core loader, environment overrides added manually**. |
| `angular/` | Angular frontend application source code. |
| `packages/` | Local npm packages used by the application (e.g., `redbox-core`, `sails-ng-common`). |
...
| `redbox-dev-tools` | `@researchdatabox/redbox-dev-tools` | TypeScript development toolkit for creating ReDBox hooks. See [Redbox Hook Kit](redbox-dev-tools). |

## Pre-Lift Shim Generation

Before Sails.js lifts, the loader exported by `@researchdatabox/redbox-core` generates shim files to bridge Sails.js to the core package. This eliminates race conditions with hook loading order.
...
3. **Controllers**: `api/controllers` contains generated shims that route requests to controller implementations in `@researchdatabox/redbox-core` (or hook overrides).
4. **Services**: Business logic is encapsulated in core services (e.g., `RecordsService`, `UsersService`, `FormsService`). Services are loaded via shims from `@researchdatabox/redbox-core`.
5. **Persistence**: Data is stored in MongoDB using Waterline ORM models defined in `api/models`.

## Services Architecture

All business logic is centralized in the `@researchdatabox/redbox-core` package. Services follow a consistent pattern:
