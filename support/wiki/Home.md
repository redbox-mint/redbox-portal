...
- **[Application Authorization and Permission Model](https://github.com/redbox-mint/redbox-portal/wiki/Application-Authorization-and-Permission-Model)**: Agreed design for configurable roles, business-capability scopes, brand and record authorization, staged migration, and the deferred OAuth decision.
- **[Authorization Contract API](https://github.com/redbox-mint/redbox-portal/wiki/Authorization-Contract-API)**: Runtime-validated effective-principal, scope/template/role, and assignment administration routes, including pagination, transactional preview/apply, and Problem Details.
- **[Authorization Route-Scope Inventory](https://github.com/redbox-mint/redbox-portal/wiki/Authorization-Route-Scope-Inventory)**: Phase 0 compatibility baseline for explicit core route declarations, legacy mappings, runtime hook validation, and test ownership.
- **[Authorization Role Inventory](https://github.com/redbox-mint/redbox-portal/wiki/Authorization-Role-Inventory)**: Maintained role reader/writer/dependency inventory and legacy role AJAX compatibility contracts.
- **[Authorization Resource-Gate Inventory](https://github.com/redbox-mint/redbox-portal/wiki/Authorization-Resource-Gate-Inventory)**: Authoritative brand/entity/record service boundaries, cross-brand response rules, background contexts, and Phase 7 test owners.
- **[Authorization Administration](https://github.com/redbox-mint/redbox-portal/wiki/Authorization-Administration)**: Administrator guide to roles, assignments, protected roles, the scope catalog, audit, and navigation `requiredScope` configuration.
- **[Authorization Scope Catalog](https://github.com/redbox-mint/redbox-portal/wiki/Authorization-Scope-Catalog)**: Core scope families, grammar, lifecycle status, protected floors, and broad record scopes.
- **[Authorization Hook Contract](https://github.com/redbox-mint/redbox-portal/wiki/Authorization-Hook-Contract)**: Hook-author contract for declaring scopes and synchronizing claim-sourced assignments.
- **[Authorization Migration and Rollout](https://github.com/redbox-mint/redbox-portal/wiki/Authorization-Migration-and-Rollout)**: Additive migration, dual-write and drift rules, legacy/shadow/enforce modes, enforce prerequisites, and rollback.
- **[Legacy Bearer Token Migration](https://github.com/redbox-mint/redbox-portal/wiki/Legacy-Bearer-Token-Migration)**: Integrator guide to opaque legacy bearer behavior, effective-scope testing, and the compatibility window.
- **[Authorization Operations](https://github.com/redbox-mint/redbox-portal/wiki/Authorization-Operations)**: Operator tooling for readiness, orphan reconciliation, mismatch acknowledgement/retention, and system-administrator recovery.
