...
- `packages/redbox-core/src/authorization/legacy-authorization-baseline.snapshot.ts`
  is the frozen immutable per-route legacy baseline (78 `auth.rules` rows,
  317 configured route entries, 160 contract route IDs, with stable route ID,
  method, pattern, controller/action, scope/public/pre-auth declaration,
  authentication expectation, brand/resource ownership, matched `auth.rules`
  indexes, granting roles, `noRuleGrant`/`broadRuleMasking` flags, and
  representative `/default/rdmp/...` variants). It is generated by the
  maintained `npm --prefix packages/redbox-core run snapshot:legacy-baseline` command
  (`packages/redbox-core/scripts/regenerate-legacy-baseline-snapshot.ts`)
  and any drift or omission between live sources and this snapshot fails
  `legacy-authorization-baseline.test.ts`.
- `packages/redbox-core/src/authorization/legacy-authorization-baseline.ts`
  retains the live derivation builders plus `describeLegacyBaselineDrift()`
  used by the drift test; the frozen snapshot above is the golden evidence.
...
- `packages/redbox-core/test/authorization/legacy-authorization-baseline.test.ts`
  proves the live sources still match the frozen snapshot (any drift or
  omission fails with regeneration instructions), that route IDs are unique,
  that no-rule and broad-rule flags are consistent with the matched rules,
  that every contract route appears exactly once in the configured baseline,
  and that no row has an unexplained authorization state.
...
the 317-row configured map. Readiness reports both counts
(`routeCount` for contracts, `configuredRouteCount` for the full Sails table)
and requires `configuredRouteCount >= routeCount > 0`.
...
and the historical no-match result are owned by `PathRulesService.test.ts`,
the two-brand fixtures in
`packages/redbox-core/test/fixtures/authorization-legacy-matrix.fixtures.ts`,
and `authorization-legacy-matrix.test.ts`, which pins close to two hundred
expectations covering every distinct legacy rule template in both brands with
read and write operations, legacy roles, bearer parity, brand isolation, the
Guest baseline, and legacy-error cases. A routeId-keyed reconciliation test
extends that template coverage to every inventoried action: each frozen route
baseline entry with `authorizationKind === 'scope'` and a legacy PathRule
grant (286 entries) is reconciled as routeId × granting role × applicable
operation (GET→read, POST/PUT/PATCH/DELETE→write, methodless `*`→both) ×
brand, requiring a same-brand, same-operation matrix witness whose principal
resolves the granting role and whose path shares a frozen rule index with the
route. Public routes with no grant, pre-auth infrastructure routes, and
no-rule-grant scope routes (31 entries frozen in
`LEGACY_MATRIX_ROUTE_EXCLUSIONS`) are excluded from parity rows; the exclusion
list itself is pinned exact. The reviewed central scope map
deliberately splits broad legacy patterns into action-level business
capabilities. Per-route matches, granting roles, no-rule grants, broad-rule
masking, and representative variants are pinned per route in the frozen
`legacy-authorization-baseline.snapshot.ts`.
...
- `packages/redbox-core/test/fixtures/authorization-request.fixtures.ts` owns bounded principal,
...
Unsafe historical behavior is not a compatibility promise. The separately
recorded security deltas in
`packages/redbox-core/test/fixtures/authorization-security-differences.fixtures.ts`
are excluded from parity expectations and carry no repository approval
(`approval: external-required`): invalid supplied bearer credentials,
unclassified routes in enforce mode, cross-brand ID lookup, a missing legacy
path rule as an enforce-mode grant, explicit Guest assignment rejection, and
stale session-role authority. Product/security sign-off for each delta is an
