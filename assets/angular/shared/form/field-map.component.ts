...
import { icon, latLng, Map, marker, point, polyline, tileLayer, Draw, geoJSON, GeoJSON, FeatureGroup, featureGroup, map } from 'leaflet';
...
export class MapField extends FieldBase<any> {
  ...
  */
  ...
  defaultLeafletOptions = {
    zoom: 4,
    center: latLng([-24.673148, 134.074574])
  };
  ...
  registerMapEventHandlers(map: Map) {
    let that = this;
    map.on(Draw.Event.CREATED, function(e: any) {
      var type = e.layerType,
        layer = e.layer;
      that.layers.push(layer);
      that.layerGeoJSON = featureGroup(that.layers).toGeoJSON();
      that.layers = [];
  ...
  drawLayers() {
    ...
    let geoJSONLayer: GeoJSON = geoJSON(this.layerGeoJSON);
    this.layers = [];
    let that = this;
  ...
  importData() {
    if(this.importDataString.length > 0) {
      ...
      if(this.importDataString.indexOf("<") == 0) {
        ...
        let parsedLayers = geoJSON(JSON.parse(this.importDataString));
        let that = this;
        parsedLayers.eachLayer(layer => {
          layer.addTo(that.drawnItems);
          that.layers.push(layer);
