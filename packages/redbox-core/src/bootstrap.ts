import type { RecordDefinitionSeedServiceExports } from './services/RecordDefinitionSeedService';
/**
...
export async function coreBootstrap(): Promise<void> {
    ...
    const seedService = sails.services.recorddefinitionseedservice as object as RecordDefinitionSeedServiceExports;
    await seedService.seed(sails.config.recordDefinitionSeeds ?? { schemaVersion: 1, seeds: [] });
    sails.log.verbose('Record-definition create-only seeds processed.');
