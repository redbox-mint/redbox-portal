...
type RecordValidationDiscoveryService = Pick<RecordValidationServices.RecordValidation, 'discoverOperations'>;
...
export namespace Services {
  ...
  export class Forms extends services.Core.Service {
    ...
    public getFormByStartingWorkflowStep(branding: BrandingModel, recordType: string, _editMode: boolean): Observable<FormAttributes> {
      ...
      return RecordTypesService.get(branding, recordType).pipe(
        flatMap(recordTypeRecord => recordTypeRecord
          ? WorkflowStepsService.getFirst(recordTypeRecord).pipe(
              map(step => step as WorkflowStepLike | null)
            )
          : of(null)),
