...
export namespace Services {
  ...
  export class Users extends services.Core.Service {
    ...
    private redactAuditValue(value: unknown, contextKey?: string): unknown {
      ...
      if (_.isPlainObject(value)) {
        ...
        return _.reduce(
          ...
          (acc, nestedValue, nestedKey) => {
            acc[nestedKey] = this.shouldRedactAuditKey(nestedKey)
              ? '[REDACTED]'
              : this.redactAuditValue(nestedValue, nestedKey);
            return acc;
          },
    ...
    private sanitizeAuditDebugContext(
      action: string,
      parsedAdditionalContext: unknown,
      rawAdditionalContext: string | null,
      parseError: boolean
    ...
    private matchesSelectedUserForDirectAudit(selectedUser: AnyRecord, row: UserAuditRow): boolean {
      ...
      return (
        String(rowUser.id ?? '') === selectedUserId || _.toLower(String(rowUser.username ?? '')) === selectedUsername
      );
    }

    private matchesSelectedUserForAdminAudit(
      selectedUser: AnyRecord,
      action: string,
      parsedAdditionalContext: unknown
    ): boolean {
      ...
      if (action === 'link-accounts') {
        return (
          String(context.primaryUserId ?? '') === selectedUserId ||
          String(context.secondaryUserId ?? '') === selectedUserId
        );
      }
      return false;
    }
    ...
    private async fetchDirectUserAuditRows(selectedUser: AnyRecord): Promise<UserAuditRow[]> {
      ...
      try {
        ...
        throw new Error(
          `Unable to fetch direct audit records for user ${String(selectedUser.id ?? '')} without an unbounded fallback query.`
        );
      }
    }
    ...
    public async getUserAudit(userId: string): Promise<UserAuditResponse> {
      ...
      const mergedRows = _.values(
        _.keyBy([...matchingDirectRows, ...matchingAdminRows], (row: UserAuditRow) => String(row.id ?? ''))
      );
        ...
        new LocalStrategy(
          ...
          function (username: string, password: string, done: DoneCallback) {
            User.findOne({
              ...
              .populate('roles')
              .exec(function (err: unknown, foundUser: unknown) {
                if (err) {
                  return done(err);
                }
                if (!foundUser) {
                ...
                bcrypt.compare(password, passwordHash, function (err: unknown, res: boolean) {
                  ...
                  // Gate disabled users
                  that
                    .assertAuthenticationAllowed(foundUserObj as UserAttributes)
                    .then(() => {
    ...
    private async triggerPreSaveTriggers(
      user: AnyRecord,
      config: AnyRecord,
        ...
        for (let i = 0; i < preSaveUpdateHooks.length; i++) {
          ...
          if (preSaveUpdateHookFunctionString != null) {
            try {
              ...
              sails.log.verbose(
                `Triggering pre save triggers: ${preSaveUpdateHookFunctionString} failureMode ${failureMode}`
              );
              ...
              sails.log.debug(
                `pre-save sync trigger ${preSaveUpdateHookFunctionString} completed for user: ${_.get(user, 'username')}`
              );
            } catch (err) {
          ...
          if (postSaveSyncHooksFunctionString != null) {
            ...
            if (_.isFunction(postSaveSyncHookFunction)) {
              try {
                ...
                sails.log.debug(
                  `post-save sync trigger ${postSaveSyncHooksFunctionString} completed for user: ${_.get(user, 'username')}`
                );
              } catch (err) {
          ...
          if (postSaveCreateHookFunctionString != null) {
            ...
            if (_.isFunction(postSaveCreateHookFunction)) {
              ...
              this.resolveHookResponse(hookResponse)
                .then(_result => {
                  sails.log.debug(
                    `post-save trigger ${postSaveCreateHookFunctionString} completed for user: ${_.get(user, 'username')}`
                  );
                })
                .catch(error => {
    ...
    protected aafAuthInit = () => {
      this.registerSailsHook('on', 'ready', async () => {
        ...
        const that = this;
        sails.log.verbose(
          `AAF auth init for default brand "${defaultBrandName}" with active providers: ${JSON.stringify(defAuthConfig.active ?? [])}`
        );
        if (defAuthConfig.active != undefined && defAuthConfig.active.indexOf('aaf') != -1) {
          ...
          aafOpts.jsonWebTokenOptions = Object.assign(
            {},
            jwtDefaults,
            (configuredAafOpts.jsonWebTokenOptions ?? {}) as Record<string, unknown>
          );
          aafOpts.jwtFromRequest = ExtractJwt.fromBodyField('assertion');
          sails.log.verbose(
            `Registering Passport strategy "aaf-jwt" with option keys: ${JSON.stringify(Object.keys(aafOpts))}`
          );
          (sails.config.passport as PassportLike).use(
            'aaf-jwt',
            new JwtStrategy(aafOpts, function (req: Sails.Req, jwt_payload: AnyRecord, done: DoneCallback) {
              ...
              User.findOne(
                ...
                function (err: unknown, user: unknown) {
                  ...
                  sails.log.verbose(
                    `AAF user lookup result: ${user ? `found user id=${String((user as AnyRecord).id ?? 'unknown')}` : 'no user found'}`
                  );
                  sails.log.verbose(
                    `AAF verify error: ${err == null ? 'null' : err instanceof Error ? err.message : String(err)}`
                  );
                  if (err) {
                  ...
                  if (user) {
                    ...
                    // Gate disabled users
                    that
                      .assertAuthenticationAllowed(userObj)
                      .then(() => {
                        ...
                        userObj.edupersonscopedaffiliation = _.isNil(attrs.edupersonscopedaffiliation)
                          ? undefined
                          : String(attrs.edupersonscopedaffiliation);
                        userObj.edupersontargetedid = _.isNil(attrs.edupersontargetedid)
                          ? undefined
                          : String(attrs.edupersontargetedid);
                        userObj.edupersonprincipalname = _.isNil(attrs.edupersonprincipalname)
                          ? undefined
                          : String(attrs.edupersonprincipalname);
                            ...
                            if (success) {
                              User.update({
                                ...
                                .exec(function (err: unknown, user: unknown) {
                                  ...
                                  that
                                    .resolveLinkedUserCandidate(updatedUsers[0])
                                    .then(resolvedUser =>
                    ...
                    const normalizedEduPersonScopedAffiliation = _.isNil(attrs.edupersonscopedaffiliation)
                      ? undefined
                      : String(attrs.edupersonscopedaffiliation);
                    const normalizedEduPersonTargetedId = _.isNil(attrs.edupersontargetedid)
                      ? undefined
                      : String(attrs.edupersontargetedid);
                    const normalizedEduPersonPrincipalName = _.isNil(attrs.edupersonprincipalname)
                      ? undefined
                      : String(attrs.edupersonprincipalname);
                    ...
                    const normalizedName =
                      ...
                      (normalizedFullName.length > 0 ? normalizedFullName : undefined) ??
                      normalizedUsername;
                    ...
                    sails.log.verbose(
                      `AAF prepared user create request for brand "${brandName}" and username "${aafUsername}"`
                    );

                    const emailAuthorizedCheck = that.checkAuthorizedEmail(
                      String(userToCreate.email ?? ''),
                      brandName,
                      'aaf'
                    );
                    ...
                    if (that.hasPreSaveTriggerConfigured(configAAF, 'onCreate')) {
                      that
                        .triggerPreSaveTriggers(userToCreate, configAAF, 'onCreate')
                        .then((userAdditionalInfo: AnyRecord) => {
                          const success = that.checkAllTriggersSuccessOrFailure(userAdditionalInfo);
                          if (success) {
            ...
            while (!configured && discoverAttemptsCtr < oidcConfig.discoverAttemptsMax) {
              ...
              try {
                ...
                if (_.isString(oidcOpts.issuer)) {
                  ...
                  oidcClientConfig = new openIdClient.Configuration(oidcOpts.issuer, clientId, clientMetadata);
                ...
                const verifyCallbackFn = async (req: AnyRecord, tokenSet: AnyRecord, done: DoneCallback) => {
                  try {
                    ...
                    that.openIdConnectAuthVerifyCallback(
                      oidcConfig as unknown as AnyRecord,
                      serverMetadata as AnyRecord,
                      req,
                      tokenSet,
                      userinfo,
                      done
    ...
    protected openIdConnectAuthVerifyCallback(
      ...
      if (oidcConfig.debugMode === true) {
        sails.log.info(
          'OIDC debug mode is active, intentionally failing the login, and redirecting to failure page with all details of this login attempt.'
        );
        const err = {
          userinfo: userinfo,
          claims: tokenClaims,
        ...
        function (err: unknown, user: unknown) {
          ...
          if (user) {
            ...
            that
              ...
              .then(() => {
                ...
                if (that.hasPreSaveTriggerConfigured(oidcConfig, 'onUpdate')) {
                  that
                    .triggerPreSaveTriggers(userObj, oidcConfig as AnyRecord)
                    .then((userAdditionalInfo: AnyRecord) => {
                      const success = that.checkAllTriggersSuccessOrFailure(userAdditionalInfo);
                      if (success) {
                        User.update({
                          ...
                          .exec(function (err: unknown, user: unknown) {
                            ...
                            const updatedUsers = user as AnyRecord[];
                            sails.log.verbose(
                              `Done, returning updated user id=${String((updatedUsers[0] as AnyRecord)?.id ?? 'unknown')}`
                            );
                            that
                              .resolveLinkedUserCandidate(updatedUsers[0])
                              .then(resolvedUser => done(null, resolvedUser as AnyRecord))
            ...
            if (that.hasPreSaveTriggerConfigured(oidcConfig, 'onCreate')) {
              that
                .triggerPreSaveTriggers(userToCreate, oidcConfig as AnyRecord)
                .then((userAdditionalInfo: AnyRecord) => {
                  const success = that.checkAllTriggersSuccessOrFailure(userAdditionalInfo);
                  if (success) {
    ...
    protected bearerTokenAuthInit = () => {
      ...
      (sails.config.passport as PassportLike).use(
        'bearer',
        new BearerStrategy(function (token: string, done: DoneCallback) {
          if (!_.isEmpty(token) && !_.isUndefined(token)) {
            ...
            User.findOne({
              ...
              .exec(function (err: unknown, user: unknown) {
                ...
                // Gate disabled users
                that
                  .assertAuthenticationAllowed(user as UserAttributes)
                  .then(() => {
                    that
                      .resolveLinkedUserCandidate(user)
                      .then(resolvedUser =>
    ...
    protected initDefAdmin = (defRoles: AnyRecord[], defAdminRole: AnyRecord) => {
      ...
      if (defaultUser == null) {
        ...
        return super.getObservable<UserModel>(User.create(defaultUser)).pipe(
          flatMap(defUser => {
            ...
            return super.getObservable<Record<string, unknown>>(q, 'exec', 'simplecb').pipe(
              flatMap(_dUser => {
                return from(defRoles).pipe(
                  map(roleObserved => {
    ...
    public addLocalUser = (username: string, name: string, email: string, password: string): Observable<UserModel> => {
      ...
      return this.getUserWithUsername(username).pipe(
        flatMap(user => {
          if (user) {
            return throwError(new Error(`Username already exists`));
          } else {
            return this.findUsersWithEmail(email, null, null).pipe(
              flatMap(emailCheck => {
    ...
    */
    ...
    public getUserWithUsername = (username: string): Observable<UserModel | null> => {
      return this.getObservable<UserModel | null>(
        User.findOne({
          username: username,
        }).populate('roles')
      );
    ...
    public getEffectiveUser = (userOrId: unknown): Observable<UserModel | null> => {
      ...
      if (_.isString(userOrId) || _.isNumber(userOrId)) {
        return this.getUserWithId(String(userOrId)).pipe(
          flatMap(user => {
            if (user != null) {
              return this.getEffectiveUser(user);
            }
            return this.getUserWithUsername(String(userOrId)).pipe(
              flatMap(userByUsername => {
      ...
      return this.getUserWithId(String(userObj.linkedPrimaryUserId)).pipe(
        map(primaryUser => {
          if (primaryUser == null) {
    ...
    public searchLinkCandidates = (
      query: string,
      brandId: string,
      excludeUserId?: string
    ): Observable<LinkedUserSummary[]> => {
    ...
    public async findAndAssignAccessToRecords(pendingValue: string, userid: string): Promise<number> {
      ...
      for (const record of recordsArr) {
        ...
        const response = await RecordsService.provideUserAccessAndRemovePendingAccess(
          recordObj.redboxOid as string,
          effectiveUsername,
          pendingValue
        );
        if (!response.wasPersisted()) {
          throw new Error(`Pending record access mutation failed (request ${response.requestId}).`);
        }
        updated += 1;
      }
      return updated;
    ...
    public checkAuthorizedEmail(email: string, branding: string, authType: string): boolean {
      ...
      const authorizedDomainsEmails = _.get(
        brandingAwareData,
        'authorizedDomainsEmails',
        {}
      ) as AuthorizedDomainsEmails;
