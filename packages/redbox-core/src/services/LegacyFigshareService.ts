...
import { FigsharePublishingConfigData, resolveFigshareConnectionToken, type ValueBinding, type WriteBackBinding } from '../configmodels/FigsharePublishing';
...
type FigsharePublicationStatus = 'idle' | 'pending' | 'syncing' | 'awaiting_upload_completion' | 'published' | 'failed';

type FigshareSyncState = {
...
export namespace Services {
  ...
  export class LegacyFigshareService extends services.Core.Service {

    private datastreamService!: DatastreamService;
    private queueService!: QueueService;
    private handlebarsHelpersRegistered = false;
    ...
    private ensureHandlebarsHelpersRegistered(): void {
      if (!this.handlebarsHelpersRegistered) {
        registerSharedHandlebarsHelpers(Handlebars);
        this.handlebarsHelpersRegistered = true;
      }
    }

    private getEmbeddedV2Config(record?: RecordLike): FigsharePublishingConfigData | null {
      const brandName = String(_.get(record as AnyRecord, 'metaMetadata.branding', _.get(record as AnyRecord, 'branding', 'default')) || 'default');
      ...
      const figsharePublishing = _.get(brandConfig, 'figsharePublishing');
      if (_.isPlainObject(figsharePublishing) && figsharePublishing.enabled === true) {
    ...
    private getEmbeddedSyncState(config: FigsharePublishingConfigData, record: AnyRecord): FigshareSyncState {
      return (_.get(record, config.record.syncStatePath, { status: 'idle' }) || { status: 'idle' }) as FigshareSyncState;
    }

    private setEmbeddedSyncState(config: FigsharePublishingConfigData, record: AnyRecord, syncState: FigshareSyncState): void {
    ...
    private getSelectedDataLocations(config: FigsharePublishingConfigData, record: AnyRecord): AnyRecord[] {
    ...
    private validateEmbeddedHandlebarsTemplate(template: string): void {
    ...
    private async persistEmbeddedV2SyncRecord(oid: string, record: RecordLike, user: AnyRecord): Promise<void> {
      try {
    ...
    private async evaluateEmbeddedBinding(binding: ValueBinding | undefined, record: AnyRecord): Promise<unknown> {
      if (binding == null) {
        return undefined;
      }
      ...
      if (binding.kind === 'handlebars') {
        this.ensureHandlebarsHelpersRegistered();
        this.validateEmbeddedHandlebarsTemplate(binding.template);
    ...
    private async buildEmbeddedMetadataPayload(config: FigsharePublishingConfigData, record: AnyRecord): Promise<AnyRecord> {
      const payload: AnyRecord = {
        title: await this.evaluateEmbeddedBinding(config.metadata.title, record),
        description: await this.evaluateEmbeddedBinding(config.metadata.description, record),
        keywords: await this.evaluateEmbeddedBinding(config.metadata.keywords, record),
      ...
      const funding = await this.evaluateEmbeddedBinding(config.metadata.funding, record);
      if (!_.isNil(funding) && funding !== '') {
        payload.funding = funding;
      }

      const categorySource = await this.evaluateEmbeddedBinding(config.metadata.categories.source, record);
      ...
      if (!config.categories.allowUnmapped && sourceCodes.length > 0 && mappedCategories.length === 0) {
        throw new Error('No Figshare categories mapped for selected record categories');
      }

      const licenseValue = await this.evaluateEmbeddedBinding(config.metadata.license.source, record);
      ...
      if (config.metadata.relatedResource) {
        payload.related_materials = [{
          title: await this.evaluateEmbeddedBinding(config.metadata.relatedResource.title, record),
          identifier: await this.evaluateEmbeddedBinding(config.metadata.relatedResource.doi, record)
        }];
      }
      ...
      if (config.metadata.customFields.length > 0) {
        ...
        for (const customField of config.metadata.customFields) {
          customFields[customField.figshareField] = await this.evaluateEmbeddedBinding(customField.value, record);
        }
    ...
    private getWriteBackUrls(config: FigsharePublishingConfigData, articleId: string): string[] {
    ...
    public preparePublication(record: RecordLike, jobId?: string): FigsharePublicationPlan {
      const recordObj = record as AnyRecord;
      const config = this.getEmbeddedV2Config(record);
      if (config == null) {
        return { action: 'skip', sameJob: false, syncState: { status: 'idle' } };
      }

      const existingState = this.getEmbeddedSyncState(config, recordObj);
      const correlationId = jobId || `${_.get(recordObj, 'oid', 'record')}-${Date.now()}`;
      ...
      if (!sameJob && (existingState.status === 'syncing' || existingState.status === 'awaiting_upload_completion')) {
        sails.log.warn(`FigService v2 - skipping duplicate sync for record ${_.get(recordObj, 'oid', '')}`);
        return { action: 'skip', articleId: _.get(recordObj, config.record.articleIdPath), sameJob: false, syncState: existingState };
      }

      const existingArticleId = _.get(recordObj, config.record.articleIdPath);
      ...
      this.setEmbeddedSyncState(config, recordObj, syncState);
      return {
        action,
    ...
    public async syncMetadata(record: RecordLike, plan?: FigsharePublicationPlan): Promise<AnyRecord> {
      ...
      const metadataPayload = await this.buildEmbeddedMetadataPayload(config, recordObj);
      if (config.testing.mode === 'fixture') {
        return {
          id: publicationPlan.articleId ?? _.get(config.testing.fixtures, 'article.id', 'fixture-article-id'),
          ...(_.get(config.testing.fixtures, 'article', {}) as AnyRecord),
          ...metadataPayload
        };
      }

      const timeoutMs = config.connection.operationTimeouts.metadataMs || config.connection.timeoutMs;
      const axiosConfig = {
        method: publicationPlan.action === 'create' ? 'post' : 'put',
        url: `${config.connection.baseUrl.replace(/\/+$/, '')}/account/articles${publicationPlan.action === 'create' ? '' : `/${publicationPlan.articleId}`}`,
        headers: {
    ...
    public async syncAssets(record: RecordLike, article: AnyRecord): Promise<AnyRecord> {
      ...
      const selectedDataLocations = this.getSelectedDataLocations(config, recordObj);
      const attachmentCount = selectedDataLocations.filter((entry) => entry.type === 'attachment').length;
      const urlCount = selectedDataLocations.filter((entry) => entry.type === 'url').length;
      const syncState = this.getEmbeddedSyncState(config, recordObj);
    ...
    public async syncEmbargo(record: RecordLike, articleId: string): Promise<AnyRecord> {
      const recordObj = record as AnyRecord;
      const config = this.getEmbeddedV2Config(record);
      if (config == null || config.embargo.mode !== 'recordDriven') {
        return {};
      }

      const embargoPayload = {
        access_type: await this.evaluateEmbeddedBinding(config.embargo.accessRights.accessRights, recordObj),
        embargo_date: await this.evaluateEmbeddedBinding(config.embargo.accessRights.fullEmbargoUntil, recordObj),
        embargo_reason: await this.evaluateEmbeddedBinding(config.embargo.accessRights.reason, recordObj)
      };

      if (config.testing.mode === 'fixture') {
        return embargoPayload;
      }

      if (!config.embargo.forceSync && _.every(Object.values(embargoPayload), (value: unknown) => _.isNil(value) || value === '')) {
        return {};
      }

      const response = await axios({
        method: 'put',
        url: `${config.connection.baseUrl.replace(/\/+$/, '')}/account/articles/${articleId}/embargo`,
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `token ${config.connection.token}`
        },
        timeout: config.connection.operationTimeouts.metadataMs || config.connection.timeoutMs,
        data: embargoPayload
      });
      return response.data as AnyRecord;
    }
    ...
    public async publishIfNeeded(record: RecordLike, articleId: string): Promise<AnyRecord> {
      const recordObj = record as AnyRecord;
      const config = this.getEmbeddedV2Config(record);
      if (config == null || config.article.publishMode === 'manual') {
        return {};
      }
      ...
      if (config.testing.mode === 'fixture') {
        return _.get(config.testing.fixtures, 'publishResult', { id: articleId, status: 'published' });
      }

      const response = await axios({
        method: 'post',
        url: `${config.connection.baseUrl.replace(/\/+$/, '')}/account/articles/${articleId}/publish`,
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `token ${config.connection.token}`
        },
        timeout: config.connection.operationTimeouts.publishMs || config.connection.timeoutMs,
        data: {}
      });
      return response.data as AnyRecord;
    }
    ...
    public writeBack(record: RecordLike, article: AnyRecord, publishResult?: AnyRecord, assetSyncResult?: AnyRecord): RecordLike {
      ...
      const articleUrls = this.getWriteBackUrls(config, articleId);
    ...
    public async syncRecordWithFigshareV2(record: RecordLike, jobId?: string): Promise<RecordLike> {
      const recordObj = record as AnyRecord;
      const config = this.getEmbeddedV2Config(record);
      if (config == null) {
        return record;
      ...
      try {
        const article = await this.syncMetadata(recordObj, plan);
    ...
    protected async performRequest(axiosConfig: AxiosConfigLike): Promise<AxiosResponseLike> {
      return axios(axiosConfig) as Promise<AxiosResponseLike>;
    }
    ...
    private async requestWithRetry(config: FigshareRuntimeConfig, axiosConfig: AxiosConfigLike, options: FigshareRequestOptions = {}): Promise<AxiosResponseLike> {
      ...
      for (let attempt = 1; attempt <= retryConfig.maxAttempts; attempt++) {
        try {
          ...
          const response = await this.performRequest(axiosConfig);
    ...
    public createUpdateFigshareArticle(oid: string, record: RecordLike, options: AnyRecord, user: AnyRecord) {
      if (this.getEmbeddedV2Config(record) != null) {
        return this.syncRecordWithFigshareV2(record, `${oid}:pre`);
      }
      const config = this.getRuntimeConfig();
    ...
    public uploadFilesToFigshareArticle(oid: string, record: RecordLike, options: AnyRecord, user: AnyRecord) {
      if (this.getEmbeddedV2Config(record) != null) {
        void this.syncRecordWithFigshareV2(record, `${oid}:post`)
          .then(async (updatedRecord: RecordLike) => {
            await this.persistEmbeddedV2SyncRecord(oid, updatedRecord, user);
          })
          .catch(async (error: unknown) => {
            await this.persistEmbeddedV2SyncRecord(oid, record, user);
...
declare global {
  let LegacyFigshareService: Services.LegacyFigshareService;
}
