...
export function projectOutputDependencyContext(
  ...
  const preflight = boundedValidationPreflight(fields, {
    maxBytes: EXPRESSION_RUNTIME_LIMITS.maxInputBytes,
    maxDepth: 1,
    maxStringLength: EXPRESSION_RUNTIME_LIMITS.maxPropertyNameLength,
    maxPropertyNameLength: EXPRESSION_RUNTIME_LIMITS.maxPropertyNameLength,
    maxWork: EXPRESSION_RUNTIME_LIMITS.maxValidationWork,
    arrayCardinalityLimit: () => EXPRESSION_RUNTIME_LIMITS.maxObjectProperties,
    objectCardinalityLimit: () => 0,
  });
  if (!preflight.ok) {
    if (preflight.reason === 'cardinality') {
  ...
  for (let index = 0; index < fields.length; index += 1) {
    const field: string = Object.getOwnPropertyDescriptor(fields, String(index))?.value;
    ...
    const value: ActionJsonValue | undefined = Object.getOwnPropertyDescriptor(prior.output.fields, field)?.value;
    if (value !== undefined) {
