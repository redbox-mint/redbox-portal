...
const MANAGED_JSONATA_FUNCTIONS = Object.freeze({
  guessNameParts: Object.freeze({ implementation: guessNameParts, signature: '<(sl):o>' }),
  luxonFormatDate: Object.freeze({ implementation: luxonFormatDate, signature: '<(snlo)(sl)(sl)?:s>' }),
});

export const MANAGED_JSONATA_CUSTOM_FUNCTION_NAMES: readonly string[] = Object.freeze(
  Object.keys(MANAGED_JSONATA_FUNCTIONS)
);

/** Installs only the fixed pure shared implementations; accepts no extension registry. */
export function registerManagedJsonataFunctions(expression: jsonata.Expression): void {
  for (const [name, definition] of Object.entries(MANAGED_JSONATA_FUNCTIONS)) {
    expression.registerFunction(name, definition.implementation, definition.signature);
  }
}
...
const MANAGED_HANDLEBARS_HELPERS = Object.freeze({
...
/** Removes all ambient helpers before installing the fixed managed registry. */
export function registerManagedHandlebarsHelpers(instance: typeof Handlebars): void {
  for (const name of Object.keys(instance.helpers)) {
    if (!MANAGED_HANDLEBARS_BLOCK_HELPER_NAMES.includes(name)) instance.unregisterHelper(name);
  }
  for (const [name, helper] of Object.entries(MANAGED_HANDLEBARS_HELPERS)) {
    const callable = runtimeFunction(helper);
    if (callable === undefined) throw new ManagedExpressionError('handlebars', 'worker', 'handlebars-helper-invalid');
    instance.registerHelper(name, (...argumentsList: RuntimeValue[]): RuntimeValue => {
      // Handlebars appends privileged runtime options even when required
      // arguments are missing. Never pass those options into pure helpers.
      argumentsList.pop();
      if (name === 'and' || name === 'or' || name === 'concat') argumentsList.push(null);
      return callable.invoke(...argumentsList);
    });
  }
}
...
function jsonataBindingIsForbidden(value: string): boolean {
  return FORBIDDEN_JSONATA_BINDINGS.has(value.replace(/^\$/, '')) || isForbiddenExpressionContextKey(value);
}
...
export function compileManagedJsonataExpression(source: string): PreparedJsonataExpression {
  if (typeof source !== 'string' || source.length > EXPRESSION_RUNTIME_LIMITS.maxExpressionLength) {
...
export function compileManagedHandlebarsTemplate(
  ...
  if (typeof source !== 'string' || source.length > EXPRESSION_RUNTIME_LIMITS.maxTemplateLength) {
