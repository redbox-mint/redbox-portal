...
export async function generateMigrationConfigShim(
    ...
    const migrationProviders = [
        ...hookMigrations.map(hook => `...${sanitizePackageNameForVar(hook.name, 'migrations')}`),
        ...localMigrationFiles.map((_, index) => `appMigration_${index}`),
    ];

    const content = `'use strict';\n/**\n * Migration config shim\n * Auto-generated by @researchdatabox/redbox-core loader\n * Do not edit manually - regenerated when .regenerate-shims marker exists\n */\n${hookImports ? `${hookImports}\n` : ''}${localImports ? `${localImports}\n` : ''}\nconst migrations = [${migrationProviders.join(', ')}].sort((a, b) => (a.name < b.name ? -1 : a.name > b.name ? 1 : 0));\nconst seenMigrationNames = new Set();\nfor (const migration of migrations) {\n  if (!migration || typeof migration.name !== 'string' || typeof migration.up !== 'function') {\n    throw new Error('Invalid Redbox migration export. Each migration must include name and up().');\n  }\n  if (seenMigrationNames.has(migration.name)) {\n    const message = 'Duplicate Redbox migration name: ' + migration.name;\n    if (process.env.NODE_ENV === 'production') {\n      throw new Error(message);\n    }\n    console.warn('[redbox-loader:warn]', message);\n  }\n  seenMigrationNames.add(migration.name);\n}\n\nconst exportedMigrations = [];\nconst exportedMigrationNames = new Set();\nfor (const migration of migrations) {\n  if (!exportedMigrationNames.has(migration.name)) {\n    exportedMigrations.push(migration);\n    exportedMigrationNames.add(migration.name);\n  }\n}\n\nmodule.exports.migrations = exportedMigrations;\n`;
