...
export interface NamedConfigArrayMergeRule {
    key: string;
}

export const NAMED_CONFIG_ARRAY_PATHS: Record<string, NamedConfigArrayMergeRule> = {
    'agendaQueue.jobs': { key: 'name' }
};
...
function getMergeKeyValue(item: unknown, key: string): string | number | undefined {
...
export function mergeNamedArrayByKey(baseArray: unknown[], sourceArray: unknown[], key: string, pathName: string): unknown[] {
    const _ = require('lodash') as typeof import('lodash');
    const merged = _.cloneDeep(baseArray);
    const indexByKey = new Map<string | number, number>();

    merged.forEach((item, index) => {
        const keyValue = getMergeKeyValue(item, key);
        if (keyValue !== undefined && !indexByKey.has(keyValue)) {
            indexByKey.set(keyValue, index);
        }
    });

    sourceArray.forEach(sourceItem => {
        const keyValue = getMergeKeyValue(sourceItem, key);
        if (keyValue === undefined || !indexByKey.has(keyValue)) {
            const nextIndex = merged.length;
            merged.push(_.cloneDeep(sourceItem));
            if (keyValue !== undefined) {
                indexByKey.set(keyValue, nextIndex);
            }
            return;
        }

        const existingIndex = indexByKey.get(keyValue) as number;
        merged[existingIndex] = mergeRedboxConfigValue(pathName, merged[existingIndex], sourceItem);
    });

    return merged;
}

export function mergeRedboxConfigValue(pathName: string, objValue: unknown, srcValue: unknown): unknown {
    const _ = require('lodash') as typeof import('lodash');
    const namedArrayRule = NAMED_CONFIG_ARRAY_PATHS[pathName];

    if (namedArrayRule && Array.isArray(objValue) && Array.isArray(srcValue)) {
        return mergeNamedArrayByKey(objValue, srcValue, namedArrayRule.key, pathName);
    }

    if (Array.isArray(objValue) && Array.isArray(srcValue)) {
        return _.merge(_.cloneDeep(objValue), srcValue);
    }

    if (isObjectLike(objValue) && isObjectLike(srcValue)) {
        const merged: Record<string, unknown> = _.cloneDeep(objValue);
        for (const [key, value] of Object.entries(srcValue)) {
            const nextPath = pathName ? `${pathName}.${key}` : key;
            merged[key] = mergeRedboxConfigValue(nextPath, merged[key], value);
        }
        return merged;
    }
...
export function mergeRedboxConfig(...configs: Array<Record<string, unknown> | undefined>): Record<string, unknown> {
    return configs.reduce<Record<string, unknown>>((merged, config) => {
        if (config === undefined) {
            return merged;
        }
        return mergeRedboxConfigValue('', merged, config) as Record<string, unknown>;
    }, {});
}
    ...
    await Promise.all(
        configNames.map(async name => {
            ...
            const mergeStatement =
                hookMerges.length > 0 ? `Loader.mergeRedboxConfig({ ${name}: Config.${name} || {} }, ${hookMerges.join(', ')}).${name}` : `Config.${name}`;

            const content = `'use strict';\n/**\n * ${name} config shim\n * Auto-generated by @researchdatabox/redbox-core loader\n * Do not edit manually - regenerated when .regenerate-shims marker exists\n *\n * Merges: core config + hook configs (alphabetical order)\n * Debug view: see support/debug-config/resolved.js\n */\nconst { Config, Loader } = require('@researchdatabox/redbox-core');\n${hookImports}\n\nmodule.exports.${name} = ${mergeStatement};\n`;
            const written = await writeFileIfChanged(filePath, content);
            if (written) {
                generated++;
            }
...
export async function generatePreLiftSnapshot(appPath: string, hookConfigs: HookConfigRegistration[]): Promise<void> {
    ...
    let mergedConfig = mergeRedboxConfig(Config);
    ...
    for (const hook of hookConfigs) {
        try {
            ...
            if (typeof hookModule.registerRedboxConfig === 'function') {
                ...
                mergedConfig = mergeRedboxConfig(mergedConfig, hookConfig);
            }
        } catch (err) {
            const message = err instanceof Error ? err.message : String(err);
