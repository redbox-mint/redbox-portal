...
export async function generateConfigShims(
    ...
    await Promise.all(
        configNames.map(async name => {
            ...
            // Use mergeWith + array concat customizer rather than plain `_.merge`. Plain
            // merge does index-based merging on arrays, which mangles entries when two
            // hooks each contribute a few items to the same array (e.g. each contributes
            // its own `agendaQueue.jobs` entry — index 0 of the hook config gets merged
            // into index 0 of core's, silently losing or relabelling core jobs). Concat
            // preserves all entries and matches the legacy `ConfigService.mergeHookConfig`
            // behaviour. Hooks that want override semantics should use `_dontMerge` /
            // `_delete` markers via the legacy merge path.
            const mergeStatement =
                hookMerges.length > 0
                    ? `_.mergeWith({}, Config.${name} || {}, ${hookMerges.join(', ')}, concatArrays)`
                    : `Config.${name}`;

            const content = `'use strict';\n/**\n * ${name} config shim\n * Auto-generated by @researchdatabox/redbox-core loader\n * Do not edit manually - regenerated when .regenerate-shims marker exists\n *\n * Merges: core config + hook configs (alphabetical order) using mergeWith +\n * array concat (so each hook's array contributions accumulate rather than\n * being mangled by lodash's default index-based array merge).\n * Debug view: see support/debug-config/resolved.js\n */\nconst _ = require('lodash');\nconst { Config } = require('@researchdatabox/redbox-core');\n${hookImports}\n\nfunction concatArrays(objValue, srcValue) {\n  if (Array.isArray(objValue) && Array.isArray(srcValue)) {\n    return objValue.concat(srcValue);\n  }\n  return undefined;\n}\n\nmodule.exports.${name} = ${mergeStatement};\n`;
            const written = await writeFileIfChanged(filePath, content);
            if (written) {
                generated++;
            }
...
export async function generatePreLiftSnapshot(appPath: string, hookConfigs: HookConfigRegistration[]): Promise<void> {
    ...
    // Match the runtime config-shim merge behaviour (array concat) so the snapshot
    // reflects what Sails actually sees. See `generateConfigShims` for context.
    ...
    for (const hook of hookConfigs) {
        try {
            ...
            if (typeof hookModule.registerRedboxConfig === 'function') {
                ...
                mergedConfig = _.mergeWith(mergedConfig, hookConfig, concatArrays);
            }
        } catch (err) {
            const message = err instanceof Error ? err.message : String(err);
