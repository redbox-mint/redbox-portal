...
export async function generateMigrationConfigShim(
  ...
  const migrationSourceEntries = [
    `  { source: 'core:record-definitions-v1', migrations: require('@researchdatabox/redbox-core').registerRedboxMigrations() },`,
    ...hookMigrations.map(
...
export async function shouldRegenerateShims(appPath: string, forceRegenerate = false): Promise<RegenerationDecision> {
  ...
  const migrationConfigPath = path.join(appPath, 'config', 'migrations.js');
  if (
    !fsSync.existsSync(migrationConfigPath) ||
    !(await fs.readFile(migrationConfigPath, 'utf8')).includes("source: 'core:record-definitions-v1'")
  ) {
    return {
      shouldRegenerate: true,
      reason: 'core record-definition migration registration is missing',
      deleteMarker: false,
    };
  }
