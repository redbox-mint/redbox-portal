  ...
  export class User extends controllers.Core.Controller {
    ...
    public logout(req: Sails.Req, res: Sails.Res) {
      ...
      if (_.isEmpty(redirUrl)) {
        redirUrl = _.isEmpty(sails.config.auth.postLogoutRedir)
          ? `${BrandingService.getBrandAndPortalPath(req)}/home`
          : sails.config.auth.postLogoutRedir;
      }
      ...
      req.logout(function (err: unknown) {
        ...
        UsersService.addUserAuditEvent(user, 'logout', requestDetails)
          .then(_response => {
    ...
    public localLogin(req: Sails.Req, res: Sails.Res) {
      const that = this;
      const passport = sails.config.passport as unknown as {
        authenticate: (
          strategy: string,
          callback: (err: Error | null, user: AnyRecord | false, info: AnyRecord) => void
          ...
          if (isAjax) {
            return that.sendResp(req, res, {
              data: {
                user: user,
                message: 'Login OK',
                url: (sails.getActions()['user/getpostloginurl'] as (req: Sails.Req, res: Sails.Res) => string)(
                  req,
                  res
                ),
              },
    ...
    public openidConnectLogin(req: Sails.Req, res: Sails.Res) {
      ...
      passport.authenticate(
        passportIdentifier,
        function (err: Error | null, user: AnyRecord | false, info: AnyRecord | string) {
          ...
          if (!_.isEmpty(err) || _.isUndefined(user) || _.isEmpty(user) || user == false) {
            ...
            sails.log.verbose(
              'After decodeErrorMappings - errorMessageDecoded: ' + JSON.stringify(errorMessageDecoded)
            );
          ...
          UsersService.addUserAuditEvent(user, 'login', requestDetails)
            .then(_response => {
              sails.log.debug(
                `User login audit event created for OIDC login: ${_.isEmpty(user) ? '' : (user as AnyRecord).id}`
              );
            })
            .catch(err => {
      ...
      if (!_.isUndefined(errorMessage) && !_.isNull(errorMessage)) {
        ...
        for (const errorMappingDetails of errorMappingList) {
          ...
          if (!_.isUndefined(regexPattern) && _.isRegExp(regexPattern)) {
            ...
            !_.isUndefined(regexPattern) &&
            !_.isRegExp(regexPattern) &&
            _.isString(regexPattern) &&
            !_.isEmpty(regexPattern)
          ) {
            matchRegex = false;
            matchString = true;
          } else {
            ...
            if (this.validateRegex(errorMessage, regexPattern)) {
              if (asObject) {
                ...
                if (!_.isEmpty(matchRegexGroupsDecoded)) {
                  sails.log.verbose(
                    'decodeErrorMappings - interpolationObj ' + JSON.stringify(matchRegexGroupsDecoded)
                  );
    ...
    public find(req: Sails.Req, res: Sails.Res) {
      ...
      UsersService.findUsersWithName(searchName, brand.id, searchSource).subscribe(
        users => {
