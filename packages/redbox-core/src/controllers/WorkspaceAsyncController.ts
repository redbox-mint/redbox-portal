...
export namespace Controllers {
  ...
  export class WorkspaceAsync extends controllers.Core.Controller {
     ...
     */
    protected override _exportedMethods: string[] = ['start', 'loop', 'status'];
    ...
    public start(req: Sails.Req, res: Sails.Res) {
      ...
      return WorkspaceAsyncService.start({
        name,
        recordType,
        username,
        service,
        method,
        args,
        actorId: authorization.context.principal.userId,
        operationId: ensureAuthorizationRequestId(req),
        branding: authorization.context.brand?.id,
    ...
    public status(req: Sails.Req, res: Sails.Res) {
      ...
      const recordType = req.param('recordType');
      const authorization = requireRequestResourceAuthorization(req);
      return WorkspaceAsyncService.status({ status, recordType, branding: authorization.context.brand?.id }).subscribe(
        (response: unknown) => {
