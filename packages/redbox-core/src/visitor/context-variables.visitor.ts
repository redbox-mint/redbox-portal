...
export class ContextVariablesFormConfigVisitor extends FormConfigVisitor {
  ...
  public async applyContextVariables(
  ...
  override async visitRelatedObjectDataFormComponentDefinition(
    item: import('@researchdatabox/sails-ng-common').RelatedObjectDataFormComponentDefinitionOutline
  ): Promise<void> {
    await item.component.accept(this);
  }
  ...
  override async visitRelatedObjectDataFieldComponentDefinition(
    ...
    if (typeof item.config.template === 'string')
      item.config.template = String(this.replaceTokens(item.config.template));
    if (typeof item.config.content === 'string') item.config.content = this.replaceTokens(item.config.content);
    if (typeof item.config.dataPath === 'string')
      item.config.dataPath = String(this.replaceTokens(item.config.dataPath));
