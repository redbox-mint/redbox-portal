...
export class ClientFormConfigVisitor extends FormConfigVisitor {
  ...
  protected applyPostPruningTransformsToNestedChildrenWithoutElementTemplate(
    item: AvailableFormComponentDefinitionOutlines
  ): void {
    const config = item?.component?.config as Record<string, unknown> | undefined;
    if (!config) {
    ...
    if (config.elementTemplate) {
      ...
      const etViewOverride = (
        et as { overrides?: { formModeClasses?: Record<string, { component?: string; template?: string }> } }
      )?.overrides?.formModeClasses?.view;
      const etClassName = et?.component?.class;
      const etIsIdentity = etViewOverride?.component === etClassName;
      const etHasExplicitTransform =
        !etIsIdentity &&
