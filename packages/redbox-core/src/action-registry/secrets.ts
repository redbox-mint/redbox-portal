...
const issuedErrorCodes = new WeakMap<object, ActionSecretProviderErrorCode>();
...
export class ActionSecretProviderError extends Error {
  ...
  constructor(code: ActionSecretProviderErrorCode) {
    ...
    this.code = safeCode;
    issuedErrorCodes.set(this, safeCode);
  }
...
export interface ActionSecretStorage {
  authorizeWrite?(slot: ActionSecretSlotIdentity): Promise<void>;
  /** Persistent providers additionally verify the current active binding. */
  authorizeHandler?(request: ActionSecretHandlerResolutionRequest): Promise<void>;
...
export function validSecretValue(value: RuntimeValue): value is string {
  return (
    typeof value === 'string' &&
    value.trim().length > 0 &&
...
class ProtectedActionSecretProvider implements ActionSecretProvider {
  ...
  async write(request: ActionSecretWriteRequest): Promise<ActionSecretWriteResult> {
    try {
      ...
      if (typeof value === 'string' && Buffer.byteLength(value, 'utf8') > ACTION_SECRET_LIMITS.maxSecretBytes) {
        throw providerError('invalid-secret-value');
      }
      if (value === undefined || (typeof value === 'string' && value.trim().length === 0)) {
        if (this.#storage.authorizeWrite) {
          await storageOperation(() => this.#storage.authorizeWrite!(access.slot));
        }
        return 'retained';
      }
      ...
      throw providerError(
        typeof error === 'object' && error !== null
          ? (issuedErrorCodes.get(error) ?? 'secret-provider-failure')
          : 'secret-provider-failure'
      );
    }
  }
  ...
  async resolveForHandler(request: ActionSecretHandlerResolutionRequest): Promise<ResolvedActionSecret | undefined> {
    try {
      ...
      const declaration = handlerSecretDeclaration(
        { ...access, resolvedBinding },
        access.slot,
        actionSecretProviderExecutionStates.get(this)?.trustedBindingRecordTypeKeys.get(resolvedBinding)
      );
      if (this.#storage.authorizeHandler) {
        await storageOperation(() => this.#storage.authorizeHandler!({ ...access, resolvedBinding }));
      }
      ...
      if (value === undefined) {
        throw providerError(
          declaration.required ? 'required-secret-not-configured' : 'secret-configured-state-mismatch'
        );
      }
      if (!validSecretValue(value)) {
        throw providerError('secret-provider-failure');
