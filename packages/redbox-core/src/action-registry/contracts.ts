...
function preflightActionContract(value: RuntimeValue, allowHandler = false): BoundedValidationFailure | undefined {
  const result = boundedValidationPreflight(value, {
    allowFunctionAtPath: path => allowHandler && path === '$.handler',
...
function parseContract<T>(
  ...
  const preflight = preflightActionContract(value, allowHandler);
