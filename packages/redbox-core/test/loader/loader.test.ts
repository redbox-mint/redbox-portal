...
import {
  ...
  type RuntimeRecord,
  type RuntimeValue,
} from '../../src/runtimeValues';

interface RuntimeVm {
  runInNewContext(code: string, contextObject: object): RuntimeValue;
}

const runtimeVm: RuntimeVm = vm;

function requireRuntimeRecord(value: RuntimeValue, label: string): RuntimeRecord {
...
function requireRuntimeArray(value: RuntimeValue, label: string): RuntimeValue[] {
...
function runtimeFailure(error: RuntimeValue): RuntimeValue {
  return error;
}

async function createHookModule(
  sandboxDir: string,
  packageName: string,
  packageJson: object,
  indexJs: string
...
describe('redbox-loader', function () {
  ...
  afterEach(async function () {
    ...
    await fsPromises.rm(sandboxDir, { recursive: true, force: true }).catch(() => undefined);
  });
  ...
  describe('mergeRedboxConfig', function () {
    it('replaces brandingConfigurationDefaults arrays from hook config', function () {
          ...
          menu: {
            items: [
              { id: 'home-auth', labelKey: 'menu-home', href: '/researcher/home' },
              {
                ...
                href: '#',
                children: [{ id: 'plan-create', labelKey: 'create-rdmp', href: '/record/rdmp/edit' }],
              },
            ],
          },
          homePanels: {
      ...
      const menu = requireRuntimeRecord(merged.menu, 'branding menu');
      const menuItems = requireRuntimeArray(menu.items, 'branding menu items');
      const panels = requireRuntimeArray(
        requireRuntimeRecord(merged.homePanels, 'branding home panels').panels,
        'branding panels'
      );

      expect(menuItems.map(item => requireRuntimeRecord(item, 'menu item').id)).to.deep.equal(['home-auth', 'plan']);
      expect(requireRuntimeRecord(menuItems[1], 'second menu item')).to.not.have.property('requiredRoles');
      expect(panels.map(panel => requireRuntimeRecord(panel, 'home panel').id)).to.deep.equal(['plan']);
    });
  });
  ...
  describe('shouldRegenerateShims', function () {
    ...
    it('should return false via existing files and production env', async function () {
      ...
      await fsPromises.writeFile(path.join(sandboxDir, 'config/actionRegistry.js'), 'x');
      await redboxLoader.generateMigrationConfigShim(path.join(sandboxDir, 'config'), sandboxDir, []);

      const result = await redboxLoader.shouldRegenerateShims(sandboxDir, false);
      expect(result.shouldRegenerate).to.be.false;
      await fsPromises.writeFile(path.join(sandboxDir, 'config/migrations.js'), 'module.exports.migrations = [];');
      const stale = await redboxLoader.shouldRegenerateShims(sandboxDir, false);
      expect(stale.shouldRegenerate).to.be.true;
      expect(stale.reason).to.equal('core record-definition migration registration is missing');
    });

    it('should return true if the action registry startup shim is missing', async function () {
      process.env.NODE_ENV = 'production';
      await fsPromises.writeFile(path.join(sandboxDir, 'api/models/test.js'), 'x');
      await fsPromises.writeFile(path.join(sandboxDir, 'api/policies/test.js'), 'x');
      await fsPromises.writeFile(path.join(sandboxDir, 'api/middleware/test.js'), 'x');
      ...
      const result = await redboxLoader.shouldRegenerateShims(sandboxDir, false);
      expect(result.shouldRegenerate).to.be.true;
      expect(result.reason).to.include('config/actionRegistry.js is missing');
    });
  });
  ...
  describe('generateAllShims', function () {
    ...
    it('should delete marker file after regeneration', async function () {
      ...
      const accessResult = await fsPromises
        .access(path.join(sandboxDir, '.regenerate-shims'))
        .then<RuntimeValue, RuntimeValue>(() => undefined, runtimeFailure);
      expect(readRuntimeProperty(accessResult, 'code')).to.equal('ENOENT');
    });
  ...
  describe('generateConfigShims', function () {
    ...
    it('should sanitize package names with dots when generating config shims', async function () {
      const hookConfigs = [{ name: '@org/test.hook', module: '@org/test.hook' }];
    ...
    it('should merge agenda queue jobs as a map keyed by name', function () {
      ...
      const jobs = requireRuntimeRecord(merged.jobs, 'merged agenda jobs');
      const auditJob = requireRuntimeRecord(jobs['IntegrationAuditService-StoreIntegrationAudit'], 'audit job');
      const moveHistoryJob = requireRuntimeRecord(jobs['MoveCompletedJobsToHistory'], 'move history job');
      expect(Object.keys(jobs)).to.include.members([
        'IntegrationAuditService-StoreIntegrationAudit',
        'MoveCompletedJobsToHistory',
        'Figshare-TransitionRecordWorkflowFromFigshareArticleProperties',
      ]);
  ...
  describe('generateBootstrapShim', function () {
    ...
    function runBootstrapShim(
      content: string,
      migrationsRequire: () => RuntimeValue
    ): { exports: RuntimeRecord; capturedMigrations: RuntimeValue } {
      let capturedMigrations: RuntimeValue;
      const fakeCore = {
        coreBootstrap: async () => undefined,
        preLiftSetup: () => undefined,
        createGeneratedBootstrap: (
          _preLift: RuntimeValue,
          _core: RuntimeValue,
          _hooks: RuntimeValue,
          migrations: RuntimeValue
        ) => {
      ...
      const moduleStub: { exports: RuntimeRecord } = { exports: {} };
      runtimeVm.runInNewContext(content, {
        require: fakeRequire,
        module: moduleStub,
    ...
    it('should tolerate a missing config/migrations.js and default to no migrations', async function () {
      ...
      const missingShim = () => {
        const error = new Error("Cannot find module './migrations'");
        writeRuntimeProperty(error, 'code', 'MODULE_NOT_FOUND');
        throw error;
      };
    ...
    it('should rethrow MODULE_NOT_FOUND raised inside config/migrations.js', async function () {
      ...
      const missingHookInsideShim = () => {
        const error = new Error("Cannot find module 'some-uninstalled-hook'");
        writeRuntimeProperty(error, 'code', 'MODULE_NOT_FOUND');
        throw error;
      };

      expect(() => runBootstrapShim(content, missingHookInsideShim)).to.throw(
        "Cannot find module 'some-uninstalled-hook'"
      );
    });
  });
  ...
  describe('findAndRegisterHooks', function () {
    ...
    it('should return hook config registrations in processing order', async function () {
      ...
      expect(result.hookConfigs.map(hook => hook.name)).to.deep.equal(['redbox-hook-zeta', 'redbox-hook-alpha']);
    });
    ...
    it('should return bootstrap, api route, and migration registrations in processing order', async function () {
      ...
      expect(result.hookBootstraps.map(hook => hook.name)).to.deep.equal(['redbox-hook-zeta', 'redbox-hook-alpha']);
      expect(result.hookApiRoutes.map(hook => hook.name)).to.deep.equal(['redbox-hook-zeta', 'redbox-hook-alpha']);
      expect(result.hookMigrations.map(hook => hook.name)).to.deep.equal(['redbox-hook-zeta', 'redbox-hook-alpha']);
    });
  });
  ...
  describe('generateMigrationConfigShim', function () {
    ...
    const coreMigrationName = '@researchdatabox/redbox-core:20260905T000000-record-definitions';
    ...
    beforeEach(async function () {
      ...
      await createHookModule(
        ...
        `module.exports.registerRedboxMigrations = () => [{ name: '${coreMigrationName}', up: async () => {} }];`
      );
    });
    ...
    it('should generate a migrations.js shim aggregating hook and app-local migrations in name order', async function () {
      ...
      const result = await redboxLoader.generateMigrationConfigShim(configDir, sandboxDir, [
      ...
      const loaded = requireRuntimeRecord(loadRuntimeModule(shimPath), 'migration shim');
      const migrations = requireRuntimeArray(loaded.migrations, 'migration list');
      expect(migrations.map(migration => requireRuntimeRecord(migration, 'migration').name)).to.deep.equal([
    ...
    it('should throw on duplicate migration names in production', async function () {
      ...
      const shimPath = path.join(configDir, 'migrations.js');
      delete require.cache[require.resolve(shimPath)];
      expect(() => loadRuntimeModule(shimPath)).to.throw('Duplicate Redbox migration name: duplicate');
    });

    it('should name the offending source when a migration export is invalid', async function () {
      const migrationsDir = path.join(sandboxDir, 'api', 'migrations');
      await fsPromises.mkdir(migrationsDir, { recursive: true });
      await fsPromises.writeFile(path.join(migrationsDir, '007-bad.js'), `module.exports = { name: 'bad-migration' };`);

      await redboxLoader.generateMigrationConfigShim(configDir, sandboxDir, []);

      const shimPath = path.join(configDir, 'migrations.js');
      delete require.cache[require.resolve(shimPath)];
      expect(() => loadRuntimeModule(shimPath)).to.throw(
        'Invalid Redbox migration export from api/migrations/007-bad.js'
      );
    });
