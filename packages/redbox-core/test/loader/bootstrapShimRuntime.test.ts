...
describe('bootstrapShimRuntime', function () {
    ...
    beforeEach(async function () {
        sinon.stub(activeRecordDefinitions(), 'assertReady').resolves();
    ...
    it('fails the lift when active-definition readiness fails after hook bootstrap', async function () {
        const readiness = activeRecordDefinitions().assertReady as sinon.SinonStub;
        readiness.rejects(new Error('Active action unavailable'));
        const hook = sinon.stub().resolves();
        let failure: any;
        try { await runBootstrap(createGeneratedBootstrap(() => {}, async () => {}, [{ name: 'test', bootstrap: hook }])); }
        catch (error) { failure = error; }
        expect(failure?.message).to.equal('Active action unavailable');
        expect(hook.calledBefore(readiness)).to.equal(true);
    });
