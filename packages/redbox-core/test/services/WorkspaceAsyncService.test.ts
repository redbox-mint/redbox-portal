...
describe('WorkspaceAsyncService', function () {
  ...
  beforeEach(function () {
    ...
    mockSails.config.workspaceAsync = { allowedOperations: ['svc.meth'] };
  ...
  describe('start', function () {
    it('should create workspace async record', async function () {
      ...
      expect(
        (global as any).WorkspaceAsync.create.calledWith(
          sinon.match({
            name: 'test',
            started_by: 'user',
            status: 'started',
  ...
  describe('update', function () {
    ...
    it('should set date_completed if finished', async function () {
      ...
      expect(
        (global as any).WorkspaceAsync.update.calledWith(
          sinon.match({ id: '1' }),
  ...
  describe('pending', function () {
    it('should find pending records', async function () {
      ...
      (global as any).WorkspaceAsync.find.returns({ sort });
  ...
  describe('status', function () {
    it('should find records by status and recordType', async function () {
      ...
      expect(result).to.deep.equal(expected);
      expect((global as any).WorkspaceAsync.find.calledWith({ status: 'error', recordType: 'rt', branding: undefined }))
        .to.be.true;
      expect(sort.calledOnceWithExactly('id ASC')).to.be.true;
      expect(limit.calledOnceWithExactly(100)).to.be.true;
    });
