...
describe('AsynchsService', function () {
  ...
  describe('start', function () {
    ...
    it('should use provided date_started if present and date_completed is set', async function () {
      ...
      const execStub = sinon.stub().yields(null, expectedResult);
      (global as any).AsynchProgress.create
        .withArgs(sinon.match({ date_started: dateStarted }))
  ...
  describe('finish', function () {
    it('should mark progress as finished with current date', async function () {
      ...
      const execStub = sinon.stub().yields(null, expectedResult);
      (global as any).AsynchProgress.update
        .withArgs({ id: progressId }, sinon.match({ status: 'finished', date_completed: sinon.match.string }))
        .returns({ exec: execStub });
    ...
    it('should merge with provided progress object', async function () {
      ...
      const execStub = sinon.stub().yields(null, expectedResult);
      (global as any).AsynchProgress.update
        .withArgs({ id: progressId }, sinon.match({ status: 'finished', other: 'field' }))
        .returns({ exec: execStub });
  ...
  describe('get', function () {
    it('should find progress objects', async function () {
      ...
      const execStub = sinon.stub().yields(null, expectedResult);
      const limit = sinon.stub().returns({ exec: execStub });
      const sort = sinon.stub().returns({ limit });
      (global as any).AsynchProgress.find.withArgs(criteria).returns({ sort });
      ...
      expect(result).to.deep.equal(expectedResult);
      expect(sort.calledOnceWithExactly('id ASC')).to.be.true;
      expect(limit.calledOnceWithExactly(100)).to.be.true;
    });
  });
