...
function createRes(): TestResponse {
  const res = {
    ...
    type(this: TestResponse, value: string) {
...
describe('validateApiContractRequest policy', function () {
  ...
  it('attaches resolved route and validated request on success', function () {
    ...
    validateApiContractRequest(req, res, () => {
      nextCalled = true;
    });
  ...
  it('returns bounded Problem Details for an invalid authorization contract request', function () {
    const path = '/default/rdmp/api/authorization/scopes';
    const req = createReq({
      path,
      originalUrl: `${path}?limit=101`,
      url: `${path}?limit=101`,
      route: { path: '/:branding/:portal/api/authorization/scopes' },
      params: { branding: 'default', portal: 'rdmp' },
      query: { limit: '101', search: 'sensitive client value' },
    });
    const res = createRes();
    let nextCalled = false;
    ...
    expect(res.statusCode).to.equal(400);
    expect(res.headers?.['Content-Type']).to.equal('application/problem+json');
    ...
    expect((res.body as { requestId?: string }).requestId).to.be.a('string').and.not.empty;
    expect(JSON.stringify(res.body)).not.to.include('sensitive client value');
  });

  it('never reflects query-string credentials in an invalid authorization Problem Details instance', function () {
    const path = '/default/rdmp/api/authorization/roles';
    const req = createReq({
      path: undefined,
      originalUrl: `${path}?access_token=credential-secret&limit=101`,
      url: `${path}?access_token=credential-secret&limit=101`,
      route: { path: '/:branding/:portal/api/authorization/roles' },
      params: { branding: 'default', portal: 'rdmp' },
      query: { access_token: 'credential-secret', limit: '101' },
    });
    const res = createRes();

    validateApiContractRequest(req, res, () => expect.fail('invalid request reached the controller'));

    expect(res.statusCode).to.equal(400);
    expect(res.body).to.deep.include({ instance: path, code: 'authorization.invalid-request' });
    expect(JSON.stringify(res.body)).not.to.include('credential-secret');
  });

  it('returns Problem Details when an authorization contract route cannot be resolved', function () {
    const path = '/default/rdmp/api/authorization/not-registered';
    const req = createReq({
      path,
      originalUrl: path,
      url: path,
      route: { path: '/:branding/:portal/api/authorization/not-registered' },
    });
    const res = createRes();

    validateApiContractRequest(req, res, () => expect.fail('unresolved route reached the controller'));
    ...
    expect((res.body as { requestId?: string }).requestId).to.be.a('string').and.not.empty;
    expect(JSON.stringify(res.body)).not.to.include('route resolution failed');
  });
  ...
  for (const malformed of malformedRecordSchemaRequestCases) {
    it(`returns raw 400 Problem Details for malformed record-schema ${malformed.name} in both API versions`, function () {
      for (const apiVersion of ['1.0', '2.0']) {
        ...
        expect(res.headers?.['Content-Type'], `${apiVersion} ${malformed.name}`).to.equal('application/problem+json');
        expect(res.headers?.['Cache-Control'], `${apiVersion} ${malformed.name}`).to.equal('private, no-cache');
