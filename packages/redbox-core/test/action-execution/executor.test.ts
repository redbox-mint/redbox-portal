...
describe('Effect action execution', function () {
  ...
  it('B07 projects only detached transition evidence, never runtime extras', function () {
    const operation = createActionExecutionOperation('onTransitionWorkflow');
    const evidence = {
      transitionId: 'submit',
    ...
    const injected = {
      ...evidence,
      parameters: { secret: 'private' },
      expression: 'private',
      internal: { helper: () => 'private' },
    };
    operation.transition = injected;
    const summary = projectRecordHookExecutionAuditSummary(operation);
    expect(summary.transition).deep.equal(evidence);
    injected.parameters.secret = 'changed';
    injected.targetStage = 'changed';
    expect(summary.transition).deep.equal(evidence);
    Object.assign(summary.transition!, { sourceStage: 'changed', parameters: { secret: 'output' } });
    expect(injected.sourceStage).equal('draft');
    expect(injected.parameters.secret).equal('changed');
    expect(projectRecordHookExecutionAuditSummary(operation).transition).not.have.property('parameters');
  });

  it('B07 omits malformed and oversized transition evidence without object coercion or references', function () {
    const evidence = {
      transitionId: 'submit',
      definitionRevisionId: 'revision-1',
      sourceStage: 'draft',
    ...
    for (const field of Object.keys(evidence)) {
      for (const value of [
        ...
        const operation = createActionExecutionOperation('onTransitionWorkflow');
        operation.transition = Object.assign({}, evidence, { [field]: value });
        expect(projectRecordHookExecutionAuditSummary(operation)).not.have.property('transition');
      }
    }
    const operation = createActionExecutionOperation('onTransitionWorkflow');
    operation.transition = { ...evidence, transitionId: 'x'.repeat(128) };
    expect(projectRecordHookExecutionAuditSummary(operation).transition).deep.equal(operation.transition);
  });
