...
describe('registered legacy record action migration', function () {
  ...
  it('rejects every persisted negative fixture expression, including computed access to a known service', () => {
    for (const recordType of loadNegativeFixture().recordTypes) {
      for (const definition of recordType.hooks.onCreate?.pre ?? []) {
        assert.throws(
          () => migrate(definition, { context: 'record-lifecycle', mode: 'onCreate', phase: 'pre' }),
          (error: Error) => error instanceof LegacyRecordActionMigrationError && error.code === 'unknown-legacy-action'
        );
      }
    }
  });
