...
function scanSource(filePath: string, contents = fs.readFileSync(filePath, 'utf8')): ScannedOccurrence[] {
...
describe('A01 legacy record action inventory', function () {
  ...
  // Opt in because source archives and shallow clones need not contain the pre-migration commit.
  if (process.env.A01_VERIFY_HISTORY === '1') {
    it('reconciles all historical occurrences and exact locations with the pre-migration source tree', function () {
      this.timeout(30000);
      const baseline = '60aa2a07a777d6ffe6791eec4bf1ea3b621dda77';
      expect(inventory.scan.baselineRevision).to.equal(baseline);
      const git = (args: string[]) => execFileSync('git', args, { cwd: repositoryRoot, encoding: 'utf8' });
      const files = git(['ls-tree', '-r', '--name-only', baseline])
        .trim()
        .split('\n')
        .filter(
          file =>
            SHIPPED_CONFIG_SOURCE_GLOBS.some(pattern => path.matchesGlob(file, pattern)) &&
            !SHIPPED_CONFIG_EXCLUSION_GLOBS.some(pattern => path.matchesGlob(file, pattern))
        );
      const scanned = files.flatMap(file =>
        scanSource(path.join(repositoryRoot, file), git(['show', `${baseline}:${file}`]))
      );
      expect(sortedOccurrences(scanned)).to.deep.equal(sortedOccurrences(inventoryOccurrences(inventory)));
      expect(scanned).to.have.length(inventory.scan.expectedCounts.totalExecutableOccurrences);
      expect(scanned.filter(item => item.nesting === 'record-hook')).to.have.length(
        inventory.scan.expectedCounts.recordHookDefinitions
      );
      expect(scanned.filter(item => item.nesting !== 'record-hook')).to.have.length(
        inventory.scan.expectedCounts.nestedExecutableDefinitions
      );
      expect(new Set(scanned.map(item => item.expression)).size).to.equal(
  ...
  it('reconciles the canonical inventory with shipped source metadata', function () {
    const scanned = discoverShippedConfigSources().flatMap(file => scanSource(file));
