...
describe('Webservice TranslationController', () => {
    ...
    (global as any).sails = {
      services: {
        brandingservice: {
          ...
          getBrandFromReq: sinon.stub().returns({ id: 'authorized-brand', name: 'authorized' }),
        },
        i18nentriesservice: {
  ...
  it('preserves false query params when setting a bundle', async () => {
    ...
    expect((global as any).I18nEntriesService.setBundle.firstCall.args[0]).to.deep.equal({
      id: 'authorized-brand',
      name: 'authorized',
    });
