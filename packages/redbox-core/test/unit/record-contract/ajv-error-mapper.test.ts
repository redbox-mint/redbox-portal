...
describe('AJV-like record-schema error mapping', function () {
  ...
  it('handles missing fields, malformed paths, accessors, proxies, and malicious strings without throwing', function () {
    const throwingAccessors = Object.defineProperties(
      {},
      {
        keyword: {
          get: () => {
            throw new Error('keyword getter executed');
          },
        },
        instancePath: {
          get: () => {
        ...
        params: {
          get: () => {
            throw new Error('params getter executed');
          },
        },
      }
    );
    ...
    const malformed = [
      ...
      7,
      {},
      { keyword: 'type', instancePath: '/bad~2escape', params: { type: 'not-a-json-type' } },
      {
        keyword: 'required',
        instancePath: '/safe',
        params: { missingProperty: 'x'.repeat(257) },
      },
      { keyword: 'x'.repeat(10_000), instancePath: '/unsafe\npath', params: {} },
      throwingAccessors,
      revoked.proxy,
    ];

    expect(() => mapAjvErrorsToRecordSchemaProblems(malformed, 20)).not.to.throw();
    expect(mapAjvErrorsToRecordSchemaProblems(malformed, 20)).to.deep.equal({
      problems: [
        { code: 'record-schema.validation', pointer: '' },
        { code: 'record-schema.validation', pointer: '' },
        { code: 'record-schema.validation', pointer: '' },
        { code: 'record-schema.type', pointer: '' },
    ...
    expect(mapAjvErrorsToRecordSchemaProblems(42, 20)).to.deep.equal({
      problems: [],
      truncated: false,
    });
  });

  it('does not invoke hostile array entry accessors or proxy traps', function () {
    const accessorErrors: unknown[] = [];
    Object.defineProperty(accessorErrors, '0', {
      configurable: true,
      enumerable: true,
      get: () => {
        throw new Error('array entry getter executed');
      },
    });
    accessorErrors.length = 1;

    const proxiedErrors = new Proxy([], {
      getOwnPropertyDescriptor: () => {
        throw new Error('array descriptor trap executed');
      },
    });

    expect(mapAjvErrorsToRecordSchemaProblems(accessorErrors, 10)).to.deep.equal({
      problems: [{ code: 'record-schema.validation', pointer: '' }],
      truncated: false,
    });
    expect(mapAjvErrorsToRecordSchemaProblems(proxiedErrors, 10)).to.deep.equal({
      problems: [],
      truncated: false,
    });
  });
