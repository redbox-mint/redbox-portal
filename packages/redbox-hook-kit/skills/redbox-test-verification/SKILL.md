...
## Mocha service coverage
- For every new/modified service in `packages/redbox-core-types/src/services`, add an integration test under the monorepo root `typescript/test/unit/services`. Prefer naming the file after the service (e.g., `MyService.test.ts`).
- Integration best practice: do not mock or fake internal dependencies; assume the test container can talk to the full stack. The test should orchestrate creating real data (often via helper seed utilities), let the service execute against the database, and assert on the persisted state or returned payloads.
- Import shared fixtures or helpers from `typescript/test/unit/services/helpers` as needed, but focus on exercising the whole service flow (setup data, call the method or endpoint, then read back the data it should have created/updated).
