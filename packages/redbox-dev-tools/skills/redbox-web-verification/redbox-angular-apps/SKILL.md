...
Use the `redbox-dev-tools` CLI generator `generate_angular_app`:
...
- Use `CacheService.getNgAppFileHash(appName, type, prefix)` for hashed asset names. `type` selects the asset category (for example `js`, `css`, `map`, or other build artifact names your bundler emits) and influences the lookup logic; `prefix` is an optional path or name prefix to scope the lookup (for example `vendor/`). If omitted, defaults are used by `CacheService` and an invalid type may fall back to the default asset group.
  - Example: `CacheService.getNgAppFileHash("myApp", "js", "vendor/")` -> `"vendor/main.abc123.js"`.
...
Example `outputPath`: `../assets/angular/<name>` and `baseHref`: `/assets/angular/<name>/`
