...
1. Search the implementation for usages of `as any`, `(object as any)`, `as unknown as`, or other patterns that cast to `any` to bypass TypeScript types.
   ...
   - Area mapping: set `area` to match the location of the cast (use `services` for service code, `models` for Waterline models, `controllers` for backend controllers, and `angular` for Angular/front-end code).
     Example: `packages/redbox-core/src/services/FooService.ts` -> `area: services`.
...
1. Services extend `CoreService`.
   - Expected location: `packages/redbox-core/src/services`.
2. Each service has a unit test in `packages/redbox-core/test`.
...
2. Models are defined in `packages/redbox-core/src/waterline-models`.
...
4. Navigation configuration is set for any new pages or menus.
   - Check core-types config models (for example `packages/redbox-core/src/configmodels/MenuConfig.ts`) and any branding defaults touched by the feature.
5. Controllers include unit tests.
   - Expected location: `packages/redbox-core/test`.
...
"file": "packages/redbox-core/src/controllers/MyController.ts",
"lines": "L88-L96",
