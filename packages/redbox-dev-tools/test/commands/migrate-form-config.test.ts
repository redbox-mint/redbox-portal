...
describe('migrate-form-config command', () => {
  ...
  it('should migrate a legacy form config and write TS output', async () => {
    ...
    await program.parseAsync(
      ['node', 'redbox-dev-tools', 'migrate-form-config', '--input', inputPath, '--output', outputPath],
      { from: 'node' }
    );
    ...
    expect(content).to.contain(' as FormConfigFrame;');
  ...
  it('should respect global --dry-run and not write output file', async () => {
    ...
    await program.parseAsync(
      ['node', 'redbox-dev-tools', '--dry-run', 'migrate-form-config', '--input', inputPath, '--output', outputPath],
      { from: 'node' }
    );
