...
let modelGeneratorModule: typeof import('../../src/generators/model');
...
describe('ModelGenerator', () => {
  ...
  before(async () => {
    ...
    fs = await import('fs');
    os = await import('os');
    modelGeneratorModule = await import('../../src/generators/model');
  ...
  beforeEach(() => {
    ...
    fs.writeFileSync(
      path.join(coreTypesRoot, 'src', 'waterline-models', 'index.ts'),
      `
export * from './User';
...
`
    ...
    fs.writeFileSync(
      path.join(coreTypesRoot, 'src', 'waterline-models', 'types.ts'),
      `
...
`
    ...
    fs.writeFileSync(
      path.join(coreTypesRoot, 'src', 'sails.ts'),
      `
...
`
    ...
    fs.writeFileSync(
      path.join(coreTypesRoot, 'src', 'decorators', 'index.ts'),
      `
