...
`;
  ...
  private updateSailsNgCommonDictionaries(): void {
    const dictionaryOutlinePath = path.join(
      this.root,
      'packages',
      'sails-ng-common',
      'src',
      'config',
      'dictionary.outline.ts'
    );
    ...
    const dictionaryModelPath = path.join(
      this.root,
      'packages',
      'sails-ng-common',
      'src',
  ...
  private updateBaseVisitors(): void {
    const baseOutlinePath = path.join(
      this.root,
      'packages',
      'sails-ng-common',
      'src',
...
`;
  ...
  private updateValidatorVisitor(): void {
    const corePath = path.join(this.root, 'packages', 'redbox-core', 'src', 'visitor', 'validator.visitor.ts');
...
`;
  ...
  private updateAngularWiring(): void {
    ...
    this.updateTextFile(formModulePath, content => {
      ...
      updated = this.appendToArrayLiteral(updated, /declarations:\s*\[/, `    ${this.componentClassName},\n`);
      return updated;
    });
  }
}
