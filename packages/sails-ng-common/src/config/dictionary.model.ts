        ...
        if (targetMap) {
          for (const [targetKind, targetName] of Object.entries(targetObj ?? {})) {
            if (targetMap.has(targetKind)) {
              throw new Error(
                `Name defaults map already has an entry for source kind '${sourceKind}' name '${sourceName}', cannot add another one.`
              );
            } else {
...
function toClassDefMap<T>(items: { kind: string; def: T; class: string }[]): Map<string, T> {
...
export const FieldComponentDefinitionMap: VisitorComponentClassDefMapType =
  toClassDefMap<VisitorComponentDefItemsType>(VisitorComponentDefs);
export const FieldModelDefinitionMap: VisitorModelClassDefMapType = toClassDefMap<VisitorModelDefItemsType>(ModelDefs);
export const FieldLayoutDefinitionMap: VisitorLayoutClassDefMapType =
  toClassDefMap<VisitorLayoutDefItemsType>(LayoutDefs);
export const FormComponentDefinitionMap: VisitorFormComponentClassDefMapType =
  toClassDefMap<VisitorFormComponentDefItemsType>(VisitorFormComponentDefs);
