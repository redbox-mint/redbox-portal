 ...
 */
const FORM_VALIDATOR_EMAIL_REGEXP = /^(?=.{1,254}$)(?=.{1,64}@)[a-zA-Z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-zA-Z0-9!#$%&'*+/=?^_`{|}~-]+)*@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)*$/;
...
export const formValidatorsSharedDefinitions: FormValidatorDefinition[] = [
    ...
    },
    {
        ...
        create: (config) => {
            ...
            const expression = formValidatorGetDefinitionItem(config, optionExpressionKey)?.toString() ?? "";
            const optionEvaluatorKey = "evaluator";
            const evaluator = formValidatorGetDefinitionItem(config, optionEvaluatorKey) as (expression: string, control: FormValidatorControl) => boolean;
            return (control) => {
                if (control.value == null || formValidatorLengthOrSize(control.value) === 0) {
                    // don't validate empty values to allow optional controls
                    return null;
                ...
                const value = control.value?.toString();
                return evaluator(expression, control)
                    ? null
                    : {
                        [optionNameValue]: {
                            [optionMessageKey]: optionMessageValue,
