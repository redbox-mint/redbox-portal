  ...
  export class MongoStorageService extends services.Core.Service implements StorageService, DatastreamService {
    ...
    private sanitizeRecordHookExecutionSummary(value: unknown): Record<string, unknown> | undefined {
      ...
      if (
        ...
        'transitionId' in transition &&
        typeof transition.transitionId === 'string' &&
        transition.transitionId.length <= 128 &&
        'definitionRevisionId' in transition &&
        typeof transition.definitionRevisionId === 'string' &&
        transition.definitionRevisionId.length <= 128 &&
        'sourceStage' in transition &&
        typeof transition.sourceStage === 'string' &&
        transition.sourceStage.length <= 128 &&
        'targetStage' in transition &&
        typeof transition.targetStage === 'string' &&
        transition.targetStage.length <= 128
      ) {
        summary.transition = {
          transitionId: transition.transitionId,
          definitionRevisionId: transition.definitionRevisionId,
