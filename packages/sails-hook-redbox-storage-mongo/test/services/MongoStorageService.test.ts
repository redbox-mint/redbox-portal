...
describe('MongoStorageService', function () {
  ...
  it('persists only the bounded execution-summary audit whitelist', async function () {
    const transition = {
      ...
      parameters: { secret: 'must-not-persist' },
      expression: 'must-not-persist',
    };
    ...
    expect(saved.transition).deep.equal({
      transitionId: 'submit',
      definitionRevisionId: 'revision-1',
      sourceStage: 'draft',
      targetStage: 'published',
    });
    transition.parameters.secret = 'changed';
    transition.sourceStage = 'changed';
    expect(saved.transition.sourceStage).equal('draft');
    saved.transition.targetStage = 'changed';
    expect(transition.targetStage).equal('published');
    expect(JSON.stringify(saved)).not.include('must-not-persist');
  ...
  it('B07 rejects malformed transition audit fields at native storage projection', async function () {
    const evidence = {
      transitionId: 'submit',
      definitionRevisionId: 'revision-1',
      sourceStage: 'draft',
    ...
    for (const field of Object.keys(evidence)) {
      for (const value of [null, ['secret'], { nested: { secret: 'private' } }, 'x'.repeat(129)]) {
        await service.createRecordAudit({
          redboxOid: 'oid-1',
          action: 'save',
          executionSummary: {
            schemaVersion: 1,
            trigger: 'record-hook',
            transition: { ...evidence, [field]: value },
          },
        } as any);
        expect(RecordAudit.create.lastCall.args[0].executionSummary).not.have.property('transition');
      }
    }
  });
